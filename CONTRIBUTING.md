...
**How this repository is published.** Development happens in a private
repository and each release is published here as a single reviewed commit
(see [HISTORY.md](HISTORY.md)). That means `git log` is a release timeline,
not a development log — worth knowing before you go looking for the commit
that introduced something. Issues and pull requests here are read and
answered normally; an accepted change is merged privately and ships in the
next published release, credited in its message.
...
- **The test suite is the spec.** Many tests freeze settled behavior
  ...
  first.
- The money conventions live in CLAUDE.md ("Money conventions") — sign
