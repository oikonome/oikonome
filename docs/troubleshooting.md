...
**Mail worked, then silently stopped.** Cloud hosts block outbound
25/465/587 without warning, and some enforce it on droplets that were
already running. Switch `OIKONOME_SMTP_PORT=2525` — Postmark, SendGrid and
Mailgun all accept it — and recreate app + worker. The worker probes the
