...
1. **Full-screen guided wizard** after the /setup account creation, so a
   fresh instance opens on a guided flow rather than a bare Accounts
   page. The Today "Welcome" checklist card is the fallback.
...
to the accounts retry form). `/api/onboarding` carries `wizard_done`;
