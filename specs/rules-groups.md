...
The Rules page listed every categorization rule on one flat table. A lived-in
ledger has thousands of them, which rendered all at once and buried the
handful the user actually authored. The shape it took instead:
...
- Edit promotion already existed (`upsert_user_rule`'s ON CONFLICT sets
  `source='user'`); disable promotion is new (`set_rule_disabled`).
...
- Viewers stay read-only exactly as before; owner-only writes.
