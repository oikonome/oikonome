...
- **Fleet**: per-tenant row — owner email, created, status, user count
  ...
  `items`, so an Item a failed release orphaned is still counted).
...
- **Delete-with-grace.** The console delete now defaults to a scheduled
  ...
  (`mode=immediate`) keeps the old unrecoverable path for the abuse case;
...
(migration 072) replace it, in value order:
...
console ON (an existing `.env` keeps working across the upgrade) but can
never open it. The tidy hosted configuration is `OIKONOME_ADMIN_ENABLED=1`
...
no longer keeps that branch unreachable on its own.
...
unchanged token-only self-host path).
