...
The budget lives in one mutable settings document, so every closed month
used to be judged against *today's* numbers, retroactively. Raise the
budget in April and March's verdict quietly flips; months that predate the
install entirely rendered as fully-budgeted "UNDER BUDGET" report cards
with today's budget and zero spend.
