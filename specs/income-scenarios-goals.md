...
1. **Named scenario list.** Any number of named scenarios ("full salary",
   ...
   cash forecast, and plan surplus. The current hardcoded pair
   (saving / not-saving biweekly) migrates to two list entries, the
   toggle to the active marker — no behavior change for existing config.
