...
if [ -f .env ]; then
  ...
  # (A partial .env stub sitting beside a live stack is exactly how an
  # install re-run mints a mismatched OIKONOME_APP_PASSWORD.)
  ...
  # can sort in creation order (dev, then demo1, demo2 …) portably — the first
...
# It is GENERATED, never tracked — it is a list of commit subjects, and this
# repository publishes releases rather than development history.
# No-op without git or the anchor tag, which is what leaves a published
# tree's release-only manifest and a demo archive copy exactly as shipped.
...
for i in $(seq 1 $TRIES); do
  ...
  if curl -sf "http://localhost:${PORT}/readyz" >/dev/null 2>&1; then
    ...
    B "2. Then open the Feedback tab - it walks you through"
    B "   everything and collects your feedback."
    B ""
