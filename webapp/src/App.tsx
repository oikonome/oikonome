...
// Login page unified onto the server-rendered /login (single login page):
// logged-out users are redirected there instead of a second in-SPA screen.
...
// the rest used to ship in one bundle that had to be parsed before the
// first API call could even be made, and two thirds of it was pages the
// session would never open. Keyed by route so warmRoute() below can start
...
const PAGE_TITLES: Record<string, string> = {
  ...
  // The two /setup routes fell through to the bare wordmark, so the one
  // kind of page you are most likely to leave a tab open on — a
  // half-finished wizard — was the one whose bookmark said nothing.
...
function ScrollToTop() {
  ...
  // movement within one — and it used to scroll the whole page to the top
  // the moment the ✎ was clicked, because the effect re-ran when the
  // navigation type flipped from POP to REPLACE. Remember the last path
  // and act only when it actually differs.
...
// external origins outright (that is what silently killed Turnstile once),
// and currentColor is what lets one glyph serve idle, hover and active
...
export default function App() {
  ...
  // expensive thing on the frame. Measured on Transactions: two main-thread
  // stalls of 242ms and 283ms per click, during which the width transition
  // freezes half-collapsed and the rail reads as broken. The rail's open
  ...
  // mouseleave, which reopened the rail; and once collapsed the pointer is
  ...
  //
  // This replaced the horizontal-strip version, which ran on every
  // navigation to centre a tab in the old bottom scroller. The drawer does
  // not scroll sideways and is closed on every navigation, so running it
  // there would only scroll an off-screen panel.
  useEffect(() => {
    if (!drawerOpen) return;
  ...
  // writes are refused wholesale (shared printed login), so the cycle
  // used to swallow a 403 and then get quietly reverted by the reconcile
  // below the next time /api/settings was refetched — the control worked
  // for a few minutes and then undid itself.
  ...
  if (me.isError) {
    ...
    // the server says so plainly — but the SPA had no branch for it, so the
    // one lockout state that is RECOVERABLE was reported as "can't reach
    // the server", i.e. as a bug on our side rather than a countdown the
    ...
    // still asleep; that one failed request used to replace the whole UI
    // with this card, and only a manual reload brought it back. Auth,
    ...
    <>
      ...
      <header className="top" ref={headerRef}><div className="bar"
        onClick={(e) => {
          ...
          // in .topline, OUTSIDE nav, so a handler down there never saw it:
          // clicking the logo left the panel stuck open — the same bug one
          // element over. Anything that navigates from the rail belongs here.
          //
        ...
        {/* one <nav> serves both shapes — a left icon rail on
            desktop, the thumb-reachable bottom strip on a phone. Same DOM,
            different CSS, so a destination can never exist in one and not
            the other. Groups are the legacy order: manage / reports /
            wealth / system. */}
        <nav id="mainnav">
          ...
          <button type="button" className="rail-pin"
                  ...
                  onClick={(e) => {
                    ...
                    // links do: unpinning left focus on this button, and
      ...
      <main>
        ...
        <footer className="mut" style={{ textAlign: "center", fontSize: 12,
          ...
          {/* the version opens the release notes on GitHub — the index, not
              the tag: a tag page 404s while a publish is still deferred */}
...
// Each of these owns a query App used to hold. Their polls and resolves
// used to re-render App — the nav, the theme, the drawer AND the current
// page — every 20 seconds; now they re-render one chip, one bell, one
// tab. What each one fetches starts once /api/me has resolved, because
// none of them mounts before the auth gate passes.

function SyncChip({ viewer }: { viewer: boolean }) {
  const qc = useQueryClient();
  // header chip: relative age of the last successful pull (legacy last_sync)
  // while a sync is in flight this is the progress feed, so it
  // polls at a pace you can watch. Idle it is 20s, not the 60s it used to
  // be, and that is the whole difference between showing scheduled syncs
  ...
  return (
    <span className={"syncchip" + (syncing ? " syncing" : "")}
      ...
      {syncing && (
        ...
        // width 0 shows nothing moving at all — measured exactly
        // that: "syncing 0 of N" over a 0px bar.
...
function SetupTab({ owner }: { owner: boolean }) {
  ...
  return (
    /* Same anatomy as every sibling — .nav-i icon + .nav-t label —
       because the collapsed rail clips anything else: the old
       inline green pill had no icon and a bare text node, so the
       rail showed "Fini…". The green lives on the class instead. */
...
const AppRoutes = memo(function AppRoutes({ me, homeWeb, homeKnown }: {
  ...
  return (
    <Routes>
      ...
      {/* the planner lives on /budget now */}
      <Route path="/balance"
             element={<Navigate to="/budget" replace />} />
      {/* Spending is the Cash Flow "spending" tab now, not a page of its
      ...
      {/* Fees is a section of Net Worth now, not a tab —
