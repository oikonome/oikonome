...
function OverviewSection({ entity }: { entity: Entity }) {
  ...
  // above this one excludes card payments (LOAN_PAYMENTS_CREDIT_CARD_PAYMENT)
  // and transfers (TRANSFER_*), so
...
function EntityDetail({ entity, flaggedUnassigned, onChange, tab,
                        combined, combining, onCombine, onAdd }:
  { entity: Entity; flaggedUnassigned: number; onChange: () => void;
    tab: BizTab; combined?: boolean; combining?: boolean;
    ...
    <div className="card" style={{ marginTop: 0 }}>
      <div style={{ display: "flex", alignItems: "baseline", gap: ".6rem",
        ...
        <label className="sub" style={{ marginLeft: "auto", display: "flex",
          ...
          <input type="checkbox" checked={!!combined} disabled={!!combining}
...
function EntitiesSection({ tab }: { tab: BizTab }) {
  ...
  // a combined-view write in flight: two absolute writes racing (check,
  // then uncheck before the first returns) can land out of order, so the
  // checkbox waits for the one it sent
  ...
  return (
    <>
      ...
      {entity && <EntityDetail entity={entity} tab={tab}
        combined={s.combined} combining={combining} onCombine={(v) => {
          if (combining) return;
          setCombining(true);
...
export default function Business() {
  ...
  // in the URL so a tab is linkable and survives a reload
