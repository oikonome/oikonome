// Accounts: institution-grouped table with
...
// Also SimpleFIN connect, Plaid hosted link and manual accounts.
...
// status dot: the whole ok/error/stale signal, left of the name
function Dot({ conn }: { conn: Connection | null }) {
  ...
  // itself, and red for it would make red stop meaning anything.
  ...
  if (kind === "ok") {
    ...
    // page doesn't look like an app bug (nothing to fix here). Only DOWN:
    // Plaid's DEGRADED measures fleet-wide NEW-LOGIN success and major
    // banks sit there for weeks at a time, so amber for it would paint
    // whole pages of healthy, syncing accounts at once and make amber
    // mean nothing. An item that is ok and syncing stays green through it.
...
// the "run" affordance for a script-fed item: collectors run on the host,
// outside the product, which CANNOT execute them (that's the whole
// credentials-stay-on-your-machine design). So "run" opens a short note,
// honest about where it runs.
function ScriptRunHint({ script }:
  ...
  return (
    <>
      {" "}<button type="button" style={{ padding: ".05rem .45rem", fontSize: 12 }}
              ...
              onClick={() => setOpen(!open)}>run</button>
      {open && (
        <span style={{ display: "block", margin: ".4rem 0 .2rem",
          ...
          not in the app — it holds your source credentials there by design.
          To run it now, run the collector the way you installed it (e.g. its
          systemd timer or the script&apos;s README).
        </span>
      )}
    </>
  );
...
export default function Accounts({ embedded }: { embedded?: boolean } = {}) {
  ...
  useEffect(() => {
    ...
    // spoof ("…sign in at evil.com"). The app's own producers are short and
  ...
  for (const [itemId, accs] of byItem) {
    ...
    // an obvious way back. Archived means closed lineages.
  ...
  // parsing it as a date would blank this line.
...
function GroupRows({ g, archived, hidden, mayEdit, owner, primary, excluded,
  ...
  // people the warning means nothing. Fall back to the status test only
  // for a server that does not send the field.
  ...
  // fixed-width dot+sync box so institution names line up;
  ...
  const badge = (
    <>
      ...
      {/* The BANK's own login rail is degraded fleet-wide (Plaid
          ...
          syncing row it would be a standing false alarm; DOWN warrants it
          everywhere. */}
...
function EditRow({ a, owner, primary, excluded, onDone, onCancel }: {
  ...
  // renames. An empty name must therefore still be sent: guarding the save
  // on a non-empty name would make the server's revert path unreachable.
...
function ConnectCard({ onDone, hosted = false, used, cap }: {
  ...
  // the bank tab is slow to come back; a second click would open a second
  ...
  return (
    <div className="card">
      ...
      {/* The two cards are peers, so they carry the same SHAPE: a
          ...
          would be a button in a field of empty space beside a dense
          importer — equal height, nothing like equal weight. */}
