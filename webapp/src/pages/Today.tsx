...
export default function Today({ date = null }: { date?: string | null }) {
  ...
  // after them), so the durable path is a 403 here and the toggle used to
  ...
  const ym = `&y=${period.y}&m=${period.m}`;
  ...
  return (
    <>
      ...
      <div className="card">
            ...
            {d.why && d.why.entries.length > 0 && (
              <details className="hw-more">
                ...
                {/* the server writes a recovery sentence when over —
                    mobile already renders it; the web dropped it */}
                {d.why.recovery && (
        ...
        {/* "This month's plan" demoted to ONE flow line — monthly facts,
      ...
      {adv && (fc || d.recent.length > 0 || paidRows.length > 0) && (
        <div className="card">
          ...
          {/* legacy fc_chart: charts.line static, h=220, zero_line, fill,
