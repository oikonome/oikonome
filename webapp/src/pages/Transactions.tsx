// Transactions — legacy transactions.html parity: month browse, universal
...
export default function Transactions() {
  const now = new Date();
  const [sp] = useSearchParams();         // legacy /transactions?… deep links
  ...
  // The toolbar carried eleven always-on controls. The three people
  // touch constantly stay in the row; date range, owner and the reimbursement
  ...
  const params: Record<string, string> = {
    ...
    // a bucket listing is the engine's own row set — the lenses below
    // would only empty it under a non-zero chip, so none rides along
    ...(bucket ? {} : {
      ...(reimbF ? { reimb: "1" } : {}),
  ...
  const clear = () => {
    ...
    setChannelF(""); setChecksF(false); setBucket(""); setMonthScope(false);
    setPage(1);
  };
