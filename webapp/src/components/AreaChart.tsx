// Single-series area/line chart — the SPA port of the legacy charts.
// Two variants, matching the two legacy render paths:
//   "interactive" (default) — the base.html JS chart look: 8%-padded y range,
//     4 gridlines, bottom-anchored area, no dots, estimated segment dashed
//     over a faint band. (Hover crosshair kept; zoom not ported.)
...
// → -1, not -2) and printed the minus inside ("$-1k")
...
export default function AreaChart({ points, height = 240,
  ...
  color?: string;
  second?: [string, number][];  // dashed 6 4 secondary series (legacy orange)
  third?: [string, number][];   // dotted 2 3 tertiary series (legacy green)
  ...
  return (
    <svg ref={ref} viewBox={`0 0 ${W} ${H}`}
      ...
      {/* dated events (card autopays): behind the series, so the money line
          ...
          because the legend owns the top-right corner (labels started at the
          top once and struck straight through it). */}
      {events.map((e, k) => (
        <g key={`ev${k}`}>
