...
#
#   * The patch is BASE + commits-since, not commits-since alone. Squashing
#     the history destroys every tag, so a base-blind count would restart
#     at 1 and instances would display a version far below the one they
#     had shipped. After a squash the single commit is tagged with the
#     version it carries and this arithmetic keeps counting from there.
#
