...
#
# The control-plane tables (users, sessions, password_resets, …) have NO
# row-level security and are global to the test database, so two suites
# sharing one database produce phantom failures in test_auth_hardening /
# test_passkeys that look exactly like a broken diff.
#
# OIKONOME_TEST_DB (server/tests/util.py) gives each parallel run its own
# database name; this check refuses a run that would share one.
#
