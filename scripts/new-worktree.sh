...
#
# WHY: two people writing ONE checkout produce, quickly, (a) a phantom
# suite failure from the shared test database and (b) a `git add -A` that
# sweeps the other's files into a commit under an unrelated message.
# Neither is fixable by agreement — one index and one database mean one
# writer. Separate worktrees give each line of work its own index;
# OIKONOME_TEST_DB gives each its own control-plane tables. A convention is
# not a lock; this is.
set -euo pipefail
cd "$(dirname "$0")/.."
