...
NSI (github.com/osmlab/name-suggestion-index, ISC license) is the
...
Usage:
 python3 scripts/refresh-merchant-seed.py [path/to/nsi.min.json]

Without an argument it downloads the latest release from the npm registry
(the repo's dist/ is only published there and on their site).
"""
...
def names_of(item: dict) -> set[str]:
    ...
    for n in raw:
        if not n:
            continue
        up = n.upper().strip()
        if (len(up) < MIN_LEN or up in GENERIC
