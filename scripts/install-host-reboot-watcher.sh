...
# the check logic lives in a real script, not inline ExecStart shell:
# systemd would expand the % in `stat -c %Y`. $UID_C is baked in at install.
...
UNIT
# remove the unnamespaced unit an older install may have left
