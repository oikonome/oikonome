...
# able to tell the operator where their database went. The one that used to
# fire without it ("couldn't start postgres") is exactly the abort a
# self-hoster hits with a bad image or a full disk, and reading it they
# believe the upgrade destroyed the database — one panicked `reset` or
# `rm -rf` away from actually destroying the folder that still holds it.
