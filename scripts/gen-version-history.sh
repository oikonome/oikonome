...
#
# The output is GENERATED, never committed (it is gitignored): a tracked
# manifest is a list of commit subjects, and this repository publishes
# releases rather than development history, so checking it in would carry
# that history in as a data file. install.sh rebuilds it on every
# install/upgrade, so the running instance's panel is current.
#
# Without the anchor tag this exits without touching the output, which is
# deliberate: a published tree ships a release-only manifest written at
# publish time, and a clone that lacks the development history must not
