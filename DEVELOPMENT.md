...
the codebase. For architecture and money conventions, see `CLAUDE.md`.
...
`make test` now refuses to start when another suite is already running
...
(`scripts/test-db-fresh.sh`) and `tests/util.py` recreates it — all 123
migrations take about 1.5s, which is cheaper than one slow test, and every
...
2. Implement it; keep the money conventions in `CLAUDE.md`.
...
**This repository is published per release** (see `HISTORY.md`): each release
lands here as a single commit, tagged with the version an instance built from
that tree reports, so there is no `make release` here.

What that means in practice:

- `git log` is a release timeline. To see what changed in a version, read its
  commit and its tag, or the Doctor page of a running instance.
- The version-history panel in the admin console reads
  ...
  because the image carries no `.git`. It is **generated, never committed**
  (`make version-history`, and `install.sh` refreshes it on every
  install/upgrade) — a tracked manifest is a list of commit subjects, which
  is development history by another name. A published tree ships a
  release-only manifest, and the generator leaves it alone in a clone that
  has no anchor tag. Commits below a later series' anchor keep the version
  they shipped as — a new series never renumbers released history.
