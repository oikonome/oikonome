...
# ten minutes — longer than any dump we have measured) and then refuses,
...
cmd_logs() {
  ...
  if [ -n "${OIKONOME_ASSUME_YES:-}" ]; then
    ...
    # minted but missing from one list rides out verbatim — ?invite= did
    # — so server/tests/test_log_link_scrubbing.py holds the copies equal.
