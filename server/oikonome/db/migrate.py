...
def run(admin_dsn: str | None = None) -> list[str]:
    ...
    try:
        ...
        conn.execute(
            ...
            # the Plaid Item ledger is APPEND-ONLY — the burn
            # count of Trial-plan slots must be monotonic (Plaid never
            # refunds a slot), so the app role may only insert and read.
