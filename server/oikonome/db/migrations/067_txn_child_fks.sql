...
--              removing an account would be worse than losing the link.
--
...
-- Idempotency: one movement per (transaction, kind), so a double-submit
-- from the Transactions category dropdown cannot record the same owner
-- contribution twice and double owner equity. Movements with no
-- transaction are unconstrained.
--
-- De-dup FIRST: CREATE UNIQUE INDEX on existing duplicates raises, rolls back
