...
--
-- 081 promoted the counterparty from the bank's statement line to
-- merchant_name as the bare person. That lost the channel in the ledger —
-- and the same person paid via Venmo and via Zelle are different things
-- to a reader and to grouping (082 ships the prefixed form directly).
-- sync/base.py now derives the prefixed form on ingest; this re-derives
-- the counterparty exactly as 081 did and prefixes the rows whose
-- merchant is still the bare parse — on a fresh install that is every row
-- 081 just touched, on a live one the same. Rows where the aggregator
