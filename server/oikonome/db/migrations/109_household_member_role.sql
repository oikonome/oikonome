-- A household has three roles now: the owner, members who can edit the
-- money, and view-only readers. The column has always been free text with
-- 'owner' as its default, which was survivable while there were two
-- values and one writer. With three — and with an owner picking between
-- them from a dropdown — a typo or a restored row carrying something else
