...
--
-- A guard in each writer covers only the writers that exist when it is
-- written. The column itself is made incapable of holding anything but a
-- document, so every writer — the collectors' push paths, the next
