...
--
-- A guard in each reader covers only the readers that exist when it is
-- written. The column itself is made incapable of holding the wrong shape,
-- so every reader of these tables — present and future — inherits that, in
-- both clients and in the collectors' push paths.
--
...
-- collector pushes that order again, which rewrites items_json. A
-- constraint must apply to the data that exists, so such rows are reset
-- rather than refused.
DO $$
DECLARE t TEXT;
BEGIN
