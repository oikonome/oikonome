...
--
-- merchant_dedup's doctrine — the one the whole display layer follows — is
-- that `name` and `merchant_name` are the SOURCE's words, written only by
-- sync, and that the app's own identity for a payee lives somewhere else so
-- it stays reversible and a re-sync cannot fight it. Overwriting
-- merchant_name would discard what Plaid actually said.
--
...
-- one chain name resolves to the pump on one row and the warehouse on the next.
...
-- Restore merchant_name from the payload where 091 appended the fuel arm.
-- Only rows whose stored merchant_name disagrees with the source's, and only
-- where the difference is exactly that fuel suffix — a row a
...
-- Derive the outlet the same way sync/base.py (fuel_arm) does, over the
