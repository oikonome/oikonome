...
--
-- Row-by-row LOWER(...) LIKE '%q%' over eight columns, parsing the raw
-- JSONB for the bank's description and running four correlated EXISTS
-- sub-selects (canonical map, merchant row, receipt items, Amazon order
-- items via jsonb_array_elements) per row, takes seconds on a large
-- ledger. Everything that belongs to the row itself lives in
