...
Companion to the upgrade triggers (PG conns pinned >20, sustained
PoolTimeout, /api p95 >2s): the sampler records the numbers those
triggers watch, so a spike is visible live in the console AND
reconstructable the next day.
...
THRESHOLDS = {
    ...
    "pg_total_conns": (16, 21),      # ">15 amber, >20 red" vs a 25 cap —
                                     # only for rows that never recorded
                                     # the server's own max_connections
    "pg_conn_pct": (64.0, 84.0),     # the same two points as a share of
                                     # the REAL cap: 16/25 and 21/25
...
def pg_conn_lines(max_conns) -> tuple[float, float]:
    ...
    25-cap constants when the cap was never sampled."""
    if not max_conns:
...
def breaches(row: dict, prev_timeouts: int | None = None) -> dict[str, str]:
    ...
    # carries it: 16 open connections is two-thirds of a managed 25-cap and
    # a sixth of a self-hosted container's 100. The absolute fallback keeps
    # rows from before the cap was sampled classifying as they always did.
