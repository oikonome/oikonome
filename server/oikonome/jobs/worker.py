...
Schedule (cron, UTC — per-tenant send-time preference comes later):
  * sync_all        hourly/tenant — pull every simplefin/plaid item, heartbeat;
                                   ...
                                   the fleet never syncs in one :00 burst;
...
def _recipients_raw(conn, tenant_id: str) -> list[str]:
    ...
    OWNER INCLUDED. "Owner always receives" was the right default for the
    silent-drop bug above, but it also made the schedule all-or-nothing for
    the one person who configures it: no way to keep the daily going to a
    partner while opting yourself out. Muting is explicit per-address
    state, so it cannot recreate the silent drop — Settings shows it.
    ...
    try:
        if recips:
            ...
            # household's balances and spending is the recipient's, and until
            # now only the first one was ever asked. An invited-but-unanswered
            # address is skipped silently here — Settings is where that state
            # is reported, because the person who needs to know is the owner,
            # not the log.
            if recips:
...
# wrong for the tenant, who then waited up to an hour for the next sweep
# even though something (a Plaid webhook, a fresh bank link, the ↻ button)
# had just said there was data to fetch.
#
...
def _sync_pass(conn, tenant_id: str, since_days: int, progress,
    ...
    # every door into a sync records it, so the UI can say a
    # sync is happening no matter who started it. Written only by
    # /jobs/sync/start, job_progress leaves the hourly cron — the sync a
    # user actually experiences most of the time — completely invisible:
    # the header chip goes on reading "synced 58m ago" while a pull is
    # running.
    #
    ...
    try:
        ...
        if _mx.creds(conn) is not None:
            ...
            # plaid/simplefin (previously MX outages only showed as an
            # in-app status dot).
            def _mx_status():
...
def emails_due(conn, now_utc: dt.datetime) -> list[str]:
    ...
    the legacy behavior (daily at email_send_hour_utc, UTC). Each
    cadence fires at most once per local day (job_runs guard)."""
    from ..engine import budget
    from .. import localtime
    cfg = budget.load_config(conn)
...
def email_tenant(tenant_id: str, send_it: bool = True,
    ...
    try:
        ...
        # emails_due schedules by. Building on UTC date.today() rendered
        # last week / last month on installs whose local midnight hadn't
        # crossed UTC yet, and building on the instance's zone does the
        # same to a household living west of it.
...
# syncs there every hour — same hourly cadence, but the fleet never piles
# onto :00 together. On the hour, every tenant syncs at once: fine at a
# handful of tenants, but the sweep is sequential, so at N tenants the last one's
# "hourly" sync starts N-1 pull-durations late, every Plaid call lands in
# one burst against the same client_id rate limit, and the spike shares
# its second with the other :00 crons. The hash is stable (crc32 of the
...
# interval, so 24 makes it two-hourly and halves the calls. Raise it when
# the fleet grows — the poll is a safety net under the webhooks, not the
# primary path, and its real cost is balance/holdings freshness, which is
# the only thing no webhook covers. Env-overridable so it can be tuned on
# a busy instance without a deploy.
...
def purge_scheduled_deletions() -> list[str]:
    ...
    try:
        ...
        for tid in candidates:
            ...
            # tenant_purge_failed audit row) made the audit trail claim an
            # erasure had not happened when it irreversibly had. The mail
