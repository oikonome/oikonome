...
def states(emails: list[str]) -> dict[str, dict]:
    ...
    One query for the whole list, rather than `state_for()` per failing
    address, each opening its own connection — rendering one Settings card
    must not cost a connection per bouncing recipient. An address with no
...
def _reactivate(email: str, provider_id: str) -> None:
    ...
    # anything but Postmark's numeric bounce id (a stored row may still
