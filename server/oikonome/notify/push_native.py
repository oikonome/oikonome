...
who wants the old routing-only payload sets `OIKONOME_PUSH_CONTENT_FREE=1`
...
def send_tenant(conn, tenant_id: str, event: str,
    ...
    if not url:
        ...
        # token on a relay-less install "delivered 0" with an empty detail,
        # and the test-push door then blamed the user's sign-in for what is
        # a server-config story
