...
BIZ_SOFTWARE = [("Creative suite — Brightline", 45.00),
                ("Design tool subscription — Inkwell", 12.00),
                ("Domain & hosting — Northwind Hosting", 20.00),
                ("Email delivery — Mailcrate", 10.00)]
...
class _Gen:
    ...
    def card_spending(self):
        ...
        # not a statement at all — treating it as one would double-count the
        # open cycle, and on the 1st of a month would show $0 card debt. The
        # card carries two different numbers:
    ...
    def business(self):
        ...
        while d <= self.today:
            ...
            # (~$25-45k/yr) next to the household's salary; much more reads
            # as a full-time job and the household's money story stops
            # adding up.
...
def _generate(g: _Gen) -> _Gen:
    ...
    seed() and the test fixture both call this, so a new generator step
    reaches the demo tests the moment it is added here — a hand-kept copy
    of the list could silently leave one out."""
    g.payroll()
    g.housing_and_bills()
    g.card_spending()
    g.investing()
