...
def verify(secret: str, code: str, at: float | None = None) -> bool:
    ...
    # compare_digest raises TypeError on non-ASCII input, so a non-ASCII
    # code is simply wrong: the short-circuit keeps the compare total (a
    # TOTP code is always 6 ASCII digits; anything else is wrong by
    # definition, not an error).
    if not code.isascii():
        return False
    counter = int((at or time.time()) // STEP)
    # A malformed base32 secret raises out of b32decode (binascii.Error, a
    # ValueError). The enrollment confirm door takes the secret from the
    # client, so garbage there must answer "wrong
...
def verify_used(secret: str, code: str, last_counter: int | None,
    ...
    """Replay-aware verify: returns the matched step-counter, or None if no
    match OR the matched counter has already been used (counter <=
    last_counter). Callers persist the returned
