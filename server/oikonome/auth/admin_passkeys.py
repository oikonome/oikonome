...
def _origins(origin: str) -> list[str]:
    """Accept the configured origin and its other scheme — the same
    proxy-drift allowance the tenant flow makes. Browsers always sign the
    real page origin, so this widens nothing an attacker controls."""
    out = [origin]
    if origin.startswith("http://"):
        out.append("https://" + origin[len("http://"):])
    elif origin.startswith("https://"):
        out.append("http://" + origin[len("https://"):])
    return out
