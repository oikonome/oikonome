...
Email+password stays the floor; passkeys are nicer and phishing-resistant
for browsers that have them.

Caveat the UI surfaces too: WebAuthn only exists in SECURE contexts —
https or localhost. A plain-http LAN install (http://192.168.1.x) can
...
def _h(ticket: str) -> str:
    """sha256 of a bearer ticket for at-rest storage: the step-up ticket is a
    bearer credential like a session or a reset token, so it must be hashed
    at rest (mirrors reset._h). A DB read primitive during the 5-min TTL
    then cannot redeem it against the exact destructive endpoints step-up
    exists to gate."""
