...
def build_zip(conn, out=None) -> bytes | None:
    ...
    # the same RLS-only discovery the GA portable archive uses:
    ...
    # the GA archive's secret-column set, plus tenant_id (ambient here,
    # meaningless to the recipient) — membership is discovered, so the
    # column filter must be shared too, or a future secret column rides
    # out in a table nobody hand-vetted. Blob columns are the one
    # deliberate difference: this ZIP is the user's full copy (on hosted,
    # the only door out), so receipt images DO travel here, while the GA
    # metadata archive omits them by design.
