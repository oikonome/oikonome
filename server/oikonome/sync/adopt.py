...
def adopt_accounts(conn, *, item_id: str, institution_id: str | None,
    ...
    for acct in incoming:
        ...
        # dropped connection) left the account already wearing the incoming
        # Plaid id with no `account_adoptions` row behind it — and that
        # state is unrecoverable, not merely untidy: `_candidates` only
        # looks at TOKENLESS items, so the renamed account now sits on the
        # live item and can never be revisited, while `filter_incoming`
...
def _opaque(tok: str) -> bool:
    ...
    or is a long alphanumeric code ('t8lqcjf1r5ghltr', 'xx0882')."""
...
def dedupe_twins(conn, account_id: str, *, apply: bool = True) -> list[dict]:
    ...
    before the restore was adopted onto it, or an adoption made before the
    guard existed).
...
class _Absorbed:
    ...
    Three conditions, all of which must hold:
      ...
      * the candidate is provably part of what the RESTORE brought back.
        ...
        predates the reconnect. Without it the candidate set was every live
        row in the window, so a charge that arrived days AFTER the
