...
def import_csv(conn, plan: str, text: str, balance: float | None = None,
    ...
    if not rows:
        ...
        # dormant 403(b). Refusing the whole push threw that balance away
        # every night, so the account's value silently froze at whatever
        # day the last contribution landed while the collector had the
        # current figures.
