...
def sync(conn, item_id: str, transport=None) -> dict:
    ...
    try:
        ...
        while True:
            ...
            # backfill in one go, and applying it after the last page made
            # it land as one silent jump — the setup wizard's import bar
            # sat still and then filled, instead of the steady progression
...
def sync_holdings(conn, item_id: str, client: Client | None = None,
    ...
    # one transaction: on an autocommit connection the per-account DELETE
    # used to commit on its own, so a bad holding further down the
    # response left the account with no positions at all until the next
    # successful run, and every net-worth read in between saw $0
    rows = []
    ...
    # types as something else (or omits from `accounts` altogether) was
    # inserted and then never deleted, so a position sold from it stayed
    # in net worth forever. Our own investment accounts join the set too,
