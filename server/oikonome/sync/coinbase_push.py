...
# A script token pushing here may ONLY create / touch rows in Coinbase's
# own namespace. Everything is derived server-side and refused unless it
# carries these namespaces: trusting the request's account/item ids or a
# free-form `prefix` would let a leaked collector token re-home a SimpleFIN
# account onto the coinbase item, rewrite its balance, and mass soft-delete
# another source's rows (`prefix: "sfin:"`).
...
def _money(value, what: str) -> float:
    ...
    uncoerced null would travel all the way to Postgres, whose
    ...
    plan_csv's _num() and amazon_orders' float().
    """
...
def import_payload(conn, payload: dict) -> dict:
    ...
    txns = [base.Transaction(
        ...
        # as_date refuses a year the app cannot read, as a ValueError the
        # route reports as a bad payload
        date=as_date(str(t["date"])[:10]),
    ...
    # so without this each row commits as it is written: anything
    # raising mid-batch (a value the checks above cannot see, a
    # constraint, a lost connection) leaves the account holding half a
    # restatement — some rows updated, the retirement pass never run —
    # while the caller gets an error saying nothing was applied.
    with conn.transaction():
        ...
        #     retire every row between them. A date the payload reports is
        #     restated in full; dates it says nothing about keep their rows.
        #     An account with no pushed transactions — including an empty
        #     transactions[] — loses nothing.
    ...
    # push too. The stamp gives the Doctor a heartbeat to judge, so a
    # collector whose keys have expired does not look fine forever.
