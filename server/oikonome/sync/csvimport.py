...
def date_order(values) -> str:
    ...
    cannot). A column that never disambiguates reads month-first — the US
    export shape this importer always assumed. Decided ONCE per file: a
    day-first bank export used to be parsed row by row, so 03/04 landed
    in March while 25/04 landed in April, and nothing said so."""
    for v in values:
