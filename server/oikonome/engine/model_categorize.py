...
primaries, with a real probability. A small trained classifier gives
calibrated probabilities at a fraction of an LLM's cost, and serves
deployments that run no LLM.
...
# below this max-probability the model abstains. Env-tunable so a
# deployment can trade coverage for precision without a release.
DEFAULT_MIN_CONF = 0.55
# the learned overlay writes durable rules from a much smaller training
# set than the shipped artifact, so it holds a stricter bar, trading some
# coverage for precision. What it abstains on still reaches the shipped model and
...
# unbounded number of tenants (multi-tenant deployments), so without a cap every
