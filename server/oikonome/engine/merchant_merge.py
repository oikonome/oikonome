...
# with and without it is one merchant (Cvs / Cvs Pharmacy, Woodlake /
# Woodlake Market) — never a fuel word, which marks an outlet
...
def norm(name: str | None) -> str:
    ...
    gone, "the " dropped, spaces collapsed. "Juniper's Nat" -> "junipers nat"."""
    ...
    # a terminal code or reference is not part of the name ("558801 quickfuel
    # 00007z14"), nor is how the bill was paid ("northfield power echeck")
...
def _one_typo(a: str, b: str) -> bool:
    """Long names one letter apart past the first word ("millbrook
    confectionery" / "millbrook confectionary")."""
...
def _code_bearing(name: str) -> bool:
    """A one-word name that carried a code the normaliser dropped: "Vine:30"
    compares as "vine", "7-Eleven" as "eleven". Such a name must not be
    read as the bare word plus a kind of business — "Vine Kitchen" is not
    Vine:30 — though it still matches a spelling of ITSELF."""
...
def _minus_place(s: str, cities: list[str]) -> str:
    ...
    by the terminal ("petrox svc station glenharb")."""
    words = s.split()
    for k in (2, 1):
        if len(words) <= k:
            continue
...
def chain_brand(members: list[dict], cities: list[str]) -> str | None:
    ...
    # a family that differs only by kind words ("Ferdy's Bar & Grill",
    # "Ferdy Bar Restaurant") is one place, and its fullest spelling is the
...
def _rank(m: dict, top: int = 0) -> tuple:
    ...
    "Mcdonald Qrt Millbroo" is a junk tail, not the fuller name), else the
...
def propose(cands: list[dict], cities: list[str],
    ...
    # two offers ("join Juniper's Natural Foods"), never three that also
...
def run(conn, *, limit: int = 25, only_ids: list[str] | None = None) -> dict:
    ...
    rows), so an hourly sync or a CSV upload no longer pays a whole-ledger
    dedup pass. The offers it writes are the ones the nightly full run
