...
# leading payment-processor / gateway wrappers (so 'GglPay TARGET' -> 'Target')
_PROC = re.compile(
...
# Street address / locality tail. Aggregators sometimes send NO
# merchant name at all, and the raw
# descriptor then carries the store's full address:
#   'ABSOLUTE SHINE DETAILING 1200 N. HAWTHORN AVE. MARAIS D OLIVE, NV, US'
# Kept, those tokens split one payee into 'Absolute Shine Detailing' and
# 'Absolute Shine Detailing Hawthorn Marais Olive'.
...
#           ONLY when something survives in front of it, so '7 Eleven 34567'
...
# ("RVCU MARAIS D'OLIVE") — removing trailing words on suspicion of being a
# place name would eat real merchants (Springfield Co-op, Nashville Tea Co).
...
def cities_for(conn) -> tuple[str, ...]:
    ...
    string at different lengths — 'Burgo Walnutda' and 'Burgo Walnutdal' are
    one restaurant — but nothing in the ledger reliably identifies which
    trailing word is a place. Deriving it measurably fails: ranking trailing
    words by how many spending categories they span marks 'chicago' a
    place and 'walnutda' a category word, because in a college town nearly
    all spending is food. So the list is supplied by whoever knows —
    default empty, which is a no-op."""
    try:
        from . import budget
    ...
    # rewrites 'Firestone Auto Center'. merchant_cities_seen is kept in
...
def _city_variants(city: str) -> set[str]:
    ...
    punctuation and short words by the time we strip — "Marais d'Olive"
    reaches us as "Marais Olive"), and the truncations banks produce by
    cutting the descriptor short ("Walnutdale" -> "Walnutdal", "Walnutda").
    Prefixes stop at 70% of the city (min 6 chars): the truncations banks
    actually produce are near-full ('Walnutdal', 'Walnutda'), while a short
    fragment would start eating real words — 'Walnut' would turn 'Black
    Walnut' into 'Black'."""
    city = (city or "").strip()
    if len(city) < 4:
        return set()
...
def harvest_city_map(conn) -> dict[str, list[str]]:
    ...
    it trails a name turns 'Firestone Auto Center' into 'Firestone Auto'
...
# such merchant in town into a single row ('Bank Of Walnutdale' -> 'Bank').
# A distinctive single word ('Burgo') is fine and is the common case.
_GENERIC_ALONE = {
...
def _strip_cities(s: str, cities) -> str:
    """Drop a trailing configured city, repeatedly ('Olive Grove Detailing Marais
    Olivenv' -> 'Olive Grove Detailing'), while what survives is still a real
    name: 'Burgo Walnutda' -> 'Burgo', but 'Bank Of Walnutdale' is left
...
# a run of hyphen-joined alphanumeric pieces ("1-800-Flowers", "WITHDRAWAL-CASH")
_MAX_LABEL = 256
...
# rather than a name: what a bank appends ("Sandwich Hut Millbrook NV",
# "Frozy D-Lite 14th St", "Walnutdale Brewing Co"). Consulted only for a TRAILING
# token, because leading, the same two letters are the business itself
# ("US Bank", "NV Dmv Renewal") — position decides, not the word.
...
# bank shorthand ('SQ Coffee House', 'CK Student Housing', 'CC Conc New',
...
# so it keeps its capitals ('US Bank', 'BP Gas', 'JC Penney') — except
# for the words that really are words at the front of a name ('At Home',
# 'La Quinta', 'In N Out'). 'us' is deliberately NOT one of them: no
...
def _short_tokens_kept(toks: list[tuple[str, bool]]) -> list[str]:
    ...
    * LEADING, two letters are an initialism and usually the whole point of
      the name — 'US Bank', 'JC Penney', 'GE Appliances', 'La Quinta',
      'St Paul Fish', 'NV Dmv Renewal'. A blanket three-letter floor
      deleted the brand and kept the generic half ('Bank', 'Penney').
    * MID-DESCRIPTOR, they are bank shorthand — 'Deposit Atm ZQ Millbrook',
      'Payment Dir DB Ref', 'Printer Refill EZ Pri'.
    * TRAILING, they are usually a word the bank truncated ('Country
      Natural Fo', 'Family Dental Ce', 'Venture Wellness Ll') — except on a
      two-word name, where the second word IS the product: 'Google Fi'.
...
def canonical_merchant(name: str, cities=()) -> str:
    ...
    # SHORT piece — "U-Haul", "T-Mobile", "H-E-B", "7-Eleven", "1-800-Flowers",
    # "A-1 Auto", "Chick-Fil-A" are one token each. Cutting on it and then
    # dropping digits and sub-3-letter pieces left "Haul", "Mobile", "Gas"
    # (H-E-B Gas), "Eleven", "Flowers": the brand's own name was the noise
    # that got stripped. A hyphen between two long words is a bank's
    # punctuation ("WITHDRAWAL-CASH", "Giant-Eagle", "CREDIT-INTEREST") and
    ...
    for t in s.split():
        if "\x00" in t and re.search(r"[A-Za-z]", t):
            ...
            if len(t) >= 3 and t.lower() not in _STOP:
                toks.append((t, False))
            continue
        # A whole two-letter WORD can be the name ("US Bank", "JC Penney",
        # "Google Fi"); a two-letter FRAGMENT never is. They
        ...
        # (U-Haul) — banks prefix bare letters to descriptors.
...
# (engine/reporting.py currently inlines byte-identical copies — the drift
# test in tests/test_merchant_dedup.py keeps them locked together until
# reporting imports from here.)
...
def detail(conn, display: str) -> dict | None:
    ...
    # display name missed a rule stored under a spelling a rename has since
    # moved on from — the card said "no rule" for a merchant the rule was
    ...
    # its key. The earlier form tested every rule with a correlated
    # name lookup over the merchants table — thousands of rules times
    # thousands of merchants, five seconds per opened merchant.
...
def listing(conn, q: str = "", limit: int = 200,
    ...
    # length as a fact ("N source names") and at the cap that fact was a lie.
