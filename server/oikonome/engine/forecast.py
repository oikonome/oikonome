...
def build(conn, today: dt.date | None = None, days: int = HORIZON_DAYS,
    ...
    # lower configured paycheck applies immediately (scenarios
    # are a named list now; the legacy pair read-migrates)
    ...
    # it in again on payday counted the same money twice for the days
    ...
    plan_daily = ((b["food"]["month_budget"] + b["other"]["month_budget"])
                  / st["days_in_month"])
    pace_daily = (st["variable_actual"] / today.day) if today.day >= 5 else plan_daily
