...
def llm_failing(conn) -> dict | None:
    ...
    with nothing produced. Store-order summaries and merchant categories stop
...
def log(conn, alerts: list[dict], today: dt.date, *, partial: bool = False):
    ...
    full-snapshot write: a caller passing only its own rows there would
    deactivate every other active alert, and the next Today load would
    re-activate them with the user's dismissals reset.
            ...
            conn.execute(
                """INSERT INTO alerts_log (kind, severity, message, first_seen,
                   ...
                   ON CONFLICT (tenant_id, kind, message) DO UPDATE SET
                       ...
                       -- makes the reset atomic with the reactivation, so
                       -- two concurrent full-snapshot writers cannot
                       -- resurrect a dismissed alert between two steps.
