...
# reassigned to Beta matched Acme through the ACCOUNT clause and Beta
...
# entity_transactions() now carries.
# SHADOW EXCLUSION, like every other money aggregate in this codebase.
# `reporting._live_accounts`, `retirement.current_buckets` and `forecast`
# all drop non-primary linked accounts before summing, each citing the same
# bug: a dual-source account counted twice. This predicate did not, and a
# business account re-linked through a second aggregator (Plaid, then
# SimpleFIN when Plaid access lapsed — the flow links.py documents as
# ordinary) leaves two rows sharing one group. The Business page's account
# picker assigns each row on its own, so both can carry entity_id, and the
# overlapping history was then counted into the P&L twice.
#
...
# Money that MOVED but was never spent. `pnl()` has always skipped these
# per-row (same double-count rule as the personal side), but the Schedule C
# review queue did not — so `unclassified_count` lit the "Needs attention"
# tile for card payments and inter-account transfers that classifying could
# not resolve: pick any line and the P&L is unchanged, leave it and the tile
# stays lit. An unclearable to-do.
#
...
def unclassified_count(conn, entity_id: str) -> int:
    ...
    most `limit` rows to keep the review queue workable. The Overview tile
    counted the list it happened to receive, so a business with 300
    unclassified rows reported "200 need attention" and stopped moving as the
    user worked through them. A count is not a page of results.
    """
    return conn.execute(
        f"""SELECT COUNT(*) AS n
              FROM transactions t
...
def classify(conn, txn_id: str, bucket: str, *, sched_c_line=None,
    ...
    # a caller that only changes the bucket passes None for them — and with
    # EXCLUDED that None overwrote a Schedule C line the user had already
    # confirmed, silently demoting the row back to Unclassified on the P&L and
    # the year-end package. The Books tab's bucket dropdown does exactly
    # that. Clearing a line is not something any caller asks for today;
    # if one ever needs to, it should say so explicitly rather than getting it
    # as a side effect of omitting an argument.
    conn.execute(
...
def _pnl_core(conn, entity_id: str, year: int | None = None,
    ...
    full pnl() from there recursed forever (pnl → capital_summary → pnl).
    ...
    for t in txns:
        ...
        if amt < 0:                                   # money in = revenue
            ...
            # (the owner's Venmo rule reaching the one inflow under it) —
            # it is revenue only when the user made it INCOME outright.
    ...
    return {"year": year, "revenue": revenue,
            ...
            # one-time allowance over the life of the business, but these
            # were computed from the transactions of the single `year`
            # passed in — so $28k of organizational cost in 2023 and another
            # $28k in 2024 reported $5,000 immediate in EACH year's P&L and
            # CSV export, $10,000 across the two, when the true lifetime
            # total of $56k is over the $50k phase-out and the correct
            # immediate deduction is $0.
