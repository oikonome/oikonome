"""Match a store's own purchase records — Amazon orders, Costco receipts —
to the card transactions that paid for them, and carry the store's
...
class Store:
    ...
    # (Amazon); wrong for one whose unmatched charges — fuel, membership —
    # already carry a useful aggregator category (Costco).
    mark_unmatched: bool
...
# trails it; costco.com orders charge at shipment, up to a few days after
# the receipt's order date
...
def run_match(conn, store: Store) -> dict:
    ...
    records = [dict(r) for r in conn.execute(
        f"SELECT dedup_key, date, amount, category, payment_method "
        f"FROM {store.records}")]

    # Budget purchase rows only: a bare name regex would stamp
    # '<Store> - Unmatched' onto brokerage *stock trades* (AMZN, COST in
    # investment accounts) and onto 'Transfer to Amazon' card-payment
    # TRANSFER rows. Skip linked shadow accounts — a shadow's duplicate
    ...
    with conn.transaction():
        ...
        # to those braces, and it names only the PERSON's rows now — a
