...
# small local models lose the number→merchant mapping in long batches, so
# batches stay short (the cache is permanent, so quality beats throughput
# here)
BATCH_SIZE = 8
...
# Category rules key on the CANONICAL merchant, not the raw descriptor.
# One payee that arrives under several descriptors ("NORTHWIND CAFE" and
# "SQ *NRTHWND CA…") is one merchant on every report but N unrelated keys
# to a rule engine keyed on the raw string, so a correction on one variant
# would never reach the others. Both sides resolve through
# merchant_canonical (raw_merchant -> canonical); an unmapped string is its
# OWN canonical (COALESCE fallback), so a merchant with no mapping keys on
# its own string. Because several raw variants collapse to one canonical, a
# lookup can match MORE THAN ONE rule row — so every match aggregates and
# applies a category only when the rules are UNANIMOUS; a canonical whose
# rules disagree is left split (never an arbitrary winner).
#
...
# by join in apply()/pending_merchants. A transaction-side fragment resolving
# on merchant_name alone would let the two sides disagree about which rows a
# bulk change covers.
_RULE_CANON = (
...
def resolve_role(cfg: dict, role: str = "categorize"):
    ...
    what" display (api._settings_view), so the two cannot drift apart."""
...
def _backend(conn=None, role: str = "categorize") -> dict:
    ...
    # http://ollama:11434, a private compose hostname a multi-tenant
    # netguard policy would (correctly, for tenant data) refuse.
    ...
    if layer == "backend":
        ...
        return {"url": b["url"], "model": b.get("model") or "",
                ...
                # encrypted at rest like the key (plaintext values
    ...
    if layer == "legacy":
        ...
        return _with_cfg_vision({
            ...
            "api_key": crypto.decrypt(conn, key) if key else "",
            # encrypted at rest like the key beside it (plaintext values
            # pass through decrypt unchanged)
            "extra_body": crypto.decrypt(conn, eb) if eb else "",
...
def _prepare(be: dict, transport):
    ...
    re-checked at USE time (SimpleFIN's every-fetch guard) and the connect is
    DNS-pinned."""
...
def _merge_extra_body(body: dict, extra_body: str) -> None:
    ...
    saves refuse anything but a JSON object, but an env value or a restored
    row can still hold one — a non-object here must degrade to a warning,
    not crash every call routed through the backend."""
    if not extra_body:
        return
    try:
...
def pending_merchants(conn, limit: int | None = None) -> list[dict]:
    ...
    q = f"""
        ...
        FROM (
            ...
            -- a merchant whose row has been renamed or retargeted (the
            -- alias string lags: the resolver writes only merchant_id)
            -- would queue under a name apply() no longer resolves to it.
            ...
            -- outlet, a fuel-arm row would never be categorized.
            {_MC_JOIN}
...
def apply(conn, canon: str | None = None,
    ...
    # a large ledger, at the end of every sync, and no index rescues it.
    #
    ...
    # the same rows a per-row unanimity check would select.
    ...
    # merchant_name instead, the write and the snapshot would resolve
    # different canonicals for the same row: a fuel-arm row (outlet "Acme
    # Gas", merchant_name "Acme") would take the brand's rule while the
    ...
    with conn.transaction():
        ...
        # label too: a payment to a person through a payment app is
        # TRANSFER_OUT to the aggregator and "childcare" to the household,
        # and a rule that could not reach those rows would leave every new
        # one landing as a transfer no matter how many times the user
...
def mcc_classify(conn, stats: dict | None = None) -> int:
    ...
    n = 0
    # an outlet (the ingest's own fuel stamp) carrying an MCC category is a
    # row the MCC should never have touched, and the statement line still
    # says pump — put the stamp back
    n += conn.execute(
        """UPDATE transactions
...
def record_run(conn, stats: dict, backend: dict) -> None:
    ...
    rec = {"error": stats.get("backend_error") if failed else None,
           ...
           # the error of a record that lacks this flag
...
def last_run(conn) -> dict | None:
    ...
    if prog.get("error") and not prog.get("error_classified"):
        # An unclassified record may quote the backend's raw reply, which
        # can be the prompt echoed back — household purchases, on the Doctor
        # page and inside any feedback bundle (see _unusable_reply). A later
        # run would overwrite it, but an instance whose backend is switched
        # off never has one, so the text is dropped on the way out rather
        # than shown.
...
# ---- Amazon item summaries ("Amazon — desk lamp" in the email) -------------
...
def _items_key(row: dict) -> str:
    ...
    item can yield 'lamp' one day and 'reading lamp' the next, and a
...
def _summary_prompt(rows) -> list[dict]:
    ...
    system = (
        ...
        "'etc'; name each item. "
        "Examples: 'phone case, usb cable', 'desk lamp', "
        "'garden hose'. "
...
def categorize_new(conn, *, limit: int | None = SYNC_MERCHANT_LIMIT,
    ...
    # the FULL sweep, deliberately. Scoping this to the merchants the run
    # touched would break this function's contract (a cached rule must
    # reach a newly pulled row even with no LLM), and the candidate set that
    # would preserve it is most of the ledger's merchants, so it buys
    # nothing.
