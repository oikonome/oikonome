...
# down with them — and on hosted, that container is serving every tenant.
...
def _year_of(out: dict, complaint: str) -> int:
    ...
    a clean 400. int() alone raised TypeError on a null year, which the
...
def _commit_rows(conn, kind, rows, years) -> dict:
    ...
    for r, year in zip(rows, years):
        if kind == "w2":
            ...
            # string, a number — reached .items() and 500'd instead of
            # being refused as the bad input it is.
