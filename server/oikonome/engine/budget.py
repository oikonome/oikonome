...
The month's spending is split three ways:
  ...
  food  — FOOD_AND_DRINK / "Amazon - Food & Drink" / "Costco - Food & Drink".
...
# understated, a false UNDER BUDGET. link_reimbursement caps new links at the
# charge, but older rows can predate that guard, and a floor is the cheaper
...
def _envelope_bills(conn, disabled: list | None = None, *,
    ...
    drips — and at 1 month it IS the pool, so nothing about the old shape
    moves. `memo`: request-scoped cache, as on _recurring_bills. `rows`: a
...
def _envelope_bills_load(conn, disabled, rows=None) -> list[dict]:
    ...
    for r in rows:
        ...
        out.append({
            ...
            # envelope even when their merchants share nothing (babysitters,
            # ATM cash, Venmo counterparties). Merchant matches keep priority.
...
def match_envelopes(rows, envelopes, matched_idx: set[int],
    ...
    for i in order:
        ...
        # its own day; a full-amount fixed row plus an overflow stamped
        # today counted it twice, and charged today for a swipe from the
        # 5th.
        if take > 0.005:
...
# match. A 16-day cap leaves a long-cycle (quarterly/annual) bill prepaid
# more than 16 days early reserved anyway — double-counting money already
...
def bill_tolerance(amount) -> float:
    ...
    the way a flat $30 floor did. $15 → $7.50, $60 → $30, $200 → $50."""
...
def month_status(conn, today: dt.date, *, historical: bool = False,
    ...
    # in this month's actuals (which is the trap that makes the naive fix
    # worse than the bug).
...
# nobody else can reproduce: bill-shaped rows are already out, Costco and
# Amazon food overrides are already in, envelope overflow has already been
