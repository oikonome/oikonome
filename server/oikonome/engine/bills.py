...
def _derive_merchant(conn, amount: float, *, match_tokens: set | None = None,
    ...
    ('Pawtown' lacks 'pack') is rejected, loose enough to absorb
    variants ('Alderway Fiber' vs 'Alderway Fiber Telecom')."""
...
def save_bill(conn, *, payee: str, amount: float, bill_type: str = "occurrence",
    ...
    babysitting budget whose spend arrives from many merchants. Both are
    ...
    thing: the monthly storage charge at a truck-rental company is rent, the
    truck rentals are transportation, and a merchant rule cannot say both.
    ...
    with conn.transaction():
        ...
        # edit (the attention queue's Accept) used to null due_on, which drops
        ...
        if bill_type == "envelope":
            ...
            # spread over its months; at interval=1 that's the old amount, exactly.
...
def attachable(conn, payee: str, merchant: str | None, names,
    ...
    → Google Store, "Riverton" → Riverton Health). Once attached, identity
    ...
    * when the ledger offers exactly ONE name the bill's words are not
      ...
      "Progressive Insurance" for a "Progressive" bill) — UNLESS a merchant
...
def recent_auto_changes(conn, days: int = 1) -> list[dict]:
    ...
    CONDITION-AWARE. Amount drift is applied without
    asking, so this notice is the only word the user gets that their plan
    moved — but it used to be a pure 24-hour digest, with no idea whether
    they had done anything about it. Edit the bill from the Bills page and
    the alert sat there anyway until the window aged out.
...
def propose_from_plaid_streams(conn, streams: list[dict],
    ...
    # bill and its refund stream) got one proposal for whichever direction
    # was seen first and silently lost the other. `income` is absent on
...
def apply_proposal(conn, pid: str, action: str) -> dict:
    ...
    row. They were a plain read followed by a write: two clicks on Approve
    (a double-tap, or the same proposal open in two tabs) both saw
    'pending' and both ran `_apply_approved`, so an add-proposal created
    the bill twice and an amount-change applied twice."""
    with conn.transaction():
        p = conn.execute(
...
def _apply_approved(conn, p, ev) -> dict:
    if p["kind"] == "add":
        ...
        # proposal lock above never contends, and the later write replaced
        # the whole companions key with a list computed before the first
        # landed — one fee silently lost.
        ...
        if not any(c["tokens"] == budget._tokens(new["tokens"]) for c in have):
            companions = ([{"tokens": c["label"], "amount": c["amount"],
                            "window_days": c["window_days"]}
                           for c in have] + [new])
            # merge just the companions key — the whole-raw overwrite from
            # get_bill's unlocked read raced a concurrent save of the host
    ...
    for r in conn.execute(
            ...
            f"""SELECT t.date, t.amount, COALESCE(t.merchant_name, t.name) AS payee,
                ...
                WHERE t.removed = 0 AND (t.pending = 0 OR %s) AND t.amount > 0
                  ...
                  -- Entity money is its own section; without this an LLC's
                  -- recurring charges were detected as the owner's bills.
...
def _scan_months(rec: dict) -> int:
    ...
    occurrence. 14 covered a yearly bill and was the constant here; a bill
    on a longer cycle (a YEARLY interval of 2, or a MONTHLY interval of
    18) has its next occurrence 12·N months out, so the
    14-month scan found nothing, `due_on` stayed in the past forever and
    the Bills page called a bill paid on its day "overdue" every day after."""
...
def _advance_due_dates(conn, today, stats):
    ...
    for b in conn.execute(
        ...
        # the loop's snapshot read raced a user's concurrent save and lost
        # whatever they had just written (lock_amount, companions, …)
...
def advance_received_income(conn, today: dt.date | None = None) -> int:
    ...
    for b in due_rows:
        ...
        # overwrite from the loop's snapshot raced a concurrent save
        conn.execute("UPDATE bills SET due_on=%s, raw = COALESCE(raw, '{}'::jsonb) || %s::jsonb, "
                     "synced_at=now() WHERE id=%s",
...
def _refill_like(events: list[dict]) -> bool:
    ...
    (coffee-card $30/$50 top-ups). Restaurant/grocery totals almost never
    repeat exactly."""
...
def _propose_new(conn, groups, active_keys, today, stats):
    ...
    for key, rws in groups.items():
        ...
        # "food" over the whole group let one lunch veto a 12-hit,
        ...
        # of the old cluster — "CEDAR VALLEY CLINIC" → "… HEALTH CARE" at a
        # new amount), the current series lives in the recent events.
        ...
        if fit and not food_core and med >= 5:
            ...
            if cyc[0] >= LONG_CYCLE:
                ...
                # parking lots six months apart do not (a truncated
                # municipal descriptor like "Co Springfield Riverfront Par"
                # lands twice a year at two unrelated amounts)
            ...
            if len(run_dates) >= need and fresh and exact:
                ...
                # drift check will compute once the bill is tracked. It
                # used to carry the whole-history median, so a bill whose
                # price rose over two years was approved at the old figure
                # and immediately "drifted" to the new one on the next
                # pass — an amount change on a ledger that had not changed.
...
def _propose_income(conn, today, stats):
            ...
            for r in conn.execute(
                ...
                f"""SELECT t.date, t.amount, COALESCE(t.merchant_name, t.name) AS payee
                   ...
                   WHERE t.removed = 0 AND t.pending = 0 AND t.amount <= -200
                     ...
                     -- INCOME-only, their paychecks were undetectable. The
    ...
    for key, rws in groups.items():
        ...
        # density kills the biweekly fit: a paycheck running ~20% above its
        # own two-year median otherwise produces no proposal at all.
        ...
        # and mid-cycle wizard runs still propose (1.5×14d = 21d drops a
        # series whose last paycheck was 23d ago). Floor 35d keeps short cycles from
        # dying a few days past one missed payday. ledger_fit uses ×2 for
...
def _maintain_bills(conn, bills, rows, today, stats):
    ...
    for b in bills:
        ...
        if bill_type == "envelope":
            ...
            # (the calendar year). Both rules below are PERIOD-relative; the
            # original pair were written for monthly pools only and were
            # wrong in both directions on an annual one:
            # a lumpy yearly cost landing twice in a 3-month window drifted
            # the whole year pool down to one charge, and two quiet months —
            # which is 10 months out of 12 for an annual cost, by definition
            # — proposed deleting the bill.
        ...
        # `amount_set_on` is stamped by save_bill; a legacy row without it
        # keeps the old behaviour.
        # raw is JSON that a restore ZIP can hand us verbatim: a malformed
        # stamp is treated as a legacy row (no gate), never a crash of the
        # nightly pass
        try:
...
def merchant_history(conn, payee: str, today: dt.date | None = None,
    ...
    # that is the whole point of the pool (babysitters, ATM cash, Venmo
    ...
    # it ('Northside Auto Detailin'), and a clean one — and the dedup map
    # already knows they are the same business. This page matched raw text
    # only, so those merges had no effect here and the same merchant listed
    # three different histories. Token matching stays as the fallback for
    ...
    # exactly the string the ledger printed, and two kinds of merchant could
    # not be looked up by that string: an OUTLET row (a chain's fuel arm,
    ...
    # returned an empty page for a name the ledger had just displayed.
    #
    ...
    # and the catalog disagreed about the same merchant in the other
    ...
    since = today - dt.timedelta(days=months * 31)
    txns = []
    for r in conn.execute(
            f"""SELECT t.id AS txn_id, t.date, t.amount, t.pending,
               ...
               FROM transactions t JOIN accounts a ON a.id = t.account_id
                    ...
                    -- TRANSACTION ROW for that shape. It measures ~0ms
                    -- while the table holds a single row; it is
                    -- O(rows x notes) and degrades the day a user actually
                    -- annotates their ledger. A LEFT JOIN scans it once.
    ...
    # The CURRENT month belongs in the table too — a bill
    # already paid this month was invisible ("monthly totals" ended at last
    # month while the txn list and lifetime total both showed the charge)
...
def _apply_drift(conn, bill_row, new_amt: float, evidence: dict, stats,
    ...
    # `-1 if amount < 0` once flipped a $0 bill to +amount (income
    # convention), making it invisible everywhere; bills are negative unless
    # the row is genuinely income (strictly positive)
