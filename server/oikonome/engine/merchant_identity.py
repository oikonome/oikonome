...
Precedence, highest first:
  ...
  4. the layer-1 string clean (merchant_dedup.canonical_merchant) — the
     fallback for the ~two-thirds of history that no aggregator resolved.
...
still read the string keep working during the cut-over. The raw key is the
...
def _upsert_plaid_merchant(conn, ident: dict, cache: dict):
    ...
    if row:
        ...
        # same name, and every surface that groups by name hid the split
        # while everything keyed on the row saw two merchants.
...
def _abbreviates_plaid_merchant(conn, name: str):
    ...
    "PROGRESSIVE *INSURANCE" to its entity "Progressive Insurance"; a
    statement import carries "PROGRESSIVE INS MAYFIELD VLG USA", which the
    layer-1 clean leaves as "Progressive Ins Mayfield Vlg Usa" (the city
    is not in the harvested list). Exact-name matching alone would mint a
    second merchant, and the household would see two names, one with a
    logo, for one insurer. The rule: the descriptor's first token equals the Plaid name's
    first token, and every further Plaid-name token is begun by the
    ...
    initialism ('US Bank Mayfield' beside Plaid's 'US Bank'), and the string
    clean keeps those now, so a three-letter floor here would leave exactly
    ...
    with conn.transaction():
        ...
        def _settle(g, adopted):
            ...
            if alias and alias[1] == "manual":
                ...
                # predates the outlet split and would undo it
...
def prune_empty(conn) -> int:
    ...
    the merchant out from under them left the only pointers in the schema
