...
def detect(conn, today: dt.date) -> list[str]:
    ...
    # each charge ("AMAZON MKTPL*5L03K5C61") gives separate orders at a
    # common price point ($19.99 + tax) distinct descriptors, while a
    ...
    for (_, amt, _), grp in sorted(groups.items(), key=lambda kv: -kv[0][1]):
        ...
        # fresh pair: three $47.39 rows on the 8th, 10th and 11th span three
        # days, so the 10th + 11th double charge went unreported.
    ...
    for r in rows:
        ...
        # inside the scan window) — `date < since` left a blind window where
        # a merchant's second charge re-fired a false "first-ever" alert
        prior = conn.execute(
