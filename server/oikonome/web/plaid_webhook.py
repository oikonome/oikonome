...
def _tenant_for_item(item_id: str) -> str | None:
    ...
    try:
        ...
        return _live_tenant(admin, item_id, active_only=True)
    finally:
        admin.close()


def _live_tenant(admin, item_id: str, *, active_only: bool = False) -> str | None:
    """The ONE tenant whose row for this Plaid item is the live connection.

    items is keyed (tenant_id, id), and a restore copies the source's Plaid
    item id into the destination household as a 'restored' shell with no
    token of its own — so an item id can name rows in several tenants.
    The webhook is tenantless and runs without RLS, so every read and write
    it makes must pin the one tenant the connection belongs to: the row
    holding an access token, never a restored shell or an archived row."""
    tenant_join = ("JOIN tenants t ON t.id = i.tenant_id AND t.status = 'active'"
                   if active_only else "")
    row = admin.execute(
        f"""SELECT i.tenant_id FROM items i {tenant_join}
            WHERE i.id=%s AND i.aggregator='plaid'
              AND COALESCE(i.status,'') NOT IN ('archived', 'restored')
            ORDER BY (i.access_token IS NULL), i.tenant_id
            LIMIT 1""",
        (item_id,)).fetchone()
...
_ITEM_STATUS = {
    ...
    # update mode is what prevents the disconnect, so it is surfaced
...
def _clear_item_status(item_id: str) -> bool:
    ...
    admin = tenancy.admin_connect()
    try:
        tenant_id = _live_tenant(admin, item_id)
        if tenant_id is None:
            return False
        cur = admin.execute(
            "UPDATE items SET webhook_status=NULL, webhook_status_at=NULL, "
            "status='ok' WHERE id=%s AND tenant_id=%s AND aggregator='plaid' "
...
def _record_item_status(item_id: str, status: str) -> bool:
    ...
    try:
        ...
        cur = admin.execute(
            ...
            "WHERE id=%s AND tenant_id=%s AND aggregator='plaid' "
            "AND COALESCE(status,'') NOT IN ('archived', 'restored')",
