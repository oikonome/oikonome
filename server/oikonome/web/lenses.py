...
def _projection(st: dict, as_of: dt.date) -> dict:
    ...
    pace_total = st["variable_actual"] / max(as_of.day, 1) * st["days_in_month"]
...
def year_summary(conn, year: int, today: dt.date | None = None) -> dict:
    ...
    # totals the lens has always shown would change shape to carry a link.
    ...
    if goal is not None and not savings.is_plan_only(goal):
        ...
        if span_end >= span_start:
            ...
            # those same monthly windows or the map paints $0 beside
            # seven $500 months.
