...
household's control:
  ...
  * **the bank connections** — linking, re-keying, disconnecting, removing
    an account. These hold credentials, they cost money per item on
    hosted, and disconnecting is felt by everyone in the house.
...
SELF_WRITE_OK = ("/api/logout", "/api/password/change", "/api/totp",
                 ...
                 # 7-day verify link lapsed had NO route to a new one —
                 # Resend and the email-change form both 403'd, a permanent
                 # self-inflicted dead end the owner could not fix either.
...
def path_within(path: str, prefixes) -> bool:
    ...
    "/api/totp/enroll"). A bare str.startswith also admitted any future
...
def read_only_route(path: str) -> bool:
    ...
    them would quietly exempt a route listed here from both — including an
    endpoint that costs the operator money per call. Adding a route here must never buy anything but the
    role gate.
    """
