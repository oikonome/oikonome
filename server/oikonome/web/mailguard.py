...
def members_only() -> bool:
    ...
    it does not belong on an instance serving ordinary households. Self-host was
    always free-form: the operator owns the SMTP.
    """
...
def filter_recipients(conn, recipients: list[str]) -> list[str]:
    ...
    email_recipients — scrub_config only strips
    secret/demo keys, not the mail boundary. Self-host is unchanged — except
...
def invite_added(user: dict, emails: list[str]) -> None:
    ...
    ONE thread for the whole batch, not one per address. The per-address
    version spawned up to MAX_RECIPIENTS threads that each checked out a
    POOLED tenant connection and then sat in an SMTP conversation — a single
    settings save could take twenty of them at once and starve every other
    request of the pool. They all belong to one tenant, so the SMTP settings
