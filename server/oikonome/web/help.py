"""In-app help — the docs/*.md the repo ships, served as JSON topics for
the SPA's Help page (single source: the repository copies stay the
canonical files; the container carries them at <root>/docs). The FAQ is
docs/faq.md like everything else — deliberately brief answers to common
stumbles, never a second docs tree."""
...
def _docs_dir() -> Path:
    ...
    (…/server/oikonome/web → …/docs). The CONTAINER is different —
    `pip install ./server` puts this file in site-packages, so the relative
    walk lands in /usr/local/lib/… and finds nothing, and a test suite run
    from the repo layout cannot notice. The image COPYs docs to /app/docs — probe both,
