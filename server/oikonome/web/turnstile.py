...
def verify_login(token: str, ip: str = "", email: str = "") -> bool:
    ...
    would lock it out. Once failures accumulate on
    either the IP or the account, the token is required.

    A Turnstile token is not the only way to pass. An account that has
    proved control of its mailbox holds a one-shot exemption, which is what
    lets a user the Cloudflare script cannot reach (JS off, blocked,
...
def refund_unlock(email: str) -> None:
    """The sign-in doors call this when the password step succeeds but the
    request cannot finish (totp_required / passkey_required with no code
    supplied) — a just-spent emailed exemption is re-armed so the SECOND
    post of the two-step flow can pass the same check. No-op when the pass
    came from a token or from no-challenge (there is no recently-consumed
    row to refund)."""
    if not email:
        return
    try:
...
def spend_unlock(email: str) -> bool:
    """Public spend-only door for /forgot — a held emailed exemption answers
    the human check there too, WITHOUT the per-account risk bucket entering
    the gate (that door must stay un-armable against a stranger's address)."""
    return _spend_unlock(email)
