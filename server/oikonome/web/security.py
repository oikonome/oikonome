...
# threading one through every call site invites a miss.
...
def _body_limit_for(content_type: str, *, upload_route: bool = True) -> int:
    ...
    read the router: an unreadable route table must widen the ceiling to
    the upload budget, never narrow it, or one bad derivation silently
...
# route too, /login included. Charging those to the upload budget below would
# let an anonymous client spend the whole instance's ingest capacity on a door
...
def _accepts_upload(scope) -> bool:
    ...
    back to True: the gate is then as wide as the content-type check alone
    ...
    if not matchers:
        ...
        # fallback widens it to the full upload ceiling rather than
...
#    itself.
#
# Both answer 408 and both give the slot back; without them, a handful of
# idle sockets could hold every slot and refuse every upload on the instance.
    ...
    async def dispatch(self, request, call_next):
        if request.method in _WRITE:
            ...
            if claim:
                ...
                # some cross-origin redirects) has no netloc, and an
                # attacker-controlled opaque context must not be able to
                # POST. A claim that IS present but doesn't parse to THIS
                # host is cross-origin, full stop; only a genuinely absent
                # claim (non-browser client) stays allowed.
...
# untrusted client picks — so without eviction one entry per scanner IP
# would accumulate for the life of the process, in a process that also holds
# the SPA and the feedback ring buffer. A key costs roughly a kilobyte (a
# deque preallocates its first block), so an open /login taking internet
# background noise would cost tens of megabytes that never come back. Same
# class as an unbounded body: a shared resource whose size an anonymous
# client decides.
#
...
class RateLimiter:
    ...
    def _sweep(self, now: float) -> None:
        ...
        # expiry, the lockout would go FIRST and /forgot's hourly counter
        # would outlive it: an attacker spraying long-window routes from a
        # few hundred addresses could reset a victim's second-factor
...
def _dual(op: str, *a, **kw):
    ...
    Without it, every call site would hand-roll this try-then-fall-back,
    each a little differently — `if store else None` here, `if store is None or
...
def record_strike(ip: str, route: str | None = None,
    ...
    # as a prior ban would turn a first offense's 15 minutes into hours,
    # which for CGNAT users sharing the address is collateral damage, not
    # deterrence.
    bkey = ("ip", ip)
...
# distributed-bruted.
#
# CRITICAL: callers must feed ONLY post-password failures here (a wrong
# TOTP after a correct password). A wrong PASSWORD must never
...
# precisely the attack Turnstile exists for. So the instance keeps a
...
# wrong, so moving it should be a code change with a test, not a config
# edit on a production box.
