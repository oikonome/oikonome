...
# user rename would show the new name on the Merchants page while the
...
# so the combined toggle is honoured); 'business' is its complement, so a
# business account's own transactions are reachable from the Accounts page.
...
def transactions(conn, year: int, month: int, *, search: str = "",
    ...
    # rule in both modes of this page.
    q = f"""SELECT {_TXN_FIELDS.format(acct=_ACCT_FULL)}
           FROM transactions t LEFT JOIN accounts a ON a.id = t.account_id
                LEFT JOIN items i ON i.id = a.item_id
                {_MC_JOIN} {_TXN_JOINS}
...
def search_transactions(conn, search: str, *, account_id: str | None = None,
    ...
    if search:
        ...
        # canonical map ("doordash" finds a truncated "Doordas" spelling), the merchant row's
...
def set_category(conn, txn_id: str, category: str,
    ...
    that delete would be silent and unrecoverable from this door.
    """
    ...
    if scope == "all" and category and category not in _cats.FLOW:
        ...
        if row and row["m"]:
            ...
            with conn.transaction():
                ...
                # and this HTTP request waits on it
...
# category once and every past + future transaction for THAT merchant (by
# its canonical) follows. Scoped to the history page's token-subset
...
# consequence count and this guard must name the same categories
...
def _write_user_rule(conn, canon: str, category: str) -> list[dict]:
    ...
    Example") would make the Rules page list one merchant as two, and when
    its category differs it breaks apply()'s unanimity gate so the user's
    new rule silently does nothing. The canonical-keyed rule covers every
...
def _page_match_sql(conn, payee: str) -> tuple[str, list] | None:
    ...
    is to ask the ledger which descriptors match: as a callable it would have to
...
def _token_family(conn, payee: str) -> list[str]:
    ...
    # Walking the DISTINCT list in Python instead would ship every descriptor
    # in the ledger to the app and re-tokenize it there, once per bulk
    # preview.
...
def merchant_category_preview(conn, payee: str, category: str,
    ...
    # payment. A user's merchant rule reaches those — a payment-app transfer to a
    # tutor is TRANSFER_OUT to the aggregator and education to the household,
...
# them all, model-written rules included
...
def list_rules(conn, source: str | None = None, q: str = "",
    ...
    if (q or "").strip():
        ...
        # lone percent would return every rule and make the collapsed
...
def _undo_rule_prior(p, what: str, default_source: str = "user") -> dict:
    ...
    legitimate, and refusing by content would make genuine snapshots
    un-undoable: a flow-named rule or an over-long custom label sits in
    the snapshot because it sat in the table. A restored flow-named rule
    is inert anyway: apply() refuses to
...
def _validate_undo(conn, undo) -> tuple[list[dict], list[dict],
    ...
    # "known" categories would reject genuine snapshots: a custom label that
    # lives only on the moved rows exists nowhere else once the write
    # lands, so its own undo would 400.
    affected: list[dict] = []
    ...
    with conn.transaction():
        ...
        for entry in canons:
            ...
            if prior:
                ...
                # that omits it would leave a previously-disabled rule live —
...
def link_reimbursement(conn, txn_id: str, other_id: str,
    ...
    A single per-tenant lock covers both the full and the partial path.
    The full path needs the transaction too: its INSERT must not commit
    ahead of the category writes, or spend math could see a pair whose
    expense is still ordinary spend and count it twice. Per-row locks would
    impose a lock order that one new call site could turn into a deadlock;
    a tenant lock has no order to get wrong. Linking is a hand-driven
