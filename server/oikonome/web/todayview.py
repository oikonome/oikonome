...
def _asset_ver() -> str:
    ...
    week-old bytes — which serves a stale pages.js after a deploy and leaves
    the console nav dead. A versioned URL is a fresh cache key end to end
...
def allowances(st: dict) -> tuple[dict, list, list]:
    ...
    plain text — and a second derivation is how the verdict pill drifted.
    """
...
def simple_day_meter(allow_tiles) -> tuple[float, float]:
    ...
    be absorbed by a neighbour's unused allowance). Raw sums here told a
    different story on the same screen: one bucket $19.50 over and another
    with $30 untouched rendered a two-thirds-full bar implying ~$10 left
    while the headline said $30. So spend counts against the bar only up
...
def build_context(st: dict) -> dict:
    ...
    # where the get-back-on-track framing lives now.
    ...
    # reported "short $5,000 by <the day of a $50 dip>"
...
_ELEMENT_STYLES = {
    ...
    # on top of the payee.
    # It bit the transactions table and not the forecast because that table
    # has no <thead>: under fixed layout the FIRST ROW sizes the columns, and
    # there that row is a data row carrying the 1% widths.
...
_CLASS_STYLES = {
    ...
    "hide-m": "",          # mobile-hide is a web concern; email keeps the column
}
...
class _Inliner(HTMLParser):
    ...
    - `.grid.colsN` used to be rewritten as side-by-side <td>s — two 49%
      panes squeezed to ~170px each on a phone. Grids now STACK: the grid
      div passes through as a plain block and its cards render full-width,
      one under the other, which is what the class table's "email is
      single-column" note always claimed. No mail client needs to support
      anything for stacked divs to work.
    - `display:flex` containers carrying `gap:` had their tiles jam
    ...
    def handle_starttag(self, tag, attrs):
        ...
        #
        # The web hides these on narrow screens via a media query; the email
        # used to keep them because "mobile-hide is a web concern". It isn't:
        # a mail client on a phone is the narrowest viewport this HTML ever
        # gets, and no mail client honours the media query that would have
        # saved it. So the transactions and forecast tables rendered five
        # columns — three of them white-space:nowrap — inside a 680px shell
        # on a ~380px screen, and the row simply ran off the card.
        #
...
def _brand_header() -> str:
    ...
    # mailable_base(), not the raw env: a LAN base (http://192.168.1.50:8042)
    # as the message's only link got the ENTIRE daily verdict silently
    # filtered while the linkless variant delivers. Without a mailable
    # base the header is simply not a link — the
