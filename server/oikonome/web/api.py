...
# encoding marker rather than as content. Tab and CR were always covered on
# exactly this reasoning; a line feed and a BOM are the same class, so the
...
def _csv_safe(v):
    ...
    if v[:1] in _CSV_INVISIBLE_LEAD + _CSV_FORMULA_LEAD \
        ...
        return "'" + v
    if v[:1] in ("\t", "\r"):
        # a leading tab/CR is neutralized whatever follows it — the original
        # rule, kept: it splits or shifts a cell in some readers on its own
        return "'" + v
    return v
...
def _today(conn) -> dt.date:
    ...
    day/month math. The instance clock is UTC on the hosted product while
...
def _same_site_download(request: Request) -> None:
    ...
    on any page a signed-in person visits dropped the file into their
    Downloads; the origin middleware wraps writes only. The household ZIP
    took the ticket route for this; the smaller downloads (worksheets,
...
def _ck_ym(y: int, m: int) -> None:
    ...
    Out of range these arrived at the engine as an unhandled ValueError from
    `dt.date(y, m, 1)` — so `?y=999999` was a 500, on /api/transactions,
    /api/business(+export.csv) and /api/receipts/report. Written inline,
    the guard is re-implemented per endpoint and every later endpoint that
    takes a y/m re-opens the same hole. One spelling, shared."""
    ...
    try:
        ...
        if bucket:
            ...
            info = _budget.bucket_ledger(
                _lenses.month_state(conn, yy, mm, today), bucket)
            if info is None:
        ...
        # search reads neither — so the same link opened a category's
        # ALL-TIME history. Derive the month's span server-side, and every
        # client old or new gets the month it asked for. An explicit range
        ...
        # raw sum of the listed rows. The raw sums counted a credit-card
        # payment as money out on top of the card swipes it paid for, and
        # counted transfers between own accounts on both sides, so the
        # header routinely showed an "overspent" month that was actually
...
def set_category(txn_id: str, user: dict = Depends(_user()),
    ...
    try:
        ...
        # `rule_written` is honest now: an "all" on a flow category or a
        # merchant-less row writes nothing, and the old `scope == "all"`
        # claim reported a write that never happened. When a rule IS
...
def _merchant_name_or_400(field: str, val: str,
    ...
    lists, and capping the read made exactly those rows unopenable."""
...
def accounts(user: dict = Depends(_user())):
    ...
    try:
        ...
        # prefix let "amazon-orders-key" stand in for "amazon", and a live
...
def _month_bill_stats(conn, cfg: dict, today: dt.date) -> dict[str, dict]:
    ...
    # income series: paychecks never entered the
    # occurrence engine — _recurring_bills was amount<0 only and _spend_rows
    # is outflows — so the Bills page's received column sat at "·" no
    # matter what posted. Same engine, deposit side: match this month's
    # income occurrences against posted non-transfer inflows.
    ...
    if income_bills:
        ...
        # inflow window left that occurrence "·" forever. Look back
...
def bills_toggle_api(user: dict = Depends(_user()),
    ...
    try:
        ...
        # idempotent — a bare flip netted a double-click to no change
    ...
    try:
        ...
        def _apply(cfg):
            ...
            if action == "restore":
                ...
                # ONE bill's analysis — the whole-table pass ran inside
                # the settings row lock and blocked every concurrent
...
def bills_history_api(user: dict = Depends(_user()), payee: str = ""):
    ...
    conn = _conn(user)
    try:
        hist = bills.merchant_history(conn, payee)
...
_SETTINGS_FIELDS = ("food_monthly", "other_monthly", "custom_buckets",
                    ...
                    # writable since (Net Worth page editor).
                    "excluded_accounts", "occurrence_caps", "disabled_bills",
                    "dismissed_hints", "manual_assets",
                    # smart-categorization LLM backend (setup-wizard parity;
                    # the SPA Settings page can render these later — additive,
                    # webapp untouched). llm_api_key and llm_extra_body are
                    ...
                    # ceiling is the whole cost control — it was readable
                    # only by editing the settings document by hand, which
                    # made it a constant in practice
                    "plaid_enrich_cap",
...
def _llm_is_local(url: str) -> bool:
    ...
    folds IPv4-mapped IPv6 spellings to their embedded v4 form). The old
    name heuristic (localhost / .local / dotless service name) survives
    only for names that do not resolve at all — an mDNS or compose name is
...
def _settings_view(cfg: dict) -> dict:
    ...
    # Assigned BEFORE the withhold loop: assigned after, the loop's pop is
    # a dead no-op and this one field of the twelve escapes the hosted
    # withholding invariant — true for a tenant carrying an older BYO MX key
    # row, rendering an MX credential card the platform's API refuses.
...
"""
    ...
    try:
        ...
        # read too; a fresh admin connect per Settings load bought nothing
...
def settings_set(user: dict = Depends(_user()), body: dict = Body(...)):
    ...
    try:
        ...
        # the behaviour that was already wanted: a 400 on field nine
        ...
        with budget.config_txn(conn) as cfg:
            ...
            if "plaid_enrich_cap" in body:
                ...
                cfg["plaid_enrich_cap"] = max(0, min(cap,
                                                     _enrich_cap_ceiling()))
            # A cap written before this ceiling existed — or before an
            # operator lowered it — is still sitting in the document and
            # would keep authorising that spend forever. Any save past this
            ...
            if "email_schedule" in body:
                ...
                if raw in (None, {}):
                    ...
                    clean = {}
                    # yearly joined the cadences; each entry carries
            ...
            if "today_view" in body:
                ...
                # pre-rename spellings, briefly live on installed builds
            ...
            if "manual_assets" in body:
                ...
                for a in assets_in[:30]:
                    ...
                    entry = {"name": name,
                             ...
                             "as_of": (str(a.get("as_of"))[:10]
                                       if a.get("as_of")
                                       else _today(conn).isoformat())}
            ...
            if "llm_url" in body or "llm_model" in body or "llm_api_key" in body:
                ...
                if url:
                    ...
                    if "llm_extra_body" in body and body["llm_extra_body"] is None:
                        ...
                        # or "[1,2]" passed a loads-only check and then
                        # crashed every call using the backend
                        try:
                            _eb = _json.loads(eb)
                        except ValueError:
                            _eb = None
                        ...
                        # written before this pass through decrypt
                        # unchanged, so old configs keep working)
            ...
            if "llm_backends" in body:
                ...
                if _fold_legacy:
                    ...
                    # resend them; omission means "keep". A value stored
                    # before either was encrypted at rest carries no
                    # envelope tag: encrypt it as it crosses over, because
            ...
            # sends {llm_vision_model} alone — with the write path only
...
def settings_recipient_resend(user: dict = Depends(_user()),
    ...
    try:
        ...
        # On hosted, being ON the saved list is not enough — a config
        # written before the member-only rule can still hold a free-form
        # external address, and this door would then make the platform mail
        # invite
...
def retirement_api(user: dict = Depends(_user()),
    ...
    try:
        ...
        # `resume` is an age like the others and was the one left unbounded.
        ...
        # These three were floored but never CAPPED, and the block
        ...
        # unquantized dollar was a distinct cache key: spend=1,2,3,… missed
        # the cache on every request and each miss ran the full simulation
        # in a shared threadpool worker. A projection does not change
        # meaningfully between $52,340 and $52,000 of spend — it does
...
def _parse_receipt_async(tenant_id: str, receipt_id: str) -> None:
    ...
    def _run() -> None:
        ...
        try:
            ...
            logging.getLogger("oikonome.receipts").warning(
                "background receipt parse failed tenant=%s: %s",
                tenant_id, e)
        finally:
            if conn is not None:
                conn.close()
...
async def receipt_upload(txn_id: str, user: dict = Depends(_user()),
    ...
    # Stream with a hard ceiling — an unbounded read() let an
    ...
    # receipts.add to refuse at 5 MB was 45 MB of pointless exposure.
    ...
    try:
        try:
            # add now runs the image-optimize (Pillow) pass; keep
...
def _categorize_after_import(tenant_id: str) -> None:
    ...
    files-only first run reached the wizard's budgets step with everything
...
async def import_bulk_analyze(user: dict = Depends(_user()),
    ...
    for f in files:
        ...
        total += len(data or b"")
        # The per-FILE cap alone let 200 × 50 MB sit in this worker's
    ...
    def _analyze():
        ...
        try:
            ...
            # size 0 was misleading, report the cap instead
...
def business_list(user: dict = Depends(_user()), year: int | None = None,
    ...
    # `limit` was bounded from the start and `offset` only floored, so an
    # offset past bigint reached OFFSET and died inside the driver as
...
def enrich_status_api(user: dict = Depends(_user())):
    ...
    try:
        ...
        # separately here drifted from what a click would actually send
...
def plan_activity_import_api(user: dict = Depends(_user()), body: dict = Body(...)):
    ...
    # The collector posts whatever it scraped. A non-string csv reached the
    # parser's string methods, and a balance that is not a number reached a
    # float column — both surfaced as an opaque 500, which tells a scripted
...
def jobs_sync_api(user: dict = Depends(_user())):
    ...
    # per-tenant single-flight lock is already held. Every value in it was
    # then checked for an "error" prefix, "already-running" is not one, and
    # the door answered ok=True with an empty result set — a caller pressing
    # Sync while one was running was told it had synced nothing, successfully.
    if results.get("_status") == "already-running":
...
#
# Shortening the claim is safe because it was never the real guard.
...
def notify_phone_api(user: dict = Depends(_user()), body: dict = Body(...)):
    ...
    try:
        if not number:                        # clear
            # Same lock as the send/verify paths below (this branch was
            # left on a bare load/save long after those two took the
            # lock). Dropping a phone number is exactly when a
        ...
        # non-list scalar — reached `for p in 1` and raised TypeError, so a
        # scripted call got a 500 instead of the 400 this branch exists to
...
# either. (A resolve-time IP check was tried too but flaps when a
# known-good host's DNS is briefly unavailable; the allowlist is the
# robust gate.)
_PUSH_HOST_SUFFIXES = (
...
def push_subscribe_api(user: dict = Depends(_user()), body: dict = Body(...)):
    ...
    #
    # Takeover now requires proof of possession — the same browser
