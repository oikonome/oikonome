...
they expected and where), optionally attaches a screenshot, and submits. The app assembles a package —
feedback.md (message + version), the Doctor
...
_REDACTIONS = [
    ...
    # rules above never matched a plain address. Local part is kept to one
    ...
    try:
        ...
        try:
            n = admin.execute(
                ...
                "WHERE coalesce(status, 'active') <> 'pending_delete'"
            ).fetchone()["n"]
        finally:
            admin.close()
        return int(n) <= 1
...
def build_package(conn, tenant_id: str, number: str, message: str,
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        # The log ring is process-wide,
        # so on a MULTI-TENANT box it carries other households' log lines
        # (emails, ids, error payloads) — PII not tenant-scoped even though
        # secret SHAPES are redacted. (doctor.bundle above is tenant-scoped.)
        #
        ...
        # on what actually matters: the real tenant count. Unknown (query fails) is treated as multi —
        # withholding logs degrades a support bundle, leaking them is a
        # privacy incident.
