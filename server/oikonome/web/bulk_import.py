...
def analyze(conn, files: list[tuple[str, bytes]]) -> dict:
    ...
    # their rows, so a file routed there would import nothing.
...
def _ensure_account(conn, name: str, amount_sign: str = "bank") -> str:
    ...
    # the same new account at once (web + phone, a double submit) would both
    # see no row and the second INSERT would hit the primary key. Serialize
    # the slug decision per tenant for the length of this call.
...
def run(conn, token: str, decisions: list[dict],
    ...
    # dispatch takes it again (same session, re-entrant). If a sync already
    # holds it, refusing here leaves the staged batch where the token still
    # points, instead of consuming the bytes and then failing every file.
...
def _run_batch(conn, batch: dict, decisions: list[dict], role: str,
    ...
    for d in decisions:
        ...
        # number, is skipped here, outside the per-file try, so it cannot
        # fail the whole run.
        ...
        try:
            ...
            # Its rows do not count toward this run's row budget (they are
            # not parsed here); restore_zip carries its own
