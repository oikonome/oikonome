...
zip the user sends manually. Lives under /api/testing/* for historical
reasons — a guided-testing checklist once shared this module.
"""
...
@router.get("/testing")
def testing_page():
    """Old bookmarks and the retired checklist's links land on Feedback."""
...
def testing_api(user: dict = Depends(_user())):
    ...
    conn = tenancy.tenant_connect(user["tenant_id"])
    try:
        # demo: off and not switchable — the submit door
        # already denies demos, this stops the UI offering it at all
...
async def testing_feedback_api(user: dict = Depends(_user()),
    ...
    # A demo visitor could relay bounded spam
    # to the operator's feedback address (demoguard covers data doors but
    # not this). Deny outright on a demo instance.
    from . import demoguard
    demoguard.deny(user)
    conn = tenancy.tenant_connect(user["tenant_id"])
    try:
