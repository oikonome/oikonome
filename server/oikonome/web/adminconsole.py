...
def _require_passkey() -> bool:
    ...
    (hosted does not need it — the token cannot sign in there at all).
...
def _cloak(request: Request):
    ...
    before form validation (an in-handler check would answer a 422 for
    missing fields on a cloaked instance)."""
    ...
    try:
        ...
        # The edge policy only binds if the origin enforces it too. Log the
        # reason (the operator debugging their own Access config gets
        # nothing from a bare 404) but tell the caller nothing.
...
def _multi_tenant() -> bool:
    ...
    `signup_page` redirects to /login, and an invite minted there would
    carry a link that cannot be used.
...
def _render(request: Request, **ctx) -> HTMLResponse:
    ...
    # perfectly normal token-only install.
...
def _dashboard(request: Request, minted_link: str | None = None,
    ...
    try:
        ...
        # the correlated form is O(tenants × table scans), which a database
        # holding thousands of tenants (a shared test database, say) cannot
        # afford. Capped: a fleet view past 500 rows needs pagination, not
        # scrolling.
    ...
    return _render(request, authed=True, tenants=tenants,
                   ...
                   # passkeys_enrolled, and Jinja's Undefined is falsy: the
                   # authed dashboard must pass it, or the form carries no
                   # step-up ticket and a second-key enrolment is refused
                   # after a full WebAuthn ceremony.
...
def _version_history(limit: int = 2000) -> list[dict]:
    ...
    box at this time". This is BUILD history — every version that has
    existed, deployed or not. Two panels, two meanings.

    Missing or unreadable manifest returns [] and the panel says so.
    The default limit is large enough for the full build history; a
    smaller one silently clips the earliest builds off the bottom.
    """
...
def _passkey_count() -> int:
    ...
    try:
        ...
        # behave as "none enrolled", which leaves the token path working
        # rather than locking the operator out mid-migration
        return 0
    finally:
        admin.close()
...
def login(request: Request, token: str = Form("")):
    ...
    if not secrets.compare_digest(_h(token), _h(real)):
        return _render(request, authed=False,
                       ...
                       passkeys_enrolled=_passkey_count())
    # The token is break-glass. Where the operator has said so, knowing it
    # is not enough — check AFTER the compare so a wrong token still reads
    # as a wrong token, and count the enrolled keys only for a right one
    # (no probe can measure the flag).
...
def _console_origins() -> list[str]:
    ...
    private-network hostname, say, which is how a PHONE reaches the console
    without any DNS configuration at all (a phone cannot use an /etc/hosts
    entry). Each extra origin gets its own passkey, because WebAuthn binds
    a credential to one relying party.
...
def passkey_register(request: Request, body: dict = Body(...)):
    ...
    * a console session — but ONCE A KEY EXISTS that session must also
    ...
    try:
        ...
        # lost. Minting a ticket needs shell on the host, which is strictly
        # stronger proof than a console session, and the ticket is
...
@router.post("/passkey/login/options",
             # Its own bucket — sharing "admin_login" would let ~5 passkey
             # ceremonies/hour lock the console's ONLY credential out
...
def _mint_invite(request: Request, *, email: str = "", days: int = 14,
    ...
    # raw form field into arithmetic. `mint` applies this identical bound
    # itself, so every caller (this form, the CLI) inherits it; the clamp
    # stays here because `days` is also written into the audit line below,
    # and an audit that records the number typed rather than the lifetime
    # actually minted is worse than no audit line at all.
    ...
    try:
        ...
        # one atomic act under an instance-wide advisory lock — otherwise any
        # number of invites could go out while one seat remained, and two
        # concurrent mints could both read the same count and both pass.
        # The target address's own outstanding invite is
...
def _host_state() -> dict:
    ...
    if bdir.is_dir():
        ...
        # human-readable rollup: count, total size, the newest/oldest,
        # distinct days covered, and a daily-health verdict (a dump inside
        # the last ~30h means the nightly job is running)
        if dumps:
    ...
    # healthy box); a non-one-shot service that exited 0 is STOPPED
    # (intentionally down, e.g. bundled postgres on a managed-DB stack);
    # only a non-zero exit / unhealthy / restarting / dead container is red,
    # so a real alert is never buried among healthy stopped containers.
        ...
        counts = admin.execute(
            """SELECT (SELECT count(*) FROM accounts WHERE tenant_id=%s)
                      ...
                      (SELECT count(*) FROM items WHERE tenant_id=%s
                         AND COALESCE(status,'ok') NOT IN ('archived', 'restored'))
                        AS items""",
...
def tenant_delete(request: Request, tenant_id: str = Form(...),
    ...
    try:
        ...
        if mode == "immediate" or grace == 0:
            # An immediate purge is unrecoverable outside backups — for the
            # household it is every bit as destructive as host
            # restore/reset/uninstall, which demand a fresh single-use
            # passkey assertion so a stolen 4-hour console cookie cannot
            # reach them. Same gate here; skipped only when no operator key
            # exists (plain-http self-host, same rule as run_command).
            # Grace-mode stays ungated: it is restorable for the whole
            # window.
            if _passkey_count():
                sess = _session(request)
        ...
        # it when not already pending_delete, so a double-schedule can't
        # overwrite the true prior state with 'pending_delete'.
        admin.execute(
            "UPDATE tenants SET "
...
# Single-use confirmation nonces for the reboot flow — a browser replaying
# the "go" POST (tab restore / refresh resubmission) finds its nonce
# consumed and gets a harmless notice instead of a reboot. In-memory on
# purpose: a restart forgetting nonces fails toward refusing.
...
# with the same nonce would run the uninstall. On a passkey-less self-host
# this arm/confirm pair is the ONLY server-side friction before the root
...
def run_command(request: Request, command: str = Form(...), arg: str = Form(""),
    ...
    # never run. os.link below IS the check: it fails if the flag is
    # already there, atomically.
