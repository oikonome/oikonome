...
def mailable_base() -> str:
    ...
    Measured with a controlled A/B: a daily verdict carrying
    `http://192.168.1.50:8042/app/` as its only link is silently dropped by
    the big mail providers, while the linkless version — same relay, same
    sender, same minute — is delivered. A bare-IP plain-HTTP link is a
    classic phishing signature, and every LAN self-host has exactly that
...
def gather(conn, today: dt.date, *, live: bool = True) -> dict:
    ...
    # here flipped those sends "historical" and silently dropped the whole
    ...
    # tenant_settings round trip here was pure waste.
    ...
    if not live:
        ...
        cards = conn.execute(
            """SELECT i.institution_name AS bank, a.mask, a.balance_current AS bal
               ...
               WHERE a.type = 'credit'
                 ...
                 -- two rendered side by side and disagreed by the business
                 -- balance)
        ...
        # made headroom too tight), overdue-unpaid re-dated rather than dropped
        # (which made headroom too rosy), and every month in the window expanded,
        ...
        # bill — but due_total counted bills only, so headroom read too
...
def build(d: dict, *, summary: bool = False) -> tuple:
    ...
    # number seeded from a lie. The plain-text mirror did not, so the one
    # surface that cannot show a caveat was the one still printing
    # "short $412 today". Mirror the suppression; the set-your-balance
    ...
    # (a LAN URL in the body is what got the whole message filtered).
    base = mailable_base()
...
def _flag_on_unless_disabled(name: str) -> bool:
    ...
    something. Reading it as `== "1"` instead — as this did — meant an
    operator who wrote `true` silently got STARTTLS DISABLED: mail went
    out in the clear while the configuration said the opposite."""
...
def resolve_smtp(conn) -> dict:
    ...
    if one predates the guard that now refuses to save it. Self-hosted: the
    ...
    # tenant smtp_host there, but a row written before that guard existed
    # would otherwise keep winning here forever — and a stale relay that
    ...
    if cfg.get("smtp_host"):
        # belt: configs saved before the guard (or smuggled past a
        # future door) get re-checked at the moment of use — a blocked host
...
def send_each(subject: str, plain: str, html: str, recipients: list[str],
    ...
    This replaces the To=sender + Bcc=everyone shape for list mail, under
    which a second address on the recipient list can silently never receive
    the daily verdict while the first does. Three reasons the fan-out is
    correct and the Bcc blast was not:

    * **Deliverability.** `smtplib.send_message` strips the Bcc header, so the
      blast arrived as `To: <sender>` with the actual
    ...
    * **Visibility.** One blast has ONE outcome, so a per-recipient failure
      was invisible: the sweep recorded success as long as the relay accepted
      the envelope. Now every address has its own result.
    * **Privacy.** Bcc already hid recipients from each other; a per-recipient
    ...
    # a household with four recipients paid four of those every cadence,
    # every day, to delete nothing. Only addresses actually carrying a
    ...
    for r in recipients:
        try:
            ...
            # evidence we get that an address works — spend it. This is
...
def send(subject: str, plain: str, html: str, recipients: list[str],
    ...
    given, else env (self-hosted default). Hosted swaps in a provider
    (SES/Postmark) at the jobs layer — same signature.
    ...
    operator notice) — To=the recipient directly. The old To=self+Bcc
    shape delivered TWICE when sender and recipient share a mailbox (an
    alias fan-in) and made the To header read as the sender."""
    s = smtp or _env_smtp()
    ...
    # Postmark link tracking wraps every URL in a track.pstmrk.it redirect.
    # For one-time links (password reset / welcome set-password) that redirect
    # — or an inbox scanner prefetching it — can consume or mangle the single
    # token before the human clicks, so the link "errors". Force tracking off
    # per-message so it can't drift back on at the server level; this also
    # matches Oikonome's no-tracking stance. Harmless header on non-Postmark
    # relays, so gate on the host to keep other providers' logs clean.
    ...
    # On hosted, connect to the address that PASSED the policy check,
...
def _smtp_deliver(msg, s: dict, host: str, connect_host: str, port: int,
    ...
    with smtplib.SMTP(connect_host, port, timeout=30) as smtp_conn:
        ...
        if s["starttls"]:
            ...
            # the same thing, and the sibling STARTTLS knob read that way
            # turned encryption off on exactly that spelling
