...
class PromotionTests(unittest.TestCase):
    ...
    def test_edit_promotes_to_user(self):
        cache(self.conn, "BIGBOX", "FOOD_AND_DRINK")         # llm rule
        data.set_merchant_category(self.conn, "BIGBOX", "GENERAL_MERCHANDISE")
        row = self._source("BIGBOX")
        self.assertEqual(row["source"], "user")
        # and it now lists under the user group, not llm
        self.assertEqual(
            [r["merchant"] for r in
             data.list_rules(self.conn, source="user")["rules"]], ["BIGBOX"])
