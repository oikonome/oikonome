...
that the CSV no longer reports are soft-retired. The prune is namespaced by
id prefix, like the coinbase door's: keyed on account + date alone, a
transaction some OTHER importer (CSV/OFX, a manual entry) had landed on the
same plan account for a restated date would be retired too, and the holdings
recomputation would drop its shares.
"""
