...
class DoorTests(unittest.TestCase):
    ...
    def test_plan_import_stamps(self):
        plan_csv.import_csv(self.conn, "401a", PLAN_CSV)
...
class AlertSweepTests(unittest.TestCase):
    ...
    def test_a_push_during_the_send_is_not_re_flagged(self):
        """The collector recovering WHILE the alert mail is going out used
        to be stamped as alerted anyway.

        `stamp` clears alerted_at on every push, and the alert then wrote it
        straight back onto a source that was healthy again. Nothing clears
        the flag except a push — so the next time that collector went quiet,
        which is precisely when there are no more pushes, it was skipped and
        stayed skipped."""
        self._stale_watched()
        ...
        # and the relapse still alerts, which is the behaviour that was lost
        _backdate(self.conn, "s1", 60)
        with mock.patch("oikonome.web.report.send") as send:
            self.assertEqual(worker.script_alert_tenant(self.tid),
                             {"alerted": 1})
