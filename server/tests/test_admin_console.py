...
class _Base(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        os.environ.pop("OIKONOME_ANALYTICS_URL", None)
        os.environ.pop("OIKONOME_ANALYTICS_TOKEN", None)
        import oikonome.web.adminconsole as _ac
        _ac._SITE_CACHE = {}

    def test_site_tab_hidden_when_analytics_unconfigured(self):
        # No OIKONOME_ANALYTICS_* env → the Site tab must not render at all
        # (self-host installs never inherit the marketing site's data).
        self._login()
        r = self.client.get("/admin/console")
        self.assertEqual(r.status_code, 200)
        self.assertNotIn('data-view="site"', r.text)
...
class ActionTests(_Base):
    ...
    def test_the_invite_lifetime_is_bounded_like_its_free_period(self):
        ...
        form value that reaches arithmetic is clamped, `free_days` on the
        same form included."""
        self._login()
...
class DeploySummarySourceTests(unittest.TestCase):
    ...
    def test_summary_prefers_manifest_over_env_note(self):
        # the exact bug: env note stale, manifest correct -> manifest wins
        from unittest import mock
...
class SingleHouseholdTests(_Base):
    ...
    def test_a_hosted_instance_still_has_them(self):
        ...
        hosted box (passkey-only, by decision), so assert the predicate
