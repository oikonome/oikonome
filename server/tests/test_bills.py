...
class TestFitters(unittest.TestCase):
    ...
    def test_refill_like(self):
        ...
        varied = [ev(TODAY, a) for a in (28.68, 20.86, 22.16, 24.44, 14.19, 35.54)]
...
class TestProposals(DetectBase):
    ...
    def test_city_named_membership_survives_a_stray_lunch(self):
        ...
        self._monthly_series("SPRINGFIELD REC CENTER", 120.10, months=6, day=2)
        add_txn(self.conn, TODAY - dt.timedelta(days=40), 113.14,
                "SPRINGFIELD TAP HOUSE", primary="FOOD_AND_DRINK")
        add_txn(self.conn, TODAY - dt.timedelta(days=9), 91.12,
                "SPRINGFIELD TAP HOUSE", primary="FOOD_AND_DRINK")
        rd.run(self.conn, TODAY)
        adds = self.pending("add")
        self.assertEqual([p["payee"] for p in adds], ["SPRINGFIELD REC CENTER"])
        self.assertEqual(adds[0]["amount"], 120.10)
    ...
    def test_variable_utility_becomes_an_envelope(self):
        ...
        amts = [434.05, 380.20, 210.00, 95.40, 81.72, 150.10, 287.00]
        for i, amt in enumerate(amts):
            add_txn(self.conn, TODAY - dt.timedelta(days=15 + 30 * i), amt,
                    "NORTHSIDE POWER", primary="RENT_AND_UTILITIES")
        rd.run(self.conn, TODAY)
        adds = [p for p in self.pending("add") if p["payee"] == "NORTHSIDE POWER"]
        self.assertEqual(len(adds), 1)
    ...
    def test_long_cycle_two_hits_need_the_same_merchant(self):
        ...
        add_txn(self.conn, TODAY - dt.timedelta(days=200), 51.01,
                "CO SPRINGFIELD RIVERFRONT PAR", primary="ENTERTAINMENT")
        add_txn(self.conn, TODAY - dt.timedelta(days=18), 54.00,
    ...
    def test_food_needs_refill_fingerprint(self):
        self._monthly_series("BISTRO", 43.17)       # varied-ish single monthly
...
class TestMaintenance(DetectBase):
    ...
    def test_malformed_amount_set_on_does_not_crash_detection(self):
        ...
        add_bill(self.conn, "Northside Power", 440.0, next_due=TODAY + dt.timedelta(days=5),
                 last_seen=TODAY - dt.timedelta(days=25))
        self.conn.execute("""UPDATE bills SET raw = raw || '{"amount_set_on":
                             "not-a-date"}'::jsonb WHERE payee='Northside Power'""")
        for i, amt in enumerate((370.07, 372.0, 365.0)):
            add_txn(self.conn, TODAY - dt.timedelta(days=10 + 30 * i), amt, "NORTHSIDE POWER")
        rd.run(self.conn, TODAY)                        # must not raise
        self.assertEqual(rd.get_bill(self.conn, "Northside Power")["amount"], -370.07)
    ...
    def test_drift_auto_applies_with_audit(self):
        ...
        for i, amt in enumerate((370.07, 372.0, 365.0)):
            add_txn(self.conn, TODAY - dt.timedelta(days=10 + 30 * i), amt, "NORTHSIDE POWER")
        rd.run(self.conn, TODAY)
        bill = rd.get_bill(self.conn, "Northside Power")
        self.assertEqual(bill["amount"], -370.07)
    ...
    def test_drift_notice_clears_once_the_amount_is_changed_again(self):
        ...
        self.conn.execute("UPDATE bills SET amount = -400.0 WHERE payee='Northside Power'")
    ...
    def test_manual_edit_gets_drift_grace(self):
        ...
        for i, amt in enumerate((370.07, 372.0, 365.0)):
            add_txn(self.conn, TODAY - dt.timedelta(days=10 + 30 * i), amt, "NORTHSIDE POWER")
        rd.run(self.conn, TODAY)
        self.assertEqual(rd.get_bill(self.conn, "Northside Power")["amount"], -440.0)

    def test_grace_expires(self):
        add_bill(self.conn, "Northside Power", 440.0, next_due=TODAY + dt.timedelta(days=5),
                 last_seen=TODAY - dt.timedelta(days=25), source="manual")
        # age the stamp past the window
        row = rd.get_bill(self.conn, "Northside Power")
        raw = as_dict(row["raw"])
