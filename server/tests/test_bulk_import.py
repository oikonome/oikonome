...
class AnalyzeTests(unittest.TestCase):
    ...
    def test_an_initials_account_name_can_still_be_matched(self):
        """An account whose name is an initialism plus filler ('ING
        Savings') matched no filename under the four-letter floor, so every
        statement dropped for it created a SECOND account beside the real
        one. The short word is used only because nothing longer survives —
        a name with a real word still never matches on a short one."""
