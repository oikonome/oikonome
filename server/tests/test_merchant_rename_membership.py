...
class RenameCoversLateArrivals(unittest.TestCase):
    ...
    def test_a_spelling_that_joins_mid_rename_is_renamed_too(self):
        ...
        add_txn(conn, "2026-07-01", 10.0, "BIGBOX WHSE #0555",
                merchant="BIGBOX WHSE #0555")
        merchant_dedup.apply(conn)
        display = conn.execute(
            "SELECT canonical FROM merchant_canonical "
            "WHERE raw_merchant='BIGBOX WHSE #0555'").fetchone()["canonical"]
        ...
        self.assertEqual(moved["BIGBOX WHSE #0555"], "Bigbox Wholesale")
