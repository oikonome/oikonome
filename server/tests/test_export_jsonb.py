...
(str() wrote Python repr — single quotes — which json.loads on the restore
side rejected, so raw/by_class silently round-tripped as NULL through
product→product export/restore)."""
...
class ExportJsonbTests(unittest.TestCase):
    ...
    def test_formula_injection_neutralized_in_export_csv(self):
        ...
        quote-prefixed so Excel/Calc treat it as text, not a formula
        (/export used _csv_cell, which lacked the
        neutralization api._csv_safe already applied elsewhere)."""
        conn = tenancy.tenant_connect(self.tid)
        try:
            conn.execute(
                """INSERT INTO transactions (id, account_id, date, amount,
    ...
    def test_export_restores_into_fresh_tenant_with_raw_intact(self):
        """The product→product round trip the bug broke: export from one
        tenant, restore into another, raw survives byte-for-byte."""
        r = export_get(self.client, "/export")
        dest = make_db()
        try:
            counts = restore.restore_zip(dest, r.content)
        ...
        try:
            ...
            conn.execute(
                """INSERT INTO transactions (id, account_id, date, amount,
                   ...
                   VALUES ('exr-1','chk','2026-07-10',80.0,'COSTCO',0,0)""")
            conn.execute(
