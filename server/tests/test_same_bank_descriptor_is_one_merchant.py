...
unenriched refund keys on the raw descriptor. Unbridged, that would mint a
second merchant for one payee and split its history, and the alias memory
cannot bridge the two because it is stored under the key that differs. Refunds and
...
class SameDescriptorOneMerchantTests(unittest.TestCase):
    ...
    def test_unenriched_refund_joins_the_enriched_charge(self):
        charge = add_txn(self.conn, "2026-09-09", 25.00, DESCRIPTOR,
                         merchant="example telecom", txn_id="chg1")
        merchant_identity.resolve(self.conn)
        refund = self._unenriched("2026-09-10", -25.00, DESCRIPTOR, "ref1")
        merchant_identity.resolve(self.conn)
    ...
    def test_one_merchant_row_for_the_descriptor(self):
        add_txn(self.conn, "2026-09-09", 25.00, DESCRIPTOR,
                merchant="example telecom", txn_id="chg2")
        merchant_identity.resolve(self.conn)
        self._unenriched("2026-09-10", -25.00, DESCRIPTOR, "ref2")
        merchant_identity.resolve(self.conn)
        n = self.conn.execute(
