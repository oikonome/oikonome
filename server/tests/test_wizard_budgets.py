...
class BucketCandidateTests(unittest.TestCase):
    ...
    def test_food_includes_mtd_and_sparse_months(self):
        # All grocery spend is in the CURRENT partial month. The window
        # must include it; otherwise 2×median(0) drops any remaining
        # non-zero months → food suggests $0 / a blank wizard field.
    ...
    def test_approved_bill_merchant_excluded_even_when_amount_drifts(self):
        # Utility bill is $100; July charge is $140 — an amount gate would
        ...
        for mo, amt in ((4, 100.0), (5, 100.0), (6, 100.0), (7, 140.0)):
            add_txn(self.conn, dt.date(2026, mo, 5), amt, "CITY POWER",
                    primary="RENT_AND_UTILITIES", account="chk",
                    txn_id=f"pw-{mo}")
        bills_mod.save_bill(self.conn, payee="City Power", amount=100.0,
                            frequency="MONTHLY", interval=1,
                            next_due=dt.date(2026, 8, 1), merchant="city power",
                            source="detected")
        out = budget.suggest_budgets(self.conn, TODAY)
        self.assertFalse([c for c in out["bucket_candidates"]
                          if "Rent" in c["name"]],
                         out["bucket_candidates"])
        # no residual city power in other
