...
`_advance_due_dates` must look as far ahead as the bill's own cycle. A
YEARLY interval-2 bill's next occurrence is 24 months out; a fixed 14-month
scan would find nothing, leave its `due_on` at the date it was paid on, and
the Bills page would call it "overdue" every day after.
"""
    ...
    def _bill(self, bid, due, recurrence):
        self.conn.execute(
            """INSERT INTO bills (id, type, payee, amount, frequency,
                                  ...
                                  is_completed, raw)
               VALUES (%s, 'BILL', %s, -672.00, %s, -28, %s, 1, 0, %s::jsonb)""",
