...
Every migrate ALTERs the role password to the env value (idempotent).
Creating the role only when absent is not enough: an operator changing
OIKONOME_APP_PASSWORD in docker/.env would restart into an app that cannot
log in (or, worse, keep the old password working forever).
