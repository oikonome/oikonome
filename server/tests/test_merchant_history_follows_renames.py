...
too. Resolved on the bank's raw descriptor instead, the app would show a
name it could not then look up: a merged fuel arm would come back empty, and
a chain's own page would sweep in the pump charges the ledger had split off —
the history and the catalog disagreeing about the same merchant in both
directions.
...
class FuelArmHistoryIsTheFuelArm(unittest.TestCase):
    ...
    display as 'Northwind Club Gas' while the warehouse rows display as 'Northwind Club'.
    The history page has to honour that split in both directions — the map
    cannot help it, because the map is keyed by merchant STRING and 'Northwind Club'
    ...
    def setUp(self):
        ...
        for day, amount in ((3, 30.00), (10, 40.00)):
            tid = add_txn(self.conn, dt.date(2026, 8, day), amount,
                          "NORTHWIND CLUB GAS #0555", merchant="Northwind Club", account="chk")
            self.conn.execute(
                "UPDATE transactions SET merchant_outlet=%s WHERE id=%s",
                ("Northwind Club Gas", tid))
        for day, amount in ((5, 250.00), (12, 120.00)):
            add_txn(self.conn, dt.date(2026, 8, day), amount,
                    "NORTHWIND CLUB WHSE #0555", merchant="Northwind Club", account="chk")
    ...
    def test_the_pump_page_shows_the_pump_charges_only(self):
        self.assertIn("Northwind Club Gas", _displays(self.conn))
        h = bills.merchant_history(self.conn, "Northwind Club Gas", today=TODAY)
        self.assertEqual(sorted(t["amount"] for t in h["txns"]),
                         [30.00, 40.00])
        cat = _catalog(self.conn, "Northwind Club Gas")
    ...
    def test_the_warehouse_page_does_not_swallow_the_pump(self):
        h = bills.merchant_history(self.conn, "Northwind Club", today=TODAY)
        self.assertEqual(sorted(t["amount"] for t in h["txns"]),
                         [120.00, 250.00])
        cat = _catalog(self.conn, "Northwind Club")
        self.assertEqual(h["lifetime"]["count"], cat["rows"])
        self.assertEqual(h["lifetime"]["total"], float(cat["total"]))
    ...
    def test_a_merged_fuel_arm_is_found_under_its_new_name(self):
        ...
        merchant_dedup.rename(self.conn, "Northwind Club Gas", "Northwind Club Fuel")
        self.assertIn("Northwind Club Fuel", _displays(self.conn))
        h = bills.merchant_history(self.conn, "Northwind Club Fuel", today=TODAY)
        self.assertEqual([t["payee"] for t in h["txns"]],
                         ["Northwind Club Fuel", "Northwind Club Fuel"])
        cat = _catalog(self.conn, "Northwind Club Fuel")
...
class RenamedMerchantHistoryFollowsTheName(unittest.TestCase):
    ...
    so a token-subset match cannot find these rows — the page must still
    answer for a merchant the ledger has just rendered."""
...
class BillMatchingStaysOnTheBankText(unittest.TestCase):
    ...
    def setUp(self):
        self.conn = make_db()
        write_config(self.conn)
        add_bill(self.conn, "Zenith Fibre", 79.99, next_due=dt.date(2026, 9, 5))
        for day, name in ((5, "ZENITH FIBRE TELECOM"),
                          (5, "ZENITH FIBRE")):
            add_txn(self.conn, dt.date(2026, 7 if day == 5 else 8, day), 79.99,
                    name, account="chk")
        add_txn(self.conn, dt.date(2026, 8, 5), 79.99, "ZENITH FIBRE TELECOM",
                account="chk")
        merchant_dedup.apply(self.conn)
    ...
    def test_bill_history_matches_raw_tokens_across_spellings(self):
        h = bills.merchant_history(self.conn, "Zenith Fibre", today=TODAY)
    ...
    def test_renaming_the_merchant_does_not_move_the_bill_history(self):
        merchant_dedup.rename(self.conn, "Zenith Fibre Telecom",
                              "Fibre Internet Co")
        h = bills.merchant_history(self.conn, "Zenith Fibre", today=TODAY)
        self.assertEqual(len(h["txns"]), 3)
        self.assertEqual(h["lifetime"]["total"], 239.97)
