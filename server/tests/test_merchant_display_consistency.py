...
class RenamedMerchantOnTheLedger(unittest.TestCase):
    ...
    def setUp(self):
        self.conn = make_db()
        self.tid = add_txn(self.conn, "2026-07-10", 42.50, "COSTCO GAS #0007",
                           merchant="Costco")
        _set_outlet(self.conn, self.tid, "Costco Gas")
        # the user's rename, via the real feature — it keys the manual
        # canonical row on the displayed string, i.e. the outlet
        merchant_dedup.rename(self.conn, "Costco Gas", "Fuel Stop")
...
class BulkWriteScopeMatchesThePage(unittest.TestCase):
    ...
    displays them. An outlet row ("Costco Gas") renamed to "Fuel Stop"
    displays under the new name and under NOTHING else — so it must be in
    "Fuel Stop"'s write scope and out of "Costco"'s. Keyed on
    merchant_name it would be the other way around."""

    def setUp(self):
        self.conn = make_db()
        self.pump = add_txn(self.conn, "2026-07-08", 60, "COSTCO GAS #0007",
                            merchant="Costco")
        _set_outlet(self.conn, self.pump, "Costco Gas")
        self.store = add_txn(self.conn, "2026-07-09", 120, "COSTCO WHSE #0007",
                             merchant="Costco")
        merchant_dedup.rename(self.conn, "Costco Gas", "Fuel Stop")
    ...
    def test_the_sibling_brand_rows_stay_out_of_that_scope(self):
        targets = {r["id"]
                   for r in data._merchant_targets(self.conn, "Costco")}
        self.assertEqual(targets, self._displayed_under("Costco"))
