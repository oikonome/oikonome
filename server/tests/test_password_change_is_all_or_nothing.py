...
On an autocommit connection that refusal landed AFTER the password had
already changed. The caller was told 403 — while their password was in fact
new, their other sessions were gone, and their tokens, devices and pending
invites were still live: a rotation reported as a failure that had already
begun, leaving exactly the persistence the rotation exists to kill. The
whole rotation is one transaction now, so the answer is true either way.
"""
