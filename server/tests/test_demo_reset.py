...
class DemoDoorTests(unittest.TestCase):
    """OIKONOME_DEMO=1 → signup / forgot / reset are 404
    (cloaked): the tenant-level demoguard can't reach pre-auth paths."""
...
class ConsoleActionTests(unittest.TestCase):
    """New console actions: tenant-status flips + audits; sync-now
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            (Path(td) / "ops" / "host-health.json").write_text(json.dumps({
                ...
                "containers": [
                    ...
                    {"name": "oikonome-8042-postgres-1", "state": "exited",
                     "status": "Exited (0) 2 hours ago"},
                    {"name": "oikonome-8043-worker-1", "state": "exited",
            ...
            try:
                ...
                r = client.get("/admin/console")
                self.assertEqual(r.status_code, 200)
                # rollup: 8042 shows "1 up · 1 stopped" (app up, postgres
                # cleanly exited — NOT red); 8043's crashed worker (Exited 1)
                # is the only one listed individually in red
                self.assertIn("oikonome-8042", r.text)
                self.assertIn("1 stopped", r.text)
                self.assertNotIn("oikonome-8042-postgres-1</b>", r.text)
                self.assertNotIn("oikonome-8042-app-1</b>", r.text)
                self.assertIn("oikonome-8043-worker-1", r.text)
...
class AuthLockoutAndBillingEventTests(unittest.TestCase):
    """Account-lockout scoping, TOTP replay, billing event ordering, and
    ops-cmd directory permissions."""
    ...
    def test_ops_cmd_not_world_writable(self):
        """install.sh defaults ops-cmd closed, and the
        root watcher installer chowns it 0700 (not 1777)."""
        import pathlib
...
class SecretReencryptSweepTests(unittest.TestCase):
    """The boot sweep re-encrypts
    tenant secrets (items.access_token + config keys) left in plaintext
    from a pre-master-key era. Runs in a THROWAWAY DB so it can control
    the master-key env without disturbing the shared suite."""
...
class PasskeyLoginAndBillingScopeTests(unittest.TestCase):
    """Passkey-only accounts block password-only login on hosted, an empty
    TOTP code never feeds the account lock, and payment_failed is scoped to
    the subscription."""
