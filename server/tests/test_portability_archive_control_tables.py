...
`recipient_invites` is the one that had been missed. A portability request
is answered with an archive of the tenant's personal data, and the addresses
a household mails its balances to (plus who added them and when each said
yes) are exactly that — third-party personal data the household controls.
The list is hand-maintained because discovery cannot see a table with no
row-level security, and it went stale; `test_control_plane_export_coverage`
