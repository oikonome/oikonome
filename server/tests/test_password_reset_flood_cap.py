...
class PasswordResetFloodCapTests(unittest.TestCase):
    ...
    def test_password_reset_is_capped_per_address(self):
        """The route's only throttle was per-IP, so a distributed caller
        could mail an unbounded number of reset links into a chosen
        victim's inbox. The sibling login-unlock door is already capped per
