"""Guards written correctly once and not carried to the second place that needed them.

Grouped here rather than scattered because they share one theme: a guard that
was written correctly ONCE and then not carried to the second place that
needed it. Several are second occurrences of a bug this project already
fixed — the SQL/Python shadow set, the TOTP single-use burn, and
`config_txn` — which is the argument for testing the CONTRACT ("these two
...
class ShadowSetHealthAwareTests(unittest.TestCase):
    ...
    the SQL picked the lowest rank, full stop. Identical answers right up to
    the moment they matter — a live failover — when Reports, the forecast
    runway and the emailed report would sum the DEAD source while Today, the
    budget and bills summed the live one. An earlier fix unified the hidden
    half of this definition and left the health half; this is that repeat.
    """
...
class ConfigTxnLockTests(unittest.TestCase):
    """`config_txn` took a row lock on an AUTOCOMMIT connection.

    `SELECT ... FOR UPDATE` commits the instant it returns there, so the lock
    was released before the caller had even read the config — the helper
    added to close a lost-update window closed nothing.
    """
...
class BooksReviewQueueTests(unittest.TestCase):
    """The Schedule C review queue counted rows it could never clear.

    `pnl()` skips transfers and credit-card payments (double-count
    prevention); `unclassified_count` did not. So the "Needs attention" tile
    lit for rows where classifying changes nothing on the P&L and leaving
    them keeps the tile lit — an unclearable to-do.
    """
...
class ArchivedStatusTests(unittest.TestCase):
    ...
    `sync_all` (:00 hourly) and `nightly_all` fired at the same instant, and
    the reaper archives Items dead 30+ days. Interleaved, the sync's own
    error-status write landed on top of 'archived' and resurrected an Item
    that no longer exists at Plaid — after which it is picked up by every
    later sweep and can only keep failing.
    """
...
class OutOfRangeQueryParamTests(unittest.TestCase):
    ...
    Four endpoints passed a caller-supplied y/m/year straight into
    `dt.date(...)`, where out-of-range raises ValueError and FastAPI turns
    that into a 500. The three /lens endpoints had guarded it since the day
    they were written — but inline, three times over, so nothing carried the
    guard to the endpoints added later. The guard is shared now.
    """
    ...
    def test_retirement_survives_an_absurd_contribution(self):
        """Unbounded employer_mo overflowed the compounding loop to inf, and
        the NaN wall that produced is not JSON-serialisable."""
        big = 10 ** 30
        r = self.client.get(
...
class TotpBurnIsAtomicTests(unittest.TestCase):
    ...
    The code was made single-use by stamping `totp_last_counter` under a
    `WHERE last < new` guard — but returned True without ever checking
    whether the write landed. Two step-ups carrying the SAME code inside its
    ~90 s window both read the old counter, both matched, and both were
    accepted, while exactly one UPDATE took effect. The guard was doing its
    job and nobody was reading the answer, so a replayed code still opened
    the credential-changing doors it was added to protect.
    """
...
class SmtpStarttlsWritePathTests(unittest.TestCase):
    """`OIKONOME_SMTP_STARTTLS` went inert the moment SMTP was set in the UI.

    `resolve_smtp` read `cfg["smtp_starttls"]`, and no code path could ever
    write that key — so the UI path forced STARTTLS with no way back, and a
    plain LAN relay was reachable only by abandoning the UI for docker/.env.
    """
