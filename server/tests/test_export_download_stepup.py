...
The session cookie is SameSite=Lax, and a Lax cookie rides a top-level
cross-site GET. `/export` (the household ZIP, receipt images included) and
`/export/dump` (the whole database on a single-tenant self-host) were plain
GETs, so `<a href="https://<instance>/export">` on any page the signed-in
owner visited dropped their data into Downloads. The origin-check
middleware only wraps write methods, so nothing else stood in the way.

Now POST /api/export/token steps up (password, plus a live code on a TOTP
