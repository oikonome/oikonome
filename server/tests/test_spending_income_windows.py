...
class WindowedTabsTests(unittest.TestCase):
    ...
    def test_merchant_variants_differing_only_in_punctuation_fold(self):
        """Over a wide window a merchant renamed by its processor ("K-O-A"
        then "K O A") must rank as ONE merchant, under the bigger
        variant's name, with the previous window compared the same way."""
        _txn(self.conn, self.bank, dt.date(2026, 8, 1), 300.0, "K-O-A",
             category_primary="FOOD_AND_DRINK")
        _txn(self.conn, self.bank, dt.date(2026, 7, 1), 200.0, "K O A",
             category_primary="FOOD_AND_DRINK")
        _txn(self.conn, self.bank, dt.date(2026, 4, 1), 50.0, "K O A",
             category_primary="FOOD_AND_DRINK")
        w = reporting.spending_window(self.conn, self.today, "3m")
        koa = [m for m in w["merchants"]
               if "k" in m[0].lower() and "o" in m[0].lower()]
        self.assertEqual(len(koa), 1)
        self.assertEqual(koa[0][0], "K-O-A")     # the bigger variant names it
        self.assertEqual(koa[0][1], 500.0)
        self.assertEqual(koa[0][2], 2)
        self.assertEqual(koa[0][3], 50.0)        # prev window, same fold
