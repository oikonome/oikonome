...
class RecurringManageApiTests(unittest.TestCase):
    ...
    def test_a_chip_no_merchant_answers_to_is_refused(self):
        ...
        try:
            mid = conn.execute(
                ...
                "('Zenith Fiber','manual') RETURNING id::text AS id"
            ).fetchone()["id"]
        finally:
            conn.close()
        r = self.client.post("/api/bills/save", json={
            "payee": "Chip Bill", "amount": 12, "cadence": "MONTHLY:1",
            "merchants": ["Zenith Fiber", "Nowhere Cable"]})
        self.assertEqual(r.status_code, 400)
    ...
    def test_history_covered_by_and_arbitrary_merchant(self):
        self.client.post("/api/bills/save", json={
            "payee": "Harbor Beans", "amount": 25, "cadence": "WEEKLY:1"})
        # a merchant string already covered by an existing bill's matcher
        h = self.client.get("/api/bills/history",
                            params={"payee": "Harbor Beans. Coffee"}).json()
        self.assertIsNone(h["bill"])
        self.assertEqual(h["covered_by"], "Harbor Beans")
...
class HistoryCanonicalGroupingTests(unittest.TestCase):
    ...
    def test_history_groups_by_canonical_merchant(self):
        ...
        try:
            ...
            clean = "ACME DRUG TESTING"
            trunc = "ACME DRUG TESTIN"
            raw = "ACME DRUG TESTING 1200 N. HAWTHORN AVE. SPRINGFIELD, IL, US"
            ...
            merchant_dedup.load_llm_map(conn, {trunc: "Acme Drug Testing"})

            # every entry point lands on the SAME 3 charges
            for payee in (clean, raw, "Acme Drug Testing"):
...
class HistoryCurrentMonthTests(unittest.TestCase):
    def test_monthly_totals_include_current_month(self):
        ...
        try:
            write_config(conn)
            today = dt.date(2026, 7, 15)
            add_txn(conn, dt.date(2026, 6, 29), 150.0, "VENTURE WELLNESS",
                    account="chk")
            add_txn(conn, dt.date(2026, 7, 8), 150.0, "VENTURE WELLNESS",
                    account="chk")
            hist = bills.merchant_history(conn, "VENTURE WELLNESS",
                                              today=today)
