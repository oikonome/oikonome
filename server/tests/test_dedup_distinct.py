...
aggregator-side ImportOverlapGuard) trusted a same-day exact-amount hit
with NO name check at all — so two different $4.50 purchases on the same
day (coffee at two different shops) collapsed into one. The rule they
were built for stays intact: opaque bank descriptors ("POS DEBIT 4417")
carry no substantive name tokens, and those still match on amount+date
alone. What changes: when BOTH sides carry real merchant tokens and they
conflict outright (no overlap, not even a squished-name substring), the
rows are two different purchases and both survive.
"""
