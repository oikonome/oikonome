...
A person's pin, a store's item match (Amazon/Costco), a bill's stamp on the
rows it matches, and a store's "Unmatched" placeholder all used to share one
untyped column, told apart by write order and by prefix: sacredness of the
person's answer was recovered by re-applying manual_categories LAST at the
end of every store match. That ordering held only while every writer
remembered it, and it let a bill's stamp overwrite an item category and a
store's "Unmatched" overwrite a bill's stamp before the re-apply put things
back.

The kind of an override is now a fact on the row (override_source), each
...
class OverrideRank(unittest.TestCase):
    ...
    def test_an_archive_without_kinds_is_classified_on_arrival(self):
        ...
        the evidence that used to imply it still does."""
        data.set_category(self.conn, self.amz, "ENTERTAINMENT")
        self.conn.execute(
