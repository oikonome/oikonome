...
CITIES = ["Northfield", "Millbrook", "Riverton"]


class TheProposerOffersWhatAPersonWouldMerge(unittest.TestCase):
    def test_a_terminal_truncation_family_is_one_merchant(self):
        c = [_cand("Juniper Natural Foods", ["Juniper's Natural Foods"], rows=75),
             _cand("Juniper Nat", ["Juniper's Nat"], rows=133),
             _cand("Juniper Natural", ["Juniper's Natural Fo"], rows=47),
             _cand("Juniper Natur", ["Juniper's Natur"], rows=10)]
        props = mm.propose(c, CITIES)
        # every short spelling is offered onto the full name (the survivor)
        self.assertIn(("Juniper Nat", "Juniper Natural Foods"), _pairs(props))
        self.assertIn(("Juniper Natural", "Juniper Natural Foods"), _pairs(props))
        self.assertIn(("Juniper Natur", "Juniper Natural Foods"), _pairs(props))
        why = next(p for p in props if p["from"]["name"] == "Juniper Nat")["signals"][0]
        self.assertIn("cut", why)

    def test_a_city_glued_on_is_the_same_name(self):
        c = [_cand("Juniper Natural Foods", rows=75),
             _cand("Juniper Natural Foods Northfieldin", rows=53)]
        props = mm.propose(c, CITIES)
        self.assertEqual(len(props), 1)
        self.assertIn("Northfield", props[0]["signals"][0])
...
class TheVetoesMakeTheKnownMistakesImpossible(unittest.TestCase):
    def test_a_brand_and_its_fuel_arm_are_never_offered(self):
        c = [_cand("Costco", entity="ent-c"), _cand("Costco Gas", parent=True)]
    ...
    def test_categories_that_disagree_are_two_businesses(self):
        c = [_cand("Boost Espresso", category="FOOD AND DRINK", rows=9),
             _cand("Boost Espresso Cafe", category="RENT AND UTILITIES", rows=9)]
        self.assertEqual(mm.propose(c, CITIES), [])
    ...
    def test_a_lone_short_word_is_not_a_cut_of_a_run_together_string(self):
        c = [_cand("Riverton", rows=30), _cand("Rivertondmv", rows=4),
             _cand("Rivertonampd", rows=4)]
        self.assertEqual(mm.propose(c, CITIES), [])
    ...
    def test_a_junk_or_city_tail_never_survives_the_name_most_rows_wear(self):
        c = [_cand("Mcdonald", rows=191, descriptors=["MCDONALDS M5900 QRT17"]),
             _cand("Mcdonald Qrt Millbroo", rows=1, descriptors=["MCDONALDS M5900 QRT17"],
                   stems=["mcdonalds m"]),
             _cand("Greenleaf", rows=88), _cand("Greenleaf Northfield", rows=1),
             _cand("2crust Bakery", rows=10), _cand("Bakery", rows=1)]
        c[0]["stems"] = {"mcdonalds m"}
        pairs = _pairs(mm.propose(c, CITIES))
        self.assertIn(("Mcdonald Qrt Millbroo", "Mcdonald"), pairs)
        self.assertIn(("Greenleaf Northfield", "Greenleaf"), pairs)
        self.assertIn(("Bakery", "2crust Bakery"), pairs)
        # the fuller name still wins when it carries its share of the rows
        c = [_cand("Juniper Nat", rows=133), _cand("Juniper Natural Foods", rows=75)]
        self.assertEqual(_pairs(mm.propose(c, CITIES)),
                         {("Juniper Nat", "Juniper Natural Foods")})

    def test_the_same_trade_in_the_same_town_is_not_the_same_business(self):
        # two MCC-8398 places both led by the town's name: the carousel is
        # not the community centre
        c = [_cand("Northfield Caro", rows=4, mcc="8398", city="Northfield"),
             _cand("Northfield Rec Center", rows=13, mcc="8398", city="Northfield"),
             _cand("Northfield Carouse", rows=1, mcc="8398", city="Northfield")]
        pairs = _pairs(mm.propose(c, CITIES))
        self.assertNotIn(("Northfield Caro", "Northfield Rec Center"), pairs)
        self.assertNotIn(("Northfield Carouse", "Northfield Rec Center"), pairs)
        # the carousel's own cut spelling still joins it
        self.assertTrue({("Northfield Caro", "Northfield Carouse"),
                         ("Northfield Carouse", "Northfield Caro")} & pairs)
        # and the same trade in one town with a shared name word still counts
        c = [_cand("Riverton Pines Health", rows=6, mcc="8011", city="Riverton"),
             _cand("Riverton Pines Hlth Ctr", rows=3, mcc="8011", city="Riverton")]
        self.assertEqual(len(mm.propose(c, ["Riverton"])), 1)
    ...
    def test_a_name_that_is_a_word_and_a_code_is_not_the_bare_word(self):
        c = [_cand("Vine:30", rows=8, category="FOOD AND DRINK"),
             _cand("Vine Kitchen", rows=2, category="FOOD AND DRINK"),
             _cand("7-Eleven", rows=3), _cand("Eleven Restaurant", rows=3)]
        self.assertEqual(mm.propose(c, CITIES), [])
        # it still meets a spelling of itself
        c = [_cand("Vine:30", rows=8), _cand("Vine 30 New York", rows=1)]
        self.assertEqual(len(mm.propose(c, ["New York"])), 1)

    def test_kind_words_alone_do_not_make_a_chain(self):
        c = [_cand("Ferdy Bar Grill", rows=2), _cand("Ferdy Bar Restaurant", rows=1)]
        props = mm.propose(c, CITIES)
        self.assertEqual(len(props), 1)
        self.assertNotIn("rename_to", props[0])
        self.assertIn(props[0]["into"]["name"], ("Ferdy Bar Grill", "Ferdy Bar Restaurant"))

    def test_a_clean_hand_over_in_time_is_cited(self):
        d = dt.date
        a = _cand("Riverton Nat", rows=20)
        a.update(first=d(2021, 1, 4), last=d(2022, 6, 30))
        b = _cand("Riverton Natural Foods", rows=20)
    ...
    def test_a_person_who_named_one_has_spoken(self):
        c = [_cand("Juniper Nat", manual=True), _cand("Juniper Natural Foods")]
        self.assertEqual(mm.propose(c, CITIES), [])
...
class OffersAreDecidedByThePerson(unittest.TestCase):
    def setUp(self):
        ...
        self.ids = {}
        for name, raw, n in (("Riverton Natural Foods", "RIVERTON NATURAL FOODS", 5),
                             ("Riverton Nat", "RIVERTON NAT", 8),
                             ("Riverton Natur", "RIVERTON NATUR", 3)):
    ...
    def test_the_nightly_offers_and_a_rerun_offers_nothing_new(self):
        ...
        self.assertEqual({o["from"] for o in offers}, {"Riverton Nat", "Riverton Natur"})
        self.assertTrue(all(o["into"] == "Riverton Natural Foods" for o in offers))
    ...
    def test_approving_merges_through_the_journal_and_is_undoable(self):
        ...
        mm.run(self.conn)
        o = next(o for o in mm.pending(self.conn) if o["from"] == "Riverton Nat")
        r = mm.decide(self.conn, o["id"], "approve")
        self.assertEqual(r.get("into"), "Riverton Natural Foods")
        self.assertEqual(self.conn.execute(
            "SELECT count(*) AS n FROM transactions t JOIN merchants m ON m.id=t.merchant_id "
            "WHERE m.name='Riverton Natural Foods' AND t.removed=0").fetchone()["n"], 13)
        self.assertNotIn("Riverton Nat", {o["from"] for o in mm.pending(self.conn)})
        # the merge is a journal entry the Merchants page can undo
        change = self.conn.execute(
            "SELECT id FROM merchant_renames WHERE raw_merchant='Riverton Nat' "
            "ORDER BY id DESC LIMIT 1").fetchone()
        self.assertIsNotNone(change)
        merchant_dedup.undo(self.conn, change["id"])
        self.assertEqual(self.conn.execute(
            "SELECT count(*) AS n FROM transactions t JOIN merchants m ON m.id=t.merchant_id "
            "WHERE m.name='Riverton Natural Foods' AND t.removed=0").fetchone()["n"], 5)

    def test_a_rejected_pair_is_never_offered_again(self):
        mm.run(self.conn)
        o = next(o for o in mm.pending(self.conn) if o["from"] == "Riverton Natur")
        mm.decide(self.conn, o["id"], "reject")
        self.assertEqual(mm.run(self.conn)["inserted"], 0)
        self.assertNotIn("Riverton Natur", {o["from"] for o in mm.pending(self.conn)})
    ...
    def test_only_the_merchants_a_sync_minted_are_offered_on_ingest(self):
        st = mm.run(self.conn, only_ids=[self.ids["Riverton Natur"]], limit=10)
        self.assertEqual(st["inserted"], 1)
        self.assertEqual([o["from"] for o in mm.pending(self.conn)], ["Riverton Natur"])
    ...
    def test_approving_a_chain_offer_puts_every_outlet_under_the_brand(self):
        ...
        self.conn.execute(
            ...
            (self.ids["Riverton Natur"], into, jsonb({"rename_to": "Petrox"})))
    ...
    def test_the_alert_counts_only_offers_the_person_can_still_act_on(self):
        ...
        merchant_dedup.rename(self.conn, "Riverton Nat", "Riverton Natural Foods")
    ...
    def test_the_nightly_retires_offers_whose_merchants_are_no_longer_two(self):
        ...
        self.assertEqual(self.conn.execute(
            ...
            "WHERE from_merchant_id=%s", (self.ids["Riverton Nat"],)
    ...
    def test_the_rehearsal_scores_the_proposer_against_the_ledgers_own_aliases(self):
        ...
        from oikonome.engine import merchant_dedup
        merchant_dedup.rename(self.conn, "Riverton Nat", "Riverton Natural Foods")
        merchant_dedup.rename(self.conn, "Riverton Natur", "Riverton Natural Foods")
