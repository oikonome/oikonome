...
It was the last caller of the To:sender+Bcc:everyone shape — a
per-address delivery failure was invisible, the address never recorded
as failing, no footer — and it stamped the in-app alert with the
server's date rather than the household's, so the Alerts page and the
Today page could name different days for the same event.
"""
