"""Outbound mail must fail LOUDLY. A dead relay used to leave the console
saying "invite emailed" while every transactional send died in a daemon
thread — the instance-wide health record, the half-hourly probe, and the
three places that read them (console banner, mint door, Doctor) are what
turned that into something an operator can see."""
import os
import smtplib
import unittest
import uuid
...
class ConsoleDoorTests(unittest.TestCase):
    ...
    is — the invite used to read "emailed" over a dead relay."""
