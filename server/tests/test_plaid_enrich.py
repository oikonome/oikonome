    ...
    def handler(request):
        if request.url.path == "/transactions/enrich":
            ...
            for t in body["transactions"]:
                if "NORTHWIND CLUB" in t["description"].upper():
                    out.append({"id": t["id"], "enrichments": {
                        "merchant_name": "Northwind Club", "website": "northwindclub.example",
                        "logo_url": "https://plaid-merchant-logos.plaid.com/northwind_club_999.png",
                        "entity_id": "ENT-NWCLUB", "payment_channel": "in store",
...
class EnrichTests(unittest.TestCase):
    def setUp(self):
        ...
        write_config(self.conn, plaid_client_id="cid", plaid_secret="sec",
                     plaid_env="sandbox")
        self.a = add_txn(self.conn, TODAY - dt.timedelta(days=1), 90.00,
                         "NORTHWIND CLUB #0999", account="chk", primary="GENERAL_MERCHANDISE")
    ...
    def test_enrich_merges_plaid_shape_and_resolves_the_merchant(self):
        ...
        self.assertEqual(r["raw"]["merchant_entity_id"], "ENT-NWCLUB")
        ...
        self.assertEqual((r["merchant"], r["plaid_entity_id"]), ("Northwind Club", "ENT-NWCLUB"))
        self.assertIn("northwind_club_999.png", r["logo_url"])
