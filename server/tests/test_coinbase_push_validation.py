...
script, so `amount` can arrive as null, an empty string, or a word. It must
be rejected as a ValueError (which the route answers as "bad payload")
before it reaches the NUMERIC NOT NULL column; otherwise the database raises
a psycopg error and the request fails with a 500. And because the connection
is autocommit, the rows must be validated before any is written; otherwise
the rows ahead of a bad one commit and the account holds half a restatement
while the caller is told the push failed.
"""
