...
current_user()'s Bearer branch returned as soon as the token resolved —
before every lockout the cookie path enforces. So a suspended, pending-delete
or add-on-frozen tenant kept a fully working WRITE path through the collector
doors indefinitely, and could not even revoke the token, because the UI that
revokes it was locked out too.
"""
import os
import unittest
import uuid
