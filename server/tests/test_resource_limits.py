...
class RestoreBatchingTests(unittest.TestCase):
    ...
    it used to cost one client/server round trip per row. Batching only helps
    if the per-row `cur.rowcount` accounting it replaced stays correct — that
    is the part worth a test, not the speed.
    """
...
class DualStoreFallbackTests(unittest.TestCase):
    """One helper for "Redis, else in-process", because eleven hand-rolled
    copies is eleven chances to get the fallback backwards — and getting it
    backwards means a limit silently stops applying exactly when Redis is
    struggling."""
