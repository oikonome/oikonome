...
class MerchantPageScopeIsTheMerchant(unittest.TestCase):
    ...
    the merchant's name. A substring scope would pull unrelated descriptors
    that share a word — "Heliosoft Systems", "Borealis Helio Group" — into
    the write, and payroll beside them must never move."""
    ...
    def setUp(self):
        self.conn = make_db()
        write_config(self.conn)
        add_txn(self.conn, TODAY, 59.0, "HELIO.COM 555-0123 SPRINGFIELD",
                merchant="Helio", primary="GENERAL_SERVICES")
        add_txn(self.conn, TODAY, 39.0, "HELIO.COM", merchant="Helio",
                primary="GENERAL_SERVICES")
        add_txn(self.conn, TODAY, -5000.00, "ACME PAYROLL",
                merchant="Acme Widgets", primary="INCOME")
        add_txn(self.conn, TODAY, 120.0, "HELIOSOFT SYSTEMS DIRECT PAY",
                merchant="HELIOSOFT SYSTEMS DIRECT PAY", primary="GENERAL_SERVICES")
        add_txn(self.conn, TODAY, 80.0, "BOREALIS HELIO GROUP",
                merchant="Borealis Helio Group", primary="MEDICAL")

    def tearDown(self):
        self.conn.close()

    def test_the_family_is_the_merchants_own_strings(self):
        self.assertEqual(data._token_family(self.conn, "Helio"), ["Helio"])

    def test_preview_counts_only_the_merchant(self):
        p = data.merchant_category_preview(self.conn, "Helio", "EDUCATION")
        self.assertEqual(p["count"], 2)
        self.assertEqual(set(p.get("variants") or []), {"Helio"})
        self.assertEqual(p.get("flow_rows", 0), 0)

    def test_the_write_moves_only_the_merchant(self):
        res = data.set_merchant_category(self.conn, "Helio", "EDUCATION")
        self.assertEqual(res["count"], 2)
        rows = {r["name"]: r["category_primary"] for r in self.conn.execute(
            "SELECT name, category_primary FROM transactions").fetchall()}
        self.assertEqual(rows["HELIO.COM"], "EDUCATION")
        self.assertEqual(rows["HELIO.COM 555-0123 SPRINGFIELD"], "EDUCATION")
        self.assertEqual(rows["ACME PAYROLL"], "INCOME")
        self.assertEqual(rows["HELIOSOFT SYSTEMS DIRECT PAY"], "GENERAL_SERVICES")
        self.assertEqual(rows["BOREALIS HELIO GROUP"], "MEDICAL")
...
class MerchantFamilyMatchesThePagesOwnPredicate(unittest.TestCase):
    ...
    own match covers — a phrase merchant refuses the lookalike ('Zenith
    Mobile' is not Skyway Mobile), a rebrand's alternatives cover both names,
    ...
    def test_a_phrase_merchant_refuses_the_lookalike(self):
        add_bill(self.conn, "Zenith Mobile", 35.0, merchant="zenith mobile")
        add_txn(self.conn, TODAY, 35.0, "ZENITH MOBILE 844-555-0123",
                merchant="ZENITH MOBILE PMT")
        add_txn(self.conn, TODAY, 90.0, "SKYWAY MOBILE #4432", merchant="Skyway Mobile")
        add_txn(self.conn, TODAY, 12.0, "PLUS MOBILE", merchant="Plus Mobile")
        self.assertEqual(self._family("Zenith Mobile"), {"ZENITH MOBILE PMT"})
    ...
    def test_the_family_agrees_with_the_page_matcher_row_by_row(self):
        ...
        add_bill(self.conn, "Zenith Mobile", 35.0, merchant="zenith mobile")
        for name, merch in [("ZENITH MOBILE 844-555-0123", "ZENITH MOBILE PMT"),
                            ("SKYWAY MOBILE #4432", "Skyway Mobile"),
                            ("ZENITH.COM", "ZENITH.COM"),
                            ("ZENITH MOBILE STORE", "ZENITH MOBILE STORE"),
                            ("PLUS MOBILE", "Plus Mobile")]:
            add_txn(self.conn, TODAY, 35.0, name, merchant=merch)
        match = budget.merchant_matcher("zenith mobile",
                                        budget._key_token("Zenith Mobile"))
        ...
        self.assertEqual(self._family("Zenith Mobile"), expect)
        self.assertIn("ZENITH MOBILE PMT", expect)
