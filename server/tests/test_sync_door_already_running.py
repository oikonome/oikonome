...
`sync_tenant` answers the sentinel {"_status": "already-running"} when the
per-tenant single-flight lock is held. The /api/jobs/sync door checked every
value for an "error" prefix, found none, and returned ok=True with an empty
result set — indistinguishable from "you have no connections". The SPA then
told the user to go connect a bank, while that bank's data was being pulled.
"""
...
class ManualSyncCategorizes(unittest.TestCase):
    ...
    and those both run the sync-time categorize pass after the pull. This
    one did not, so rows a person had just pulled sat under the
    aggregator's generic category — variable spend, no bill match — until
    the next hourly run."""
