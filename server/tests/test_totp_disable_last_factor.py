...
Like passkey_delete, totp_disable refuses to remove the LAST strong factor
while hosted forced-2FA is in effect; otherwise a captured password +
session + one live code could permanently downgrade a TOTP-only account to
password-only login. The refusal is: blocked when TOTP is the only factor, allowed when a
