    ...
    def test_settlement_swap_keeps_user_state(self):
        ...
        pending_page = {"added": [
            {"transaction_id": "pt-pend", "account_id": "p-chk",
             ...
             "name": "CORNER BAKERY", "pending": True,
             "personal_finance_category": {"primary": "FOOD_AND_DRINK"}}],
            "modified": [], "removed": [],
        ...
        self.conn.execute(
            "UPDATE transactions SET category_override='GROCERIES', "
            "owner_override='member' WHERE id='pt-pend'")
        self.conn.execute(
            "INSERT INTO transaction_notes (txn_id, note) "
            "VALUES ('pt-pend', 'split with a friend')")
        settle_page = {"added": [
            {"transaction_id": "pt-post", "account_id": "p-chk",
             "date": "2026-08-02", "amount": 40.00,
             "name": "CORNER BAKERY", "pending": False,
        ...
        self.assertEqual(row["category_override"], "GROCERIES")
        self.assertEqual(row["owner_override"], "member")
        ...
        self.assertEqual(note["note"], "split with a friend")
...
class HostedLinkSessionStoreTests(unittest.TestCase):
    ...
    `_PLAID_LINK` caps entries per tenant and ages abandoned sessions out, so
    one household starting link sessions can never push out a different
    household's in-flight one — whose link page would otherwise redirect to
    /accounts with nothing said.
    """
