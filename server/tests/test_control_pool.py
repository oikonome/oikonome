...
The concurrency knee under load is the per-request unpooled TLS connect in
web/app._control_conn fighting a managed Postgres connection cap. These
tests pin the shape that avoids it: control-plane queries come from a
...
class ControlPoolTests(unittest.TestCase):
    ...
    def test_control_conn_is_pooled_and_reused(self):
        """Sequential checkouts reuse pooled connections — the per-request
        connect (the load-test knee) is gone. Asserted via the pool's own
