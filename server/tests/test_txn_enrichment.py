...
CHECK = {
    ...
    "amount": 850.00, "name": "Check Paid #1052", "merchant_name": None,
    "pending": False, "payment_channel": "other", "check_number": "1052",
...
SPOTIFY = {
    ...
    "amount": 12.99, "name": "PAYPAL *SPOTIFY", "merchant_name": "Spotify",
    "pending": False, "payment_channel": "online",
    "counterparties": [
        {"name": "PayPal", "type": "payment_app", "entity_id": "ENT-PP",
         "confidence_level": "VERY_HIGH"},
        {"name": "Spotify", "type": "merchant", "entity_id": "ENT-SP",
         "confidence_level": "VERY_HIGH"}],
...
PAGE = {"added": [STORAGE, CHECK, SPOTIFY], "modified": [], "removed": [],
...
class EnrichmentFromRawTests(unittest.TestCase):
    def test_plaid_shape(self):
        ...
        self.assertEqual(base.enrichment_from_raw(SPOTIFY)["payment_processor"], "PayPal")
        self.assertEqual(base.enrichment_from_raw(CHECK)["check_number"], "1052")
...
class IngestTests(unittest.TestCase):
    ...
    def test_columns_land_on_ingest(self):
        ...
        self.assertEqual((r["mcc"], r["payment_channel"], r["location_city"],
                         ...
                         ("4225", "in store", "Springfield", "1057", "2026-08-15"))
        self.assertEqual(self._row("en2")["check_number"], "1052")
    ...
    def test_fuel_rows_the_mcc_pass_overwrote_are_put_back(self):
        """Before the pass learned to skip fuel it had already flipped pump
        rows to the warehouse's grocery code; the next pass restores the
        ingest's stamp — the row is an outlet, and the receipt says pump."""
        self.conn.execute(
    ...
    def test_search_reaches_check_number_city_and_processor(self):
        self.assertEqual([r["id"] for r in data.search_transactions(self.conn, "1052")[0]],
                         ["en2"])
