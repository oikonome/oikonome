...
The page answers "what do I have, and is it all syncing?" and stated neither.
Add-a-connection appeared twice — a toggle at the top and the embedded import
section at the foot. And a broken connection was a red dot whose meaning lived
in a hover title, which a phone never shows.
"""
...
class AccountsShapeTests(unittest.TestCase):
    ...
    def test_a_broken_connection_names_its_failure_in_words(self):
        """…and names the RIGHT failure. "sync failing" used to cover
        both a dead login and a bank having a bad afternoon, and both
        got a re-auth button — so the fixable case and the
        nothing-to-do case were indistinguishable. The words now split
        with the server's classification."""
