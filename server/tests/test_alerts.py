...
class AlertTests(unittest.TestCase):
    ...
    def test_build_orders_worst_first(self):
        st = {"anomalies": ["Possible double charge: X $20 ×2"],
              "recurring_drifts": [{"payee": "Northside Power", "old": 440, "new": 370}],
...
class AnomalyTests(unittest.TestCase):
    ...
    def test_double_charge_flagged(self):
        add_txn(self.conn, TODAY, 47.39, "Uber Eats")
        add_txn(self.conn, TODAY - dt.timedelta(days=1), 47.39, "Uber Eats")
        msgs = self.detect()
    ...
    def test_separate_orders_same_price_not_flagged(self):
        ...
        for ref in ("5L03K5C61", "537G66M70"):
            add_txn(self.conn, TODAY - dt.timedelta(days=1), 21.19,
                    f"AMAZON MKTPL*{ref}", merchant="Amazon")
        add_txn(self.conn, TODAY, 21.19, "AMAZON MKTPLACE PMTS",
                merchant="Amazon", pending=1)
        self.assertEqual(self.detect(), [])

    def test_pending_duplicate_waits_for_posting(self):
        add_txn(self.conn, TODAY, 47.39, "UBER EATS", pending=1)
        add_txn(self.conn, TODAY, 47.39, "UBER EATS", pending=1)
        self.assertEqual(self.detect(), [])
    ...
    def test_older_twin_does_not_hide_fresh_pair(self):
        ...
        add_txn(self.conn, TODAY - dt.timedelta(days=3), 47.39, "Uber Eats")
        add_txn(self.conn, TODAY - dt.timedelta(days=1), 47.39, "Uber Eats")
        add_txn(self.conn, TODAY, 47.39, "Uber Eats")
        msgs = self.detect()
    ...
    def test_stale_pair_not_renagged(self):
        add_txn(self.conn, TODAY - dt.timedelta(days=2), 47.39, "Uber Eats")
        add_txn(self.conn, TODAY - dt.timedelta(days=3), 47.39, "Uber Eats")
        self.assertEqual(self.detect(), [])
    ...
    def test_transfers_never_flagged(self):
        for _ in range(2):
            add_txn(self.conn, TODAY, 5000.0, "BROKER EDI PYMNTS",
