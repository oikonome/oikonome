...
/signup and /forgot. Nothing removed a key once made, so every scanner that
ever tripped a limit stayed resident for the life of the process: one more
...
class SecurityBucketsAreTheLastToBeForgotten(unittest.TestCase):
    """Which bucket gives way is a security question, and expiry order got
    the answer backwards.

    `dead_at` is now + window_s, so ranking candidates by it ranks them by
    ...
    doors). Ranked on expiry alone the lockout therefore went FIRST and
    /forgot's hourly counter outlived it, which is a sprayer's win
