...
rows are where the access tokens live. A shortfall (fewer released than
held, or the call blowing up) must count as a failed service: the
pending-erasure marker is written and names the Item ids that did not
release, which is exactly what an operator removes from the Plaid
dashboard. Otherwise a Plaid outage at delete-time reads as "released 0",
the wipe goes ahead, and the Items keep billing with nothing in the
database to find them by.
"""
...
class PlaidPartialReleaseTests(unittest.TestCase):
    ...
    def test_a_db_failure_during_release_propagates_instead_of_reading_as_zero(self):
        ...
        from 'the tenant held no Items', so the erasure would wipe the rows
        (and the only access tokens) with no failed-service marker while
        the Items keep billing."""
