...
class BillTxnCategoryTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            self.storage = add_txn(conn, d, 120.00, "U-HAUL STORAGE",
                                   account="chk", merchant="U-Haul",
                                   primary="HOME_IMPROVEMENT")
            self.storage_prev = add_txn(conn, d - dt.timedelta(days=30), 120.00,
                                        "U-HAUL STORAGE", account="chk",
                                        merchant="U-Haul", primary="HOME_IMPROVEMENT")
            self.truck = add_txn(conn, d, 500.00, "U-HAUL TRUCK RENTAL",
                                 account="chk", merchant="U-Haul",
                                 primary="TRANSPORTATION")
        finally:
            conn.close()
        r = self.client.post("/api/bills/save", json={
            "payee": "U-Haul", "amount": 120.00, "cadence": "MONTHLY:1",
    ...
    def test_archive_reverts_restore_restamps_delete_reverts(self):
        self.client.post("/api/bills/delete", json={"payee": "U-Haul",
                                                     "mode": "archive"})
        self.assertEqual(self._cat(self.storage), ("HOME_IMPROVEMENT", None, None))
        self.client.post("/api/bills/restore", json={"payee": "U-Haul"})
        self.assertEqual(self._cat(self.storage)[0], "RENT_AND_UTILITIES")
        self.client.post("/api/bills/delete", json={"payee": "U-Haul",
                                                     "mode": "archive"})
        self.client.post("/api/bills/delete", json={"payee": "U-Haul",
    ...
    def test_new_charge_is_stamped_by_the_nightly_pass(self):
        conn = tenancy.tenant_connect(self.tid)
        try:
            new = add_txn(conn, TODAY, 120.00, "U-HAUL STORAGE", account="chk",
                          merchant="U-Haul", primary="HOME_IMPROVEMENT")
    ...
    def test_reset_sticks_when_the_source_has_no_category_of_its_own(self):
        ...
        conn = tenancy.tenant_connect(self.tid)
        try:
            bare = add_txn(conn, TODAY, 120.00, "U-HAUL STORAGE",
                           account="chk", merchant="U-Haul", primary=None)
            bills.apply_txn_categories(conn)
        finally:
            conn.close()
        ...
        try:
            ...
            n = conn.execute(
                """INSERT INTO manual_categories (transaction_id, category, set_at, bill_id)
                   VALUES (%s,'RENT_AND_UTILITIES',now(),'rec:u-haul')
    ...
    def test_two_bills_overlapping_the_closer_amount_wins(self):
        conn = tenancy.tenant_connect(self.tid)
        try:
            # a second U-Haul bill at $100 with its own category; the $120.00
            # charge is inside both tolerances ($30) — closer wins, every run
            bills.save_bill(conn, payee="U-Haul Locker", amount=100,
                            frequency="MONTHLY", interval=1,
                            next_due=(TODAY + dt.timedelta(days=5)).isoformat(),
                            merchant="u-haul", txn_category="GENERAL_SERVICES")
            for _ in range(3):
                bills.apply_txn_categories(conn)
                self.assertEqual(self._cat(self.storage)[0], "RENT_AND_UTILITIES")
            odd = add_txn(conn, TODAY, 101.0, "U-HAUL STORAGE", account="chk",
                          merchant="U-Haul", primary="HOME_IMPROVEMENT")
            bills.apply_txn_categories(conn)
        finally:
            conn.close()
    ...
    def test_business_rows_are_never_stamped(self):
        ...
        try:
            biz = add_txn(conn, TODAY, 120.00, "U-HAUL STORAGE", account="bizchk",
                          merchant="U-Haul", primary="HOME_IMPROVEMENT")
            bills.apply_txn_categories(conn)
        finally:
            conn.close()
    ...
    def test_rename_keeps_the_marks(self):
        self.client.post("/api/bills/save", json={
            "payee": "U-Haul Storage", "orig_payee": "U-Haul",
            "amount": 120.00, "cadence": "MONTHLY:1",
            "next_due": (TODAY + dt.timedelta(days=20)).isoformat()})
        self.assertEqual(self._cat(self.storage)[2], "rec:u-haul-storage")
        self.assertEqual(self._cat(self.storage)[0], "RENT_AND_UTILITIES")
