...
Enrolment — adding a second factor requires the password, not only a live
session. Otherwise a stolen session could enroll its OWN authenticator (or
passkey), and since enroll revokes every other session, theft would escalate
into locking the real owner out of their own money, using the 2FA machinery
as the weapon. The password is the thing a session thief does not have.
