"""infra-stress sampler + admin-console panel.
...
class BreachTests(unittest.TestCase):
    def test_pg_conn_thresholds(self):
        # rows without the server's cap classify against the 25-cap constants
    ...
    def test_pg_conns_are_scored_against_the_servers_own_cap(self):
        """16 connections is two-thirds of a managed 25-cap and a sixth of
        a container's 100: the same count must not breach on the second.
    ...
    def test_p95_needs_a_crowd_before_it_counts_as_pressure(self):
        ...
        must. A row that never recorded a count keeps the old behaviour."""
...
class PanelTests(unittest.TestCase):
    ...
    def test_summary_and_detail_markup_present(self):
        # The fragment carries BOTH the one-line summary (kept for the
        # standalone ?summary=1 endpoint) and the full detail. The panel is
        # always-expanded — no collapse toggle — so the
        # console shows the detail; CSS hides the sumline there.
        _insert(60, pg_total_conns=6, mem_used_gb=1.9, mem_total_gb=3.8,
                disk_used_pct=41, host_load1=0.4, api_p95_ms=36.0)
        r = self.client.get("/admin/console/infra")
        self.assertEqual(r.status_code, 200)
        # no collapse control remains…
        self.assertNotIn("data-infra-toggle", r.text)
        # …but the one-line summary markup is still rendered (summary endpoint)
    ...
    def test_panel_always_expanded_client_wiring(self):
        # the infra panel is always expanded — the client
        # forces it open and wires NO collapse toggle.
    ...
    def test_caption_removed_from_full_page(self):
        _insert(60, pg_total_conns=6)
        r = self.client.get("/admin/console")
        self.assertEqual(r.status_code, 200)
        self.assertIn("Infra stress", r.text)           # heading stays
        # the old caption text is gone entirely (behavior kept, label dropped)
