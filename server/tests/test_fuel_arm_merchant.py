...
Northwind Club's pump and Northwind Club's warehouse are one payee to an aggregator and two
...
them apart ("NORTHWIND CLUB GAS #0123" vs "NORTHWIND CLUB WHSE #0123"), so the split is read
from the text.
...
class FuelArmParser(unittest.TestCase):
    def test_the_descriptor_names_the_brand_then_the_pump(self):
        self.assertEqual(fuel_arm("Northwind Club", "NORTHWIND CLUB GAS #0123"), "Northwind Club Gas")
        self.assertEqual(fuel_arm("Kroger", "KROGER FUEL CTR #2020"),
                         "Kroger Fuel")

    def test_the_store_and_the_website_are_not_the_pump(self):
        self.assertIsNone(fuel_arm("Northwind Club", "NORTHWIND CLUB WHSE #0123"))
        self.assertIsNone(fuel_arm("Northwind Club", "WWW NORTHWIND CLUB COM"))

    def test_a_merchant_already_named_for_fuel_is_left_alone(self):
        self.assertIsNone(fuel_arm("Northwind Club Gas", "NORTHWIND CLUB GAS #0147"))
    ...
    def test_punctuation_in_the_brand_cannot_hide_the_pump(self):
        ...
        self.assertEqual(fuel_arm("A-B-C", "A-B-C FUEL #0555"),
                         "A-B-C Fuel")
        self.assertEqual(fuel_arm("A-B-C", "A B C FUEL TX"),
                         "A-B-C Fuel")
        self.assertEqual(fuel_arm("Lowe's", "LOWES FUEL CENTER"), "Lowe's Fuel")
        # and the store is still the store
        self.assertIsNone(fuel_arm("A-B-C", "A-B-C #0123 SPRINGFIELD"))
...
class FuelArmOnIngest(unittest.TestCase):
    ...
    def test_the_pump_and_the_warehouse_become_two_merchants(self):
        ...
        self.assertEqual(self._sync("f1", "Northwind Club", "NORTHWIND CLUB GAS #0123"),
                         ("Northwind Club Gas", "Northwind Club"))
        self.assertEqual(self._sync("f2", "Northwind Club", "NORTHWIND CLUB WHSE #0123"),
                         (None, "Northwind Club"))

    def test_a_resync_re_derives_the_same_outlet(self):
        # pending → posted runs the same row through again; a rule that
        # appended each time would produce "Northwind Club Gas Gas"
        self.assertEqual(self._sync("f3", "Northwind Club", "NORTHWIND CLUB GAS #0123"),
                         ("Northwind Club Gas", "Northwind Club"))
        self.assertEqual(self._sync("f3", "Northwind Club", "NORTHWIND CLUB GAS #0123"),
                         ("Northwind Club Gas", "Northwind Club"))
...
class FuelArmRefilesOnIngest(unittest.TestCase):
    ...
    def test_a_proven_pump_ingests_as_fuel_with_its_outlet(self):
        # Plaid files A-B-C's pump under groceries; the statement line is
        # the evidence that refiles it
        self._sync("g1", "A-B-C", "A-B-C FUEL #0555",
    ...
    def test_the_user_s_category_survives_a_resync(self):
        ...
        self._sync("g2", "Northwind Club", "NORTHWIND CLUB GAS #0123",
                   category="FOOD_AND_DRINK")
        ...
        self.assertEqual(self._row("g2"),
                         ("TRANSPORTATION", "TRANSPORTATION_GAS",
                          "ENTERTAINMENT", "Northwind Club Gas"))
    ...
    def test_a_store_row_from_the_same_brand_keeps_its_category(self):
        ...
        self._sync("g3", "Northwind Club", "NORTHWIND CLUB WHSE #0123",
...
class FuelArmStoredHistory(unittest.TestCase):
    ...
    def test_stored_pump_rows_get_an_outlet_and_nothing_else_moves(self):
        self._row("h1", "Northwind Club", "NORTHWIND CLUB GAS #0123")
        self._row("h2", "Northwind Club", "NORTHWIND CLUB WHSE #0123")
        self._row("h3", "Northwind Club", None)
        # already named for its own pump
        self._row("h4", "Northwind Club Gas", "NORTHWIND CLUB GAS #0147")
        ...
        self._row("h8", "A-B-C", "A-B-C FUEL #0555")
        self._migrate()
        self.assertEqual(self._outlet("h1"), ("Northwind Club Gas", "Northwind Club"))
        self.assertEqual(self._outlet("h2"), (None, "Northwind Club"))
        self.assertEqual(self._outlet("h3"), (None, "Northwind Club"))
        # already named for its own pump: nothing to add
        self.assertEqual(self._outlet("h4"), (None, "Northwind Club Gas"))
        self.assertEqual(self._outlet("h5"), (None, "Kroger"))
        self.assertEqual(self._outlet("h8"), ("A-B-C Fuel", "A-B-C"))

    def test_running_it_twice_changes_nothing_the_second_time(self):
        self._row("h6", "Northwind Club", "NORTHWIND CLUB GAS #0123")
        self._migrate()
        self._migrate()
        self.assertEqual(self._outlet("h6"), ("Northwind Club Gas", "Northwind Club"))
    ...
    def test_a_proven_pump_is_refiled_as_fuel(self):
        ...
        self._row("h9", "A-B-C", "A-B-C FUEL #0555",
                  category="FOOD_AND_DRINK")
        self._migrate()
