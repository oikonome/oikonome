...
class YearEndPackageTests(unittest.TestCase):
    ...
    def test_download_filename_survives_a_non_latin_entity_name(self):
        """`_filename_safe` used the Unicode-aware
        str.isalnum(), so Greek/Cyrillic/CJK letters passed straight through
        and still raised the latin-1 UnicodeEncodeError the function exists to
        prevent. A business named in a non-Latin script is entirely ordinary."""
