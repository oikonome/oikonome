...
class LinkTests(unittest.TestCase):
    ...
    def test_stale_collector_source_fails_over_to_plaid_balance(self):
        # A workplace plan fed by a file collector (a plan-CSV item) and a
        # balance-only aggregator link: the file is the richer primary, but
        # once it stops arriving the aggregator balance must take over
        # instead of the group freezing on a stale figure.
        _mk_source(self.conn, "plan", "plan_csv", "plan-work",
                   mask=None, typ="investment", bal=40000.0)
        _mk_source(self.conn, "pl-2", "plaid", "pl-work", mask="2468",
                   typ="investment", bal=40500.0)
        links.create(self.conn, ["plan-work", "pl-work"])
        self.assertEqual(links.shadow_ids(self.conn), ["pl-work"])
        self.conn.execute(
            "UPDATE accounts SET updated_at = now() - interval '3 days' "
            "WHERE id='plan-work'")
        self.assertEqual(links.shadow_ids(self.conn), ["plan-work"])
    ...
    def test_default_order_prefers_the_source_that_delivered_data(self):
        # balance-only aggregator link vs the collector that has the ledger:
        # the ledger goes first even though Plaid outranks it as an
        # aggregator; a linked pair with data on both sides keeps the
        # aggregator order
        _mk_source(self.conn, "plan", "plan_csv", "plan-work",
                   mask="2468", typ="investment", bal=40000.0)
        _mk_source(self.conn, "pl-2", "plaid", "pl-work", mask="2468",
                   typ="investment", bal=40500.0)
        self.conn.execute(
            """INSERT INTO holdings (account_id, symbol, quantity, price, value)
               VALUES ('plan-work', 'FXAIX', 10, 100, 1000)""")
        self.assertEqual(
            links.default_order(self.conn, ["pl-work", "plan-work"]),
            ["plan-work", "pl-work"])
...
class SimplefinOrgSplitTests(unittest.TestCase):
    ...
    def test_orgs_become_items_and_types_guessed(self):
        ...
        payload = {"accounts": [
            {"id": "L1", "name": "NORTHWIND 401K PLAN",
             "balance": "40000", "currency": "USD",
