...
A row is a logo, a name and one line of facts; the rest opens beside the
list. Merge picks an EXISTING merchant from a search and Rename is its own
action, because one control that renamed OR merged depending on what was
typed would fork a merchant on a case or whitespace slip. The offers are
a queue with the evidence behind a click. Both clients.
"""
