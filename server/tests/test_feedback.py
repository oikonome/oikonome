...
class FeedbackTests(unittest.TestCase):
    ...
    def test_smtp_emails_with_attachment(self):
        ...
        # addressed to the inbox alone; a Bcc shape would also deliver the
        # bundle to the sending mailbox
...
class FeedbackLogGateTests(unittest.TestCase):
    """The log ring ships only on a true single-tenant instance.

    A self-hosted instance with separate logins is multi-tenant with
    OIKONOME_HOSTED unset, so the gate counts tenants rather than reading
    the flag, and fails closed: a support bundle missing logs is an
    inconvenience, one carrying another tenant's log lines is a disclosure.
    """
    ...
    def test_gate_counts_real_tenants(self):
        """Self-host, flag unset, two tenants: logs must be withheld."""
        import inspect
    ...
    def test_a_household_in_its_deletion_grace_still_counts(self):
        """One live household plus one pending deletion is two tenants:
        the frozen household's log lines are still in the ring."""
        from unittest import mock

        from oikonome.web import feedback
        rows = [{"status": "active"}, {"status": "pending_delete"}]

        class _Admin:
            def execute(self, sql, *a, **k):
                live = [r for r in rows
                        if "pending_delete" not in sql
                        or r["status"] != "pending_delete"]
