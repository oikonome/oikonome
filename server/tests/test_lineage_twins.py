...
def _account(conn, aid, iid):
    conn.execute("INSERT INTO accounts (id, item_id, name, type, mask) "
                 "VALUES (%s,%s,'Business Checking','depository','0007')",
                 (aid, iid))
...
class LineageTwinTests(unittest.TestCase):
    ...
    def test_raw_descriptor_twin_is_retired_and_override_carried(self):
        _txn(self.conn, "raw", "biz", self.d, 25.01,
             "REF000000000042 CLOUDHOST.COM CLOUDHOST.COUS",
             merchant="Cloudhost.cous", override="SOFTWARE")
        _txn(self.conn, "clean", "biz", self.d, 25.01, "Cloudhost",
             merchant="Cloudhost")
    ...
    def test_posted_charge_beats_its_stale_card_hold(self):
        _txn(self.conn, "hold", "biz", self.d, 9.99,
             "Card Hold; 000000000000007 GLIMBOX*SUBSCRIPTION", pending=1)
        _txn(self.conn, "posted", "biz", self.d, 9.99,
             "XX0007 Glimbox*Subscription Glimboxco.Com")
        out = adopt.dedupe_twins(self.conn, "biz")
        self.assertEqual([(o["kept"], o["removed"]) for o in out],
                         [("posted", "hold")])
    ...
    def test_two_different_merchants_same_day_same_amount_both_stay(self):
        ...
        _txn(self.conn, "fuel", "biz", self.d, 4.50, "RIVERTON FUEL 5678",
             merchant="Riverton Fuel")
        self.assertEqual(adopt.dedupe_twins(self.conn, "biz"), [])
    ...
    def test_twins_left_by_a_link_that_synced_first_are_retired_on_request(self):
        ...
        _txn(self.conn, "restored", "old-biz", self.d, 25.01,
             "REF000000000042 CLOUDHOST.COM")
        ...
        adopt.adopt_accounts(self.conn, item_id="cu-item", institution_id="ins_9",
                             ...
                             incoming=[{"account_id": "biz2", "mask": "0007",
                                        "type": "depository"}])
        _txn(self.conn, "fresh", "biz2", self.d, 25.01, "Cloudhost",
             merchant="Cloudhost")
    ...
    def test_losing_note_moves_when_survivor_has_none(self):
        _txn(self.conn, "raw", "biz", self.d, 25.01,
             "REF000042 CLOUDHOST.COUS", merchant="Cloudhost.cous")
        _txn(self.conn, "clean", "biz", self.d, 25.01, "Cloudhost",
             merchant="Cloudhost")
        self._note("raw", "paid the annual server")
        adopt.dedupe_twins(self.conn, "biz")
        notes = self._notes()
        # the note followed the transaction to the survivor, not the row id
        self.assertEqual(notes, {"clean": "paid the annual server"})
