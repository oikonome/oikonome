...
class CanonicalRuleTests(unittest.TestCase):
    ...
    def test_rule_under_one_variant_applies_to_all_variants(self):
        ...
        cache(self.conn, "SUNRISE PLAY BARN", "ENTERTAINMENT")
        # …and generic-bucket transactions arrive under the OTHER variants
        add_raw_txn(self.conn, "a", "2025-07-01", 10, "GglPay SR PLAYLARKSPUR NV",
                    primary="GENERAL_MERCHANDISE",
                    raw={"personal_finance_category": {"primary": "GENERAL_MERCHANDISE"}})
        add_raw_txn(self.conn, "b", "2025-07-02", 12, "SR PLAY BARN POS",
                    primary="OTHER",
    ...
    def test_conflicting_rules_for_one_canonical_stay_split(self):
        canon(self.conn, "AIR HOST SPRING", "Air Host Springfield")
        canon(self.conn, "AIR HOST SPRINGFIELD DELI", "Air Host Springfield")
        cache(self.conn, "AIR HOST SPRING", "TRAVEL")
        cache(self.conn, "AIR HOST SPRINGFIELD DELI", "FOOD_AND_DRINK")
        add_raw_txn(self.conn, "a", "2025-07-01", 10, "AIR HOST SPRING",
                    primary="OTHER",
                    raw={"personal_finance_category": {"primary": "OTHER"}})
        llm_categorize.apply(self.conn)
    ...
    def test_user_correction_on_one_variant_covers_all_incl_sharp(self):
        ...
        self.assertEqual(rule["merchant"], "Sunrise Play Barn")
...
class RekeyMigrationTests(unittest.TestCase):
    ...
    def test_single_raw_variant_is_rekeyed(self):
        canon(self.conn, "GglPay SR PLAYLARK", "Sunrise Play Barn")
        rule(self.conn, "GglPay SR PLAYLARK", "ENTERTAINMENT")
        self._migrate()
        self.assertEqual(list(rules(self.conn)), ["Sunrise Play Barn"])
    ...
    def test_conflicting_raw_rules_stay_split(self):
        ...
        canon(self.conn, "AIR HOST SPRING", "Air Host Springfield")
        canon(self.conn, "AIR HOST SPRINGFIELD DELI", "Air Host Springfield")
        rule(self.conn, "AIR HOST SPRING", "TRAVEL")
        rule(self.conn, "AIR HOST SPRINGFIELD DELI", "FOOD_AND_DRINK")
        self._migrate()
        self.assertEqual(set(rules(self.conn)),
                         {"AIR HOST SPRING", "AIR HOST SPRINGFIELD DELI"})
...
class PendingCanonicalTests(unittest.TestCase):
    ...
    def test_variants_collapse_to_one_pending_canonical(self):
        canon(self.conn, "SR PLAY BARN", "Sunrise Play Barn")
        canon(self.conn, "GglPay SR PLAYLARK", "Sunrise Play Barn")
        self._generic("a", "SR PLAY BARN")
        self._generic("b", "GglPay SR PLAYLARK", primary="GENERAL_MERCHANDISE")
    ...
    def test_rule_under_any_variant_hides_the_canonical(self):
        ...
        canon(self.conn, "SR PLAY BARN", "Sunrise Play Barn")
        canon(self.conn, "GglPay SR PLAYLARK", "Sunrise Play Barn")
        cache(self.conn, "SR PLAY BARN", "ENTERTAINMENT")
        self._generic("b", "GglPay SR PLAYLARK")
