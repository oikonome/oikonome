...
existing history in the file's date window) and the per-row scan. The
old flat candidate list made every claim scan everything fetched — an
O(rows × candidates) product where a decade-spanning file over a large
account meant ~10^9 comparisons pinning a worker and a pooled DB
connection. Candidates are now indexed by (amount, date) so a claim only
examines rows that could match, the fetch refuses a pathological
account, and a per-claim ceiling bounds the worst case where thousands
of rows share one amount/date cell.
"""
...
class ClaimScanBoundTests(unittest.TestCase):
    ...
    def test_off_day_match_prefers_the_nearest_date(self):
        """The bucketed scan keeps the old preference order: nearest date
        first for off-day name matches."""
        d = dt.date(2026, 7, 10)
