    ...
    def test_reset_token_expired(self):
        ...
        with _control() as conn:
            u = conn.execute("SELECT id FROM users WHERE email=%s",
                             (email,)).fetchone()
            token = reset_mod.create_reset(conn, u["id"])
        # backdate via the ADMIN role: the app role's password_resets
        # UPDATE is column-scoped to used_at only (expires_at is exactly
        # what injected SQL must not extend)
