...
def _fresh_code(secret):
    """A code that is accepted now. Step-up doors burn the code, so these
    tests, which are about the step-up GATE and not replay, clear the burn
    first. See util.clear_totp_burn.
    """
...
class Needs2FATests(unittest.TestCase):
    ...
    def test_factorless_hosted_account_can_still_sign_out_of_mobile(self):
        """Signing out must never require enrolling first, at the web door
        (/api/sessions/revoke) and the mobile door (/api/devices/revoke)
        alike; otherwise a factorless phone gets 'enroll a second factor' on
        every sign-out attempt — a dead end the client would misreport as a
        failed revocation."""
