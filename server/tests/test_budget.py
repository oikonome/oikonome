...
class TestMatching(BudgetBase):
    ...
    def test_merchant_match_is_specific(self):
        """A bill's canonical merchant matches ONLY that merchant: 'cobalt
        refresh' won't swallow 'Hawkins Cobalt' — every token must be
        present."""
        add_bill(self.conn, "Cobalt Refresh", 10.59,
                 next_due=TODAY.replace(day=4), last_seen=TODAY.replace(day=4))
        self._set_merchant("Cobalt Refresh", "cobalt refresh")
        add_txn(self.conn, TODAY.replace(day=4), 10.59, "GglPay HAWKINS COBALT",
                primary="FOOD_AND_DRINK")
        add_txn(self.conn, TODAY.replace(day=6), 10.59, "COBALT REFRESH EZ PRI")
        st = self.status()
        rows = st["buckets"]["fixed"]["rows"]
        self.assertEqual(len(rows), 1)
        self.assertIn("REFRESH", rows[0]["name"])
    ...
    def test_merchant_absorbs_variant(self):
        """Merchant tokens are a subset test: bill merchant 'alderway fiber'
        matches the longer descriptor 'Alderway Fiber Telecom'."""
        add_bill(self.conn, "Fiber Internet", 70.0,
                 next_due=TODAY.replace(day=5), last_seen=TODAY.replace(day=5))
        self._set_merchant("Fiber Internet", "alderway fiber")
        add_txn(self.conn, TODAY.replace(day=5), 70.0, "ALDERWAY FIBER TELECOM",
                merchant="Alderway Fiber Telecom")
        rows = self.status()["buckets"]["fixed"]["rows"]
        self.assertEqual(len(rows), 1)
...
class TestOccurrences(unittest.TestCase):
    ...
    def test_negative_interval_is_clamped(self):
        """Restore can store interval=-1; expansion used to walk
        backward until OverflowError. Clamp to 1, same as save_bill."""
