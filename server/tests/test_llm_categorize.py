...
class FlowGuardTests(unittest.TestCase):
    ...
    def test_llm_rule_never_contradicts_a_concrete_plaid_primary(self):
        ...
        burger stand → Rent & Utilities because "Rogers" reads like the
    ...
    def test_seed_rules_take_the_trust_guard_too(self):
        ...
        layer matches bare brand words by substring ("Boost Espresso"
        takes Boost Mobile's utilities label, "Rogers Ice Cream" the
        telecom's). No machine source outranks a concrete Plaid
        primary."""
        self.conn.execute(
            "INSERT INTO merchant_categories (merchant, category_primary, "
            "source) VALUES ('Boost Espresso', "
    ...
    def test_user_rule_still_beats_everything(self):
        self.conn.execute(
            "INSERT INTO merchant_categories (merchant, category_primary, "
            "source) VALUES ('Rogers Ice Cream', 'FOOD_AND_DRINK', 'user')")
        add_raw_txn(self.conn, "t4", "2025-07-04", 26, "Rogers Ice Cream",
...
class AmazonAndProposalTests(unittest.TestCase):
    ...
    def test_repeat_purchase_reuses_existing_summary(self):
        ...
        # one month and 'wildflower honey' the next is the bug this kills
...
class OutletRowsMoveWithWhatThePreviewCounted(unittest.TestCase):
    ...
    instead would give a chain's fuel arm (outlet "Costco Gas",
    merchant_name "Costco") the warehouse's rule: recategorized,
    ...
    def setUp(self):
        ...
        add_raw_txn(self.conn, "gas", "2026-07-10", 42.50, "Costco",
                    primary="GENERAL_MERCHANDISE", raw=PLAID_GENERIC)
        self.conn.execute("UPDATE transactions SET merchant_outlet='Costco Gas' "
                          "WHERE id='gas'")
        add_raw_txn(self.conn, "warehouse", "2026-07-11", 210.00, "Costco",
                    primary="GENERAL_MERCHANDISE", raw=PLAID_GENERIC)
    ...
    def test_a_rule_on_the_outlet_moves_the_outlet_row(self):
        cache(self.conn, "Costco Gas", "TRANSPORTATION")
        previewed, moved = self._moved("Costco Gas", "TRANSPORTATION")
    ...
    def test_a_rule_on_the_brand_leaves_the_outlet_row_alone(self):
        ...
        cache(self.conn, "Costco", "FOOD_AND_DRINK")
        previewed, moved = self._moved("Costco", "FOOD_AND_DRINK")
    ...
    def test_the_categorizer_learns_the_outlet_as_its_own_merchant(self):
        ...
        pend = {m["merchant"] for m in
                llm_categorize.pending_merchants(self.conn)}
        self.assertEqual(pend, {"Costco Gas", "Costco"})
