"""the demo generator — a seeded synthetic household must
light up every product surface: verdict, forecast (with card
scenarios), recurring, budgets, lenses, cash flow, net worth. Same
seed, same household."""
...
class DemoGeneratorTests(unittest.TestCase):
    ...
    def test_receipts_seeded(self):
        ...
        # staged. Bounded on both sides: a runaway seeder
        # would be as wrong as an empty one.
    ...
    def test_first_signin_lands_on_today(self):
        """The wizard is fully skipped (spec §4): the onboarding payload —
        ...
        # sync/import steps. Every key resume scans must
        # be marked or /welcome re-enters mid-wizard instead of bouncing.
    ...
    def test_alert_strip_is_exactly_two(self):
        """A strip of six notifications reads noisy. Bills are
        seeded at the ledger's own recent median, so bills.run applies
        ZERO amount drifts on a fresh demo — the live strip is exactly the
    ...
    def test_food_budget_reads_1500(self):
        """The Food line (post-carve-outs) shows a
        round $1,500 — and the generated grocery spend lives near it, so
        the bar reads lived-in rather than permanently under."""
        from oikonome.engine import budget
        cfg = budget.load_config(self.conn)
        st = budget.month_status(self.conn, TODAY)
    ...
    def test_savings_goals_shape(self):
        """A six-figure Savings target reads absurd. "Savings" stays name-keyed (money-map row) but open-ended;
        "Vacation" is the relatable $5k goal, mid-flight via its
        token-matched transfers."""
        ...
        # 133.33 — correct arithmetic that the hardcoded number called a
        # failure, three days out of every month. Derive it the same way the
        # data is seeded instead.
        today = dt.date.today()
