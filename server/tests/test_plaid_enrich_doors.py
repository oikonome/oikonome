...
operator's Plaid account, so the plan has to be consulted before the send
and the monthly ceiling has to be something a person can actually set,
not a value reachable only by editing the settings document by hand.
...
class TheEnrichDoorConsultsThePlan(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
...
class TheEnrichCapIsAnOperatorCeilingOnHosted(unittest.TestCase):
    ...
    ceiling, and a ceiling the spender can raise is not a ceiling at all. Self-host bills its own Plaid account, so the full
    range stays theirs."""
...
class TheEnrichCeilingHoldsAtSpendTimeNotOnlyAtWriteTime(unittest.TestCase):
    ...
    number already sitting in the document — one written before the door
    clamped, or restored from an older backup. Until that value is read
    through the ceiling it keeps authorising the operator's Plaid spend for
    as long as nobody happens to save settings again, which may be never.
    ...
    def _store(self, value):
        ...
        legacy value the write-time clamp never saw."""
...
class CsvCellsCannotHideAFormulaBehindAnInvisibleCharacter(unittest.TestCase):
    """A leading tab or CR was already neutralized on the reasoning that an
    invisible first character does not stop a spreadsheet reaching the `=`
    behind it. A line feed and a byte-order mark are the same class and were
    not covered, so the guard was inconsistent with its own rationale."""
...
class ThePlanImportDoorRefusesBadBodiesInsteadOf500ing(unittest.TestCase):
    ...
    A number where the CSV text belongs, or an unparseable CSV, reached the
    parser unguarded and surfaced as an opaque server error — which also
    tells a scripted collector nothing about what it got wrong."""
