...
class RenameAndMerge(unittest.TestCase):
    ...
    def test_renaming_moves_every_row_under_that_name(self):
        add_txn(self.conn, "2026-08-01", 20, "Harbor Rehab")
        add_txn(self.conn, "2026-08-02", 30, "Harbor Rehab")
        out = merchant_dedup.rename(self.conn, "Harbor Rehab",
                                    "Harbor Rehabilitation")
        self.assertEqual(out, {"raws": 1, "rows": 2})
        self.assertEqual(self._displayed(), {"Harbor Rehabilitation": 2})

    def test_merging_is_the_same_act_as_renaming(self):
        # two spellings of one clinic, which is what an aggregator that
        # cannot resolve a small merchant produces over time
        add_txn(self.conn, "2026-08-01", 20, "Harbor Rehab")
        add_txn(self.conn, "2026-08-05", 40, "harbor rehabilitation")
        merchant_dedup.rename(self.conn, "harbor rehabilitation",
                              "Harbor Rehab")
        self.assertEqual(self._displayed(), {"Harbor Rehab": 2})

    def test_the_users_word_outranks_the_automatic_layers(self):
        add_txn(self.conn, "2026-08-01", 20, "Harbor Rehab")
        merchant_dedup.rename(self.conn, "Harbor Rehab", "Harbor Rehab PT")
        row = self.conn.execute(
            "SELECT method FROM merchant_canonical "
            "WHERE raw_merchant='Harbor Rehab'").fetchone()
        ...
        self.assertEqual(self._displayed(), {"Harbor Rehab PT": 1})

    def test_an_empty_name_is_refused(self):
        add_txn(self.conn, "2026-08-01", 20, "Harbor Rehab")
        with self.assertRaises(ValueError):
            merchant_dedup.rename(self.conn, "Harbor Rehab", "   ")
    ...
    def test_renaming_to_an_existing_name_merges_regardless_of_case(self):
        ...
        # typing "costco" when "Costco" exists must land in that bucket,
        # not fork a second one differing only in case
        add_txn(self.conn, "2026-08-01", 20, "Costco")
        add_txn(self.conn, "2026-08-02", 30, "Kirkland")
        merchant_dedup.rename(self.conn, "Kirkland", "costco")
        self.assertEqual(self._displayed(), {"Costco": 2})
    ...
    def test_ambiguous_existing_casings_keep_the_name_as_typed(self):
        ...
        add_txn(self.conn, "2026-08-01", 20, "Costco")
        add_txn(self.conn, "2026-08-02", 30, "COSTCO")
        add_txn(self.conn, "2026-08-03", 40, "Kirkland")
        merchant_dedup.rename(self.conn, "Kirkland", "costco")
        self.assertEqual(self._displayed(),
                         {"Costco": 1, "COSTCO": 1, "costco": 1})
    ...
    def test_recasing_a_merchant_keeps_the_casing_the_user_typed(self):
        ...
        add_txn(self.conn, "2026-08-01", 20, "Costco")
        merchant_dedup.rename(self.conn, "Costco", "COSTCO")
        self.assertEqual(self._displayed(), {"COSTCO": 1})
...
class Undo(unittest.TestCase):
    ...
    def test_undo_restores_a_previous_mapping(self):
        add_txn(self.conn, "2026-08-01", 20, "Harbor Rehab Valley")
        merchant_dedup.rename(self.conn, "Harbor Rehab Valley", "Harbor Rehab")
        merchant_dedup.rename(self.conn, "Harbor Rehab", "Harbor PT")
        merchant_dedup.undo(self.conn, self._last_change())
        self.assertEqual(self._shown("Harbor Rehab Valley")["canonical"],
                         "Harbor Rehab")
    ...
    def test_undoing_a_first_mapping_removes_the_row_entirely(self):
        ...
        merchant_dedup.rename(self.conn, "Harbor Rehab", "Harbor PT")
        merchant_dedup.undo(self.conn, self._last_change())
        self.assertIsNone(self._pinned("Harbor Rehab"))
    ...
    def test_the_journal_records_both_ends_of_the_change(self):
        ...
        self.assertEqual(row["raw_merchant"], "Harbor Rehab")
        self.assertIsNone(row["from_canonical"])
        self.assertEqual(row["to_canonical"], "Harbor PT")
    ...
    def test_undoing_a_superseded_change_is_refused(self):
        ...
        add_txn(self.conn, "2026-08-01", 20, "Harbor Rehab Valley")
        merchant_dedup.rename(self.conn, "Harbor Rehab Valley",
                              "Harbor Rehab")
        first = self._last_change()
        merchant_dedup.rename(self.conn, "Harbor Rehab", "Harbor PT")
        mid = self._last_change()
        merchant_dedup.rename(self.conn, "Harbor PT", "Harbor Clinic")
        ...
        self.assertIsNone(self._pinned("Harbor Rehab Valley"))
...
class CatalogOverHttp(unittest.TestCase):
    ...
    The engine grew `order` and the screen grew a control for it, but nothing
    carried the value between them for a while: the endpoint called the
    engine positionally and the parameter was simply dropped. An engine-level
    test cannot see that, so this one goes through the door the client uses.
    """
