...
class AdoptionTests(unittest.TestCase):
    ...
    def test_an_empty_shell_is_retired_with_nothing_to_adopt(self):
        ...
        the candidate pool is empty and the sweep never used to run —
        leaving the shell nagging forever."""
    ...
    def test_an_institution_with_no_live_link_is_left_flagged(self):
        ...
        _item(self.conn, "citi", "error:NO_TOKEN", "ins_99", "Globex Card")
        _account(self.conn, "citi-acct", "citi", mask="4242", type_="credit")
        self._adopt([{"account_id": "new-acct", "mask": "1234",
                      "type": "depository"}])
        self.assertEqual(self.conn.execute(
            "SELECT status FROM items WHERE id='citi'"
        ).fetchone()["status"], "error:NO_TOKEN")

    def test_a_renamed_institution_still_matches_on_its_id(self):
        """Plaid returned 'Acme Bank' where the restore carried 'Acme', so
        a name comparison left the shell behind and it sat in the app
        saying "sync failing" next to the working connection. The id is
        the stable identifier and wins whenever both sides have one."""
        self.conn.execute(
...
class BackfillGuardTests(unittest.TestCase):
    ...
    def test_the_guard_outlives_the_first_sync(self):
        """Closing it after one pagination was the other half of the bug:
        Plaid keeps delivering for days, so the second sync arrived with
        the guard already shut and inserted the duplicates."""
...
class CarriedPlaidIdTests(unittest.TestCase):
    ...
    def test_an_edit_cannot_land_inside_the_rename_and_be_deleted(self):
        """The window the rename used to leave open: the copy is a SNAPSHOT
        and the delete drops whatever the row holds at the end, so an edit
        committing in between is carried by neither and destroyed by the
        delete — silently, after the API already answered 200. The worker
...
class _AfterTheCopy:
    ...
    lands — the exact moment a concurrent web edit used to be able to slip
    in between the copy and the delete."""
