"""Tier-gated Plaid product attachment and the never-refresh rule.

Liabilities and Investments are separately-billed per-Item subscriptions,
so a HOSTED link requests ["transactions"] only unless
OIKONOME_PLAID_PRODUCTS_EXTRA says otherwise.
...
class ContainerEnvironmentTests(EnvMixin, unittest.TestCase):
    ...
    state no containerized install can reach, and would stay green while the
    default is dead in the standard deployment path."""
...
class HostedProductsPullGateTests(EnvMixin, unittest.TestCase):
    ...
    def test_self_host_opt_out_pulls_nothing(self):
        """The opt-out reaches the sync too: off-hosted this method honours
        the variable, or it would spend the owner's money on a product their
        links never consented to."""
        os.environ["OIKONOME_PLAID_PRODUCTS_EXTRA"] = "none"
        self.assertEqual(self._run(), [])
    ...
    def test_self_host_pulls_by_default_under_a_container_environment(self):
        ...
        the opt-out would take liability detail and holdings away from every
        self-host that follows the docs."""
        os.environ["OIKONOME_PLAID_PRODUCTS_EXTRA"] = ""
