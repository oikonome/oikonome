...
class VenmoCounterpartyParser(unittest.TestCase):
    def test_all_caps_name_with_address_tail(self):
        self.assertEqual(
            venmo_counterparty(
                "VENMO *CASEY EXAMPLE 7700 EASTPORT PARKWAY 8558124430, NY, US"),
            "Venmo — Casey Example")
...
class VenmoMerchantPromotion(unittest.TestCase):
    ...
    def test_ledger_payee_is_the_person(self):
        ...
        self._sync_venmo(
            "v4", "VENMO *CASEY EXAMPLE 7700 EASTPORT PARKWAY 8558124430, NY, US")
...
_OUT = ("NOW Withdrawal — Zelle payment from Sam Owner (Bank Spending "
        "Account XXXXXX1234) to CASEY EXAMPLE")
_IN = ("NOW Deposit — Zelle payment from TAYLOR SAMPLE to Sam Owner "
       "(Bank Interest Checking XXXXXX5678)")
...
class ZelleCounterpartyParser(unittest.TestCase):
    ...
    def test_ach_receiver_tag_stripped(self):
        self.assertEqual(
            zelle_counterparty(
                "ACH Withdrawal — Zelle payment from SAM OWNER (Bank Spending "
                "Account 1234) to CASEY EXAMPLE RECEIVER", money_in=False),
            "Zelle — Casey Example")
    ...
    def test_llc_suffix_survives_title_casing(self):
        self.assertEqual(
            zelle_counterparty(
                ...
                "(Bank Spending Account XXXXXX1234)", money_in=True),
    ...
    def test_counterparty_name_containing_to_survives_the_split(self):
        ...
        the first " to " turned that payment into merchant "Zelle — Farm".
        The own side's "(Bank … Account …)" parenthetical anchors the true
        boundary when present; without one, the split keeps the side being
        returned whole."""
        # money out: the OWN (from) name carries " to "; the first-split
        # used to hand back "Table LLC (…" as the recipient
        self.assertEqual(
            zelle_counterparty(
                "Zelle payment from Farm to Table LLC (Bank Spending "
                "Account XXXXXX1234) to Jane Doe", money_in=False),
            "Zelle — Jane Doe")
        # money out: the recipient's own name carrying " to " stays whole
        self.assertEqual(
            zelle_counterparty(
                "Zelle payment from Sam Owner (Bank Spending Account "
    ...
    def test_truncated_line_names_nobody(self):
        self.assertIsNone(zelle_counterparty(
            "NOW Withdrawal — Zelle payment from Sam Owner (Bank",
            money_in=False))
        self.assertIsNone(zelle_counterparty(
...
class ZelleMerchantPromotion(unittest.TestCase):
    ...
    def test_truncated_descriptor_merchant_is_replaced(self):
        ...
        self.assertEqual(
            self._sync_zelle("z2", _OUT, 60.0,
                             merchant="Zelle payment from Sam Owner (Bank"),
            "Zelle — Casey Example")
    ...
    def test_backfill_migration_agrees_with_ingest_parser(self):
        cases = [(_OUT, 60.0), (_IN, -45.0),
                 ("NOW Withdrawal — Zelle payment from Sam Owner (Bank", 10.0),
