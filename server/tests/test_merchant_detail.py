...
class MerchantDetailTests(unittest.TestCase):
    def setUp(self):
        ...
        for i, (city, addr, lat, lon, d) in enumerate([
            ...
            add_raw_txn(self.conn, f"c{i}", d, 40 + i, "COSTCO WHSE #03",
    ...
    def test_places_are_grouped_busiest_first_with_latest_coordinates(self):
        d = merchant_dedup.detail(self.conn, "COSTCO WHSE #03")
    ...
    def test_rule_is_reported(self):
        ...
        data.set_merchant_category(self.conn, "COSTCO WHSE #03", "FOOD_AND_DRINK")
        d = merchant_dedup.detail(self.conn, "COSTCO WHSE #03")
    ...
    def test_the_pin_and_the_address_come_from_the_same_visit(self):
        ...
        add_raw_txn(self.conn, "c3", "2026-08-12", 55, "COSTCO WHSE #03",
                    primary="GENERAL_MERCHANDISE", raw={})
        self.conn.execute(
        ...
        add_raw_txn(self.conn, "c4", "2026-08-15", 12, "COSTCO WHSE #03",
                    primary="GENERAL_MERCHANDISE", raw={})
        self.conn.execute(
            "UPDATE transactions SET location_city='Lakeview', "
    ...
    def test_business_rows_stay_out_of_the_category_and_the_places(self):
        ...
        for i, d in enumerate(["2026-08-10", "2026-08-11", "2026-08-12",
                               "2026-08-13"]):
            add_raw_txn(self.conn, f"b{i}", d, 200, "COSTCO WHSE #03",
    ...
    def test_rule_is_found_through_the_canonical_not_the_spelling(self):
        # The rule lives under the canonical ("Costco"); the merchant ROW
        # the raws resolve to has since taken the aggregator's name
        # ("Costco Wholesale"), which is what the ledger displays. Neither
        ...
        self.conn.execute(
            ...
            "VALUES ('COSTCO WHSE #03', 'Costco', 'manual')")
        self.conn.execute(
            "INSERT INTO merchant_categories (merchant, category_primary, "
            "source) VALUES ('Costco', 'FOOD_AND_DRINK', 'user')")
        mid = self.conn.execute(
            "INSERT INTO merchants (name, name_source) "
            "VALUES ('Costco Wholesale', 'plaid') RETURNING id").fetchone()["id"]
        self.conn.execute(
            "UPDATE transactions SET merchant_id=%s WHERE name='COSTCO WHSE #03'",
            (mid,))
        llm_categorize.apply(self.conn)
        self.assertIsNone(merchant_dedup.detail(self.conn, "COSTCO WHSE #03"))
        d = merchant_dedup.detail(self.conn, "Costco Wholesale")
