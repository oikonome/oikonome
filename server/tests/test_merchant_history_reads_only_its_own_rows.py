...
which is the whole-ledger read — and the two answers
...
def _unnarrowed(conn, payee: str) -> dict:
    """The answer without the SQL narrowing: every non-removed row fetched,
    every row put to the Python predicate."""
...
class AMerchantHistoryIsNarrowedWithoutChangingTheAnswer(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for day, amt in ((2, 35.00), (16, 45.00)):
            tid = add_txn(conn, d(2026, 7, day), amt, "NORTHWIND CLUB GAS #0555",
                          merchant="Northwind Club", account="chk")
            conn.execute("UPDATE transactions SET merchant_outlet=%s "
                         "WHERE id=%s", ("Northwind Club Gas", tid))
        for day, amt in ((4, 200.00), (19, 90.00)):
            add_txn(conn, d(2026, 7, day), amt, "NORTHWIND CLUB WHSE #0555",
                    merchant="Northwind Club", account="chk")
        ...
        add_txn(conn, d(2026, 7, 3), 70.00, "A-MOBILE PCS SVC", account="chk")
        add_txn(conn, d(2026, 8, 3), 70.00, "A-MOBILE PCS SVC", account="chk")
        ...
        add_bill(conn, "Zenith Fibre", 79.99, next_due=d(2026, 9, 5))
        add_txn(conn, d(2026, 6, 5), 79.99, "ZENITH FIBRE TELECOM",
                account="chk")
        add_txn(conn, d(2026, 7, 5), 79.99, "ZENITH FIBRE", account="chk")
        add_txn(conn, d(2026, 8, 5), 79.99, "ZENITH FIBRE TELECOM",
                account="chk")

        # --- a phrase bill: its lone long token must not take A-Mobile ----
        ...
        add_bill(conn, "Tutoring", 400, bill_type="envelope",
                 category="CHILD CARE", match_category=True)
        add_txn(conn, d(2026, 7, 6), 100.0, "ZELLE TUTOR PAYMENT",
    ...
    # every entry point a client can link to, plus the ones answered by the
    # token fallback alone
    PROBES = ["Northwind Club", "Northwind Club Gas", "The Blue Bottle Co.", "Amazon",
              "AMZN MKTP US*2H4K9", "Nova Mobile", "A-Mobile", "Helio",
              "Care", "Borealis Healthcare", "Café Müller Berlin",
              "Muller Grocery", "Zenith Fibre", "Tutoring",
    ...
    def test_the_near_misses_are_still_told_apart(self):
        ...
        self.assertTrue(got["Northwind Club"] and got["Northwind Club Gas"])
        self.assertFalse(set(got["Northwind Club"]) & set(got["Northwind Club Gas"]))
        # a phrase bill takes its own carrier and not the other one, while
        # the arbitrary-merchant page for "A-Mobile" still falls back to its
        ...
        self.assertFalse(set(got["Nova Mobile"]) & {"A-MOBILE PCS SVC"})
        self.assertEqual(len(got["A-Mobile"]), 4)
    ...
    def test_the_tokenizer_never_sees_the_whole_ledger(self):
        ...
        with mock.patch.object(budget, "_tokens", counting):
            bills.merchant_history(self.conn, "Zenith Fibre", today=TODAY)
            bills.merchant_history(self.conn, "Northwind Club Gas", today=TODAY)
