...
LLM_JSON = {"merchant": "BIGBOX", "date": "2026-07-10", "total": 63.00,
            "tax": 3.00, "tip": None,
            "items": [
...
class ReceiptTests(unittest.TestCase):
    ...
    def test_parse_tag_and_report(self):
        ...
        self.assertEqual(lst["parsed"]["merchant"], "BIGBOX")
        ...
        self.assertEqual(rep["rows"][0]["payee"], "BIGBOX WHOLESALE")
...
class UploadEndpointTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            ...
            cls.txn = _add(conn, TODAY, 63.00, "BIGBOX WHOLESALE",
                           account="chk")
        finally:
            conn.close()
...
LLM_JSON_CATS = {"merchant": "BIGBOX", "date": "2026-07-10", "total": 63.00,
                 "tax": 3.00, "tip": None,
                 "items": [
...
class ItemSearchTests(unittest.TestCase):
    ...
    def test_search_and_grouping(self):
        ...
        self.assertEqual(out["rows"][0]["payee"], "BIGBOX WHOLESALE")
        ...
        self.assertEqual(out["groups"][0]["key"], "BIGBOX WHOLESALE")
    ...
    def test_a_backend_body_never_reaches_the_receipt_panel(self):
        ...
        try:
            ...
            bad = httpx.MockTransport(lambda r: httpx.Response(500, json={
                "error": secret}))
            with self.assertLogs("oikonome.receipts", "DEBUG") as logs:
                receipts.parse_one(conn, rid, transport=bad)
            row = receipts.for_txn(conn, txn)[0]
            ...
            # the warning names the failure class only — it reaches the
            # feedback bundle's log ring, which keeps INFO and up …
            warned = [r for r in logs.records if r.levelname == "WARNING"]
            self.assertTrue(warned)
            for r in warned:
                self.assertNotIn("sk-live-7f3a", r.getMessage())
                self.assertNotIn("llm.test", r.getMessage())
            # … and the body goes to DEBUG, marked out of that ring
            detail = [r for r in logs.records if r.levelname == "DEBUG"
                      and "sk-live-7f3a" in r.getMessage()]
            self.assertTrue(detail)
            self.assertTrue(all(getattr(r, "no_capture", False)
                                for r in detail))
        finally:
            conn.close()
