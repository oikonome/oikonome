...
class BulkCategoryTests(unittest.TestCase):
    def setUp(self):
        self.conn = make_db()
        for raw in ("SUNRISE PLAY BARN", "GglPay SR PLAYLARKSPUR NV"):
            canon(self.conn, raw, "Sunrise Play Barn")
        # two descriptor variants of one merchant, both mis-categorized
        add_raw_txn(self.conn, "a", "2025-07-01", 10, "SUNRISE PLAY BARN",
                    primary="PERSONAL_CARE",
                    raw={"personal_finance_category": {"primary": "PERSONAL_CARE"}})
        add_raw_txn(self.conn, "b", "2025-07-02", 12, "GglPay SR PLAYLARKSPUR NV",
    ...
    def test_preview_counts_canonical_rows(self):
        ...
        p = data.merchant_category_preview(self.conn, "GglPay SR PLAYLARKSPUR NV",
                                           "ENTERTAINMENT")
        self.assertEqual(p["merchant"], "Sunrise Play Barn")
        self.assertEqual(p["count"], 2)

    def test_apply_covers_all_variants_past_and_future(self):
        res = data.set_merchant_category(self.conn, "SUNRISE PLAY BARN",
                                         "ENTERTAINMENT")
        ...
        canon(self.conn, "SR PLAY BARN POS", "Sunrise Play Barn")
        add_raw_txn(self.conn, "c", "2025-08-01", 9, "SR PLAY BARN POS",
                    primary="OTHER",
    ...
    def test_user_rule_reaches_flow_rows(self):
        ...
        # transfer: a Venmo/Zelle payment to the babysitter is TRANSFER_OUT
        # to Plaid and childcare to the household, and every new one must
        # land where the rule says — not as a transfer until hand-fixed.
        canon(self.conn, "NW XFER", "Sunrise Play Barn")   # sibling variant
    ...
    def test_flow_category_rejected(self):
        with self.assertRaises(ValueError):
            data.set_merchant_category(self.conn, "SUNRISE PLAY BARN",
                                       "TRANSFER_OUT")
    ...
    def test_variants_disclose_canonical_siblings(self):
        ...
        # whose tokens DON'T match its canonical sibling ("GglPay SR
        # PLAYLARK…" vs "SUNRISE PLAY BARN") still moves the sibling's
        # rows via the shared canonical — the confirm must NAME that scope,
        # not just count it.
        p = data.merchant_category_preview(self.conn,
                                           "GglPay SR PLAYLARKSPUR NV",
                                           "ENTERTAINMENT")
        self.assertEqual(p["count"], 2)          # sibling row counted…
        self.assertIn("SUNRISE PLAY BARN", p["variants"])   # …and named
        self.assertIn("GglPay SR PLAYLARKSPUR NV", p["variants"])
...
class TokenFamilyTests(unittest.TestCase):
    ...
    def setUp(self):
        self.conn = make_db()
        # one real store, split across DIFFERENT canonicals by truncation
        canon(self.conn, "LKS CAROUSEL", "Lks Carousel")
        canon(self.conn, "LKS CAROUSEL LKS", "Lks Carousel Lks")
        add_raw_txn(self.conn, "t1", "2025-07-01", 15, "LKS CAROUSEL",
                    primary="PERSONAL_CARE",
                    raw={"personal_finance_category":
                         {"primary": "PERSONAL_CARE"}})
        add_raw_txn(self.conn, "t2", "2025-07-02", 36, "LKS CAROUSEL LKS",
    ...
    def test_preview_spans_the_page_scope_and_names_variants(self):
        p = data.merchant_category_preview(self.conn, "LKS CAROUSEL",
                                           "ENTERTAINMENT")
        self.assertEqual(p["count"], 2)      # both canonicals' rows counted
        self.assertIn("LKS CAROUSEL", p["variants"])
        self.assertIn("LKS CAROUSEL LKS", p["variants"])
    ...
    def test_preview_can_scope_to_the_single_canonical(self):
        ...
        self.assertEqual(p["variants"], ["LKS CAROUSEL"])

    def test_apply_covers_every_canonical_the_page_matches(self):
        res = data.set_merchant_category(self.conn, "LKS CAROUSEL",
                                         "ENTERTAINMENT")
        self.assertEqual(res["count"], 2)
        ...
        self.assertIn("Lks Carousel", rules)
        self.assertIn("Lks Carousel Lks", rules)

    def test_scope_is_the_payees_tokens_not_the_reverse(self):
        # clicking the LONGER name must not sweep the shorter one's rows:
        # 'LKS CAROUSEL LKS' tokens are not all present in 'LKS CAROUSEL'…
        # (they are — same token SET — so use a genuinely narrower payee)
        canon(self.conn, "LKS CAROUSEL GIFT SHOP", "Lks Carousel Gift Shop")
        add_raw_txn(self.conn, "gift", "2025-07-03", 9,
                    "LKS CAROUSEL GIFT SHOP", primary="PERSONAL_CARE",
                    raw={"personal_finance_category":
                         {"primary": "PERSONAL_CARE"}})
        p = data.merchant_category_preview(self.conn,
                                           "LKS CAROUSEL GIFT SHOP",
                                           "ENTERTAINMENT")
    ...
    def test_flow_rows_across_the_family_count_apply_and_undo_alike(self):
        ...
        add_raw_txn(self.conn, "x1", "2025-07-03", 300, "LKS CAROUSEL",
                    primary="TRANSFER_OUT", raw={})
        add_raw_txn(self.conn, "x2", "2025-07-04", 120, "LKS CAROUSEL LKS",
                    primary="TRANSFER_OUT", raw={})
        add_raw_txn(self.conn, "ov", "2025-07-05", 9, "LKS CAROUSEL LKS",
...
class PageBoundaryTests(unittest.TestCase):
    ...
    T-Mobile rows the page never showed. The write must never match looser
    ...
    def test_phrase_bill_never_sweeps_lookalike_merchants(self):
        ...
        add_raw_txn(self.conn, "us", "2025-07-01", 25, "AX MOBILE",
                    ...
                    raw={"personal_finance_category":
                         {"primary": "GENERAL_SERVICES"}})
        add_raw_txn(self.conn, "tmo", "2025-07-02", 80, "T-MOBILE",
                    primary="GENERAL_SERVICES",
                    raw={"personal_finance_category":
                         {"primary": "GENERAL_SERVICES"}})
        ...
        self.assertNotIn("T-MOBILE", p["variants"])
        ...
        self.assertNotIn("T-MOBILE", rules)
