...
Reimbursements never stated its own number — "how much am I owed?" is the
reason the list exists and you had to add the column yourself. Partial
expectations were a checkbox plus a naked $ input in the merchant cell of
EVERY row: permanent form controls for a state most rows never enter.

Rules opened with two explanatory paragraphs before a single rule appeared,
put its rarest action (rename a category) above its content, and never said
how much any rule actually does.
"""
