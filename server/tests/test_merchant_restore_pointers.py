...
class RestoredMerchantPointers(unittest.TestCase):
    ...
    def test_pointers_at_merchants_the_archive_omitted_land_as_null(self):
        ...
        data = _zip({
            "merchants": (M_COLS, [
                {"id": real, "name": "Costco", "name_source": "plaid",
                 "kind": "merchant"}]),
            ...
            "merchant_canonical": (
                ["raw_merchant", "canonical", "method", "merchant_id"], [
                    {"raw_merchant": "COSTCO WHSE", "canonical": "Costco",
...
class MergeOffersSurviveARestore(unittest.TestCase):
    ...
    def test_decisions_and_offers_round_trip_and_a_rejection_still_holds(self):
        ...
        for name, raw, n in (("Riverton Natural Foods", "RIVERTON NATURAL FOODS", 5),
            ...
            for i in range(n):
                ...
                src.execute("UPDATE transactions SET merchant_id=%s WHERE id=%s",
                            (mid, tid))
        mm.run(src)
        rejected = next(o for o in mm.pending(src) if o["from"] == "Riverton Natur")
        ...
        self.assertEqual(rows[rejected["id"]]["f"], ids["Riverton Natur"])
        self.assertEqual(rows[rejected["id"]]["i"], ids["Riverton Natural Foods"])
        ...
        self.assertNotIn("Riverton Natur", {o["from"] for o in mm.pending(dest)})
    ...
    def test_an_offer_about_a_merchant_the_archive_omitted_is_dropped(self):
        ...
        rows = [{"id": real, "name": "Riverton Natural Foods",
