...
A live session is not proof of the human. Without step-up, a stolen cookie
could enroll its OWN second factor — and enrolling revokes every other
session, so session theft would escalate into locking the real owner out
of their own money using the 2FA machinery as the weapon. The password (or
a passkey step-up) is the thing a session thief does not have, so every
route that mutates credentials, factors, identity, or delegated access
re-asks for it.

Step-up is applied route by route, and the next unguarded one is a single
`def` away at all times; reading every mutating route by eye is
probabilistic, this is not.

Two assertions, doing different jobs:

* STEPUP_REQUIRED is a PIN. Every route in it calls a step-up helper. If a
  refactor drops the call, the test fails — a guard cannot be removed
  silently.
* SENSITIVE matches by PATH. Any new mutating route that looks like it
  ...
  STEPUP_EXEMPT with a reason, so it is triaged when it lands, by the
  person who has the context.
...
class ReauthClassificationTests(unittest.TestCase):
    ...
    a wording change on the server must not silently reclassify a prompt
    as a logout; otherwise the app blanks to the login screen and eats the
    message."""
