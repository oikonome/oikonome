...
2. Requests that differ only below the grid the page's own controls step on
   ...
   exact parameter tuple; without the snap, walking `spend` by a dollar
   would miss the cache — and run ~10k simulations — on every request.
"""
...
class TheRetirementProjectionIsBounded(unittest.TestCase):
    ...
    def test_a_dollar_of_spend_does_not_buy_a_fresh_simulation(self):
        ...
        The memo key is the full parameter tuple, so without snapping an
        attacker (or a jittery client) that walks a parameter by 1 would
        get a full recompute per request while also evicting every other
        tenant's entry from the shared LRU."""
