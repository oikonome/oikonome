...
after local midnight). Liveness belongs to the caller: live=True means
`today` IS the caller's present; only a live=False past-day view may be
historical. If gather() re-decided liveness by comparing that date to
dt.date.today() — the instance zone — a mismatch would flip the send
"historical" and silently drop the whole fixed-bill schedule from a live
daily email.
"""
...
class LiveGatherAcrossZones(unittest.TestCase):
    ...
    def test_a_past_day_in_the_current_month_is_not_historical(self):
        # same-month back-nav keeps the live schedule
