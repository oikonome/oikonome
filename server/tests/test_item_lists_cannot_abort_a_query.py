...
#
# It reads whatever expression the call was handed, however wrapped. A
# scanner that matched only a BARE `alias.col` would let a wrapped argument
# — `COALESCE(alias.col, '[]'::jsonb)`, the idiom that looks like a shape
# guard and is not one — take the call site OUT of the check entirely: a
# guard that a wrapper defeats reports a clean tree while the class it
# exists to prevent spreads.
...
class RestoredItemListShapes(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.c_obj = add_txn(self.conn, "2026-08-04", 90.00,
                             "Northwind Club Object")
        self.c_ok = add_txn(self.conn, "2026-08-05", 30.00,
                            "Northwind Club Good")
...
class ADatabaseThatAlreadyHoldsABadRow(unittest.TestCase):
    """A database holding a row the constraint would refuse still answers
    — the read path never assumes the constraint."""
...
class ItemListColumnsAreConstrained(unittest.TestCase):
    ...
    Guarding each reader as it is written is not enough: a new table copies
    the write idiom, and the readers it does not know about are the ones
    that break. Declaring '[]' as a column's default is the author
    ...
    def test_a_wrapped_argument_is_still_seen(self):
        """A wrapper or a path read must not hide the value from the
        scanner; otherwise each of these reads as clean while the value
