...
* Split accounts were always created type=depository, so card
accounts broke spend math (credit sign/exclusion never applied); the
split now infers type from the vendor-side account name, conservatively.
* A multi-statement OFX file (several <STMTRS> blocks, e.g.
checking + savings from one bank) landed entirely in the selected
account; it now splits per statement, keyed by ACCTID suffix.
"""
