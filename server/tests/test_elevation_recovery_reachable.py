...
the login screen offers when the authenticator is gone — and both clients
must be able to send that shape. The recovery form must be reachable from
the password path too; otherwise an account with a password and TOTP sees
a password field, a six-digit field capped at six characters, and no way
out: a 24-character recovery code cannot even be typed, let alone sent.
...
class WebElevationSheetTests(unittest.TestCase):
    ...
    def test_the_recovery_form_sends_the_shape_the_server_documents(self):
        ...
        (password_required), so a form that could only send one would be a
    ...
    def test_the_recovery_field_can_hold_a_recovery_code(self):
        ...
        A maxLength of six, as on the authenticator field, would make one
        untypeable."""
...
class MobileElevationSheetTests(unittest.TestCase):
    ...
    def test_the_password_form_offers_the_recovery_route(self):
        """setForm("recovery") must be reachable from the password form,
        not only from the passkey path's failure."""
