...
class RuleReachesUnresolvedRow(unittest.TestCase):
    ...
    def test_merchant_keyed_rule_applies_to_a_row_with_no_merchant_yet(self):
        ...
        mid = _merchant(conn, "Northwind Club")
        # the rule resolves to the merchant ROW…
        _alias(conn, "NORTHWIND CLUB #0555", "Northwind Club", mid)
        _rule(conn, "NORTHWIND CLUB #0555", "FOOD_AND_DRINK")
        # …while the transaction is not resolved yet, only deduped
        tid = add_txn(conn, "2026-07-01", 88.0, "NORTHWIND CLUB #0555",
                      merchant="NORTHWIND CLUB #0555", primary="GENERAL_MERCHANDISE")
...
class TwoRulesOneRow(unittest.TestCase):
    ...
    def _two_routes(self, conn, string_source="llm", merchant_source="llm"):
        ...
        mid = _merchant(conn, "Northwind Club")
        _alias(conn, "NORTHWIND CLUB #0555", "Northwind Club", mid)
        _rule(conn, "NORTHWIND CLUB #0555", "FOOD_AND_DRINK", merchant_source)
        # a second rule whose string carries no merchant of its own but
        # whose canonical is the same display name
        _alias(conn, "NORTHWIND CLUB FUEL", "Northwind Club", None)
        _rule(conn, "NORTHWIND CLUB FUEL", "TRANSPORTATION", string_source)
        return add_txn(conn, "2026-07-02", 44.0, "NORTHWIND CLUB #0555",
                       merchant="NORTHWIND CLUB #0555",
...
class UserRuleSurvivesADisagreeingAliasRule(unittest.TestCase):
    ...
    def test_a_model_rule_on_a_sibling_alias_does_not_cancel_the_user_rule(self):
        ...
        mid = _merchant(conn, "Northwind Club")
        _alias(conn, "NORTHWIND CLUB #0555", "Northwind Club", mid)
        _alias(conn, "NORTHWIND CLUB GAS #55", "Northwind Club", mid)
        _rule(conn, "NORTHWIND CLUB #0555", "FOOD_AND_DRINK", "user")
        _rule(conn, "NORTHWIND CLUB GAS #55", "TRANSPORTATION", "model")
        tid = add_txn(conn, "2026-07-03", 30.0, "NORTHWIND CLUB #0555",
                      merchant="NORTHWIND CLUB #0555", primary="GENERAL_MERCHANDISE")
