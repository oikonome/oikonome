...
taxdoc preview). Each had a per-process dict: invisible to other
workers (the follow-up request failed if a different process answered),
silently evicting at a fixed count, and the taxdoc one not tenant-bound
at all — a token minted under tenant A committed rows under tenant B's
connection. Staging lives in the RLS-bound import_staging table now:
tenant binding is the database's job, any worker can finish what
another started, eviction is an explicit TTL + per-tenant byte budget
(without one, an owner could park gigabytes of "pending" uploads
on a shared node).
"""
...
class StagingStoreTests(unittest.TestCase):
    ...
    def test_concurrent_puts_cannot_both_fit_under_the_budget(self):
        """Two parallel uploads used to read the same held-bytes SUM before
        either inserted, so both fit under a budget only one left room for
        — the check-and-insert now serializes on a per-tenant advisory
        lock, and the second caller sees the first's bytes and refuses."""
        import threading
...
class BulkStagingTests(unittest.TestCase):
    ...
    def test_bulk_survives_a_worker_swap(self):
        ...
        # different connection object. The old dict made this a 404.
        plan = bulk_import.analyze(
            self.conn, [("stmt.csv",
                         b"Date,Description,Amount\n2026-07-01,Y,-7.00\n")])
        tid = self.conn.execute(
...
class TaxdocTenantBindingTests(unittest.TestCase):
    """The taxdoc stage was the one with NO tenant check at all."""
