...
class SplitMerchantRepairTests(unittest.TestCase):
    ...
    def _split_pair(self, *, enriched_first=True):
        ...
        if enriched_first:
            add_txn(self.conn, "2026-07-01", 24.63, DESCRIPTOR,
                    merchant="example telecom", txn_id="chg")
            merchant_identity.resolve(self.conn)
            self._unenriched("2026-07-02", -24.63, DESCRIPTOR, "ref")
            self._split_off("ref")
        else:
            self._unenriched("2026-07-01", -24.63, DESCRIPTOR, "ref")
            merchant_identity.resolve(self.conn)
            add_txn(self.conn, "2026-07-02", 24.63, DESCRIPTOR,
                    merchant="example telecom", txn_id="chg")
            merchant_identity.resolve(self.conn)
    ...
    def test_the_survivor_is_the_merchant_with_the_stronger_name(self):
        ...
        self.conn.execute(
            """INSERT INTO transactions (id,account_id,date,amount,name,
                   merchant_name,category_primary,pending,removed,raw)
               VALUES ('p1','card',%s,24.63,%s,'Example Telecom',
        ...
        self._unenriched("2026-07-02", -24.63, DESCRIPTOR, "u1")
