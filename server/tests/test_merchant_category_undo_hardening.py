...
and flow-refusal tests rejected genuine snapshots (a custom label that
lived only on the moved rows, a legacy flow-named rule) and made their
undo 400 forever. The merchant detail read stays control-character-strict
...
class UndoSnapshotShapeTests(unittest.TestCase):
    def setUp(self):
        ...
        self.res = data.set_merchant_category(self.conn, "SUNRISE PLAY BARN",
                                              "ENTERTAINMENT")
    ...
    def test_lists_of_strings_are_refused_not_crashed(self):
        # a list of strings where objects belong used to die on .get → 500
        self._refused({"canons": ["x"], "affected": ["y"]}, "strings")
        self._refused({"canons": [{"canon": "Sunrise Play Barn"}],
                       "affected": [["id", "prev"]]}, "nested list")
        self._refused({"canons": "Sunrise Play Barn"}, "canons not a list")
    ...
    def test_oversized_lists_are_refused(self):
        ...
        rules = [{"merchant": "Sunrise Play Barn",
    ...
    def test_prev_is_shape_checked_not_membership_checked(self):
        ...
        # label lives only in the snapshot, and a membership test against
        # "known" categories 400'd exactly those undos forever
    ...
    def test_a_custom_category_named_by_a_snapshotted_rule_restores(self):
        ...
        undo = {"canons": [{"canon": "Sunrise Play Barn",
    ...
    def test_a_flow_named_prior_rule_restores_and_stays_inert(self):
        ...
        # replayed undo, an old row) — refusing it made that undo 400
        # forever. Restoring it is safe because apply() refuses to act on
    ...
    def test_a_prior_rule_over_the_old_120_char_cap_restores(self):
        # rule categories are bounded like every other snapshot name (200);
        # the old 120-char cap 400'd snapshots holding a custom category
        # set_merchant_category itself had accepted
...
class MerchantDetailInputTests(unittest.TestCase):
    ...
    display name the catalog lists, and a capped read made exactly those
    rows unopenable."""
