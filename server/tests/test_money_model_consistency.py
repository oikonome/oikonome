...
class ReimbOutstandingTests(unittest.TestCase):
    ...
    def test_fully_received_flag_never_alerts(self):
        ...
        # the matching deposit has to exist: reimbursements carry real FKs
...
class PrepaidLongCycleBillTests(unittest.TestCase):
    """The occurrence match window (and the pre-month-start payment
    lookback) caps at 35 days, so a long-cycle (quarterly/annual) bill
    prepaid weeks early is matched, not reserved again. The window never
    exceeds HALF the bill's cycle, so monthly and shorter bills keep their
    15/7/3-day windows — a prior-month payment can never be nearer to next
    month's occurrence than to its own."""
    ...
    def test_monthly_decoy_case_unchanged(self):
        """The decoy case holds under the 35-day cap: a Jul 1 payment of
