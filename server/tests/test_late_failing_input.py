"""Input must be refused at the door, not become a 500 or a wrong number later.

Grouped together because they share one shape: input accepted at the door
that fails somewhere the user cannot connect to what they typed.
"""
...
class InputBoundsTests(unittest.TestCase):
    ...
    def test_absurd_year_is_rejected_on_the_way_in(self):
        """An entity formed in year 9999 must be refused on entry; otherwise
        the compliance calendar later raises from `date(year+1, ...)` — a
        500 on a page the user never linked to the value they typed."""
...
class PurgeAndIdempotencyTests(unittest.TestCase):
    ...
    def test_purging_a_transaction_takes_its_annotations_with_it(self):
        """Tables keyed on a transaction id must not keep rows for a purged
        transaction; otherwise a future id collision re-attaches an old
        note to a new transaction."""
        self.conn.execute(
    ...
    def test_equity_movement_survives_its_transaction(self):
        ...
        would be worse than an orphaned link."""
        equity.record_movement(self.conn, self.ent["id"], kind="contribution",
...
class SyncSingleFlightTests(unittest.TestCase):
    """Several doors reach sync_tenant; only one run per tenant proceeds."""
...
class MigrationOverExistingDuplicatesTests(unittest.TestCase):
    """Migration 067 must de-dup existing rows before its unique index.

    Otherwise the index meets a duplicate pair a double-click already
    stored, CREATE UNIQUE INDEX raises, the whole migration rolls back (one
    transaction per file) without stamping schema_migrations, and it
    re-fails on every boot: the instance does not start until an operator
    hand-deletes rows.
    """
    ...
    def test_migration_dedups_instead_of_wedging_the_instance(self):
        ...
        try:
            ...
            add_txn(conn, "2026-07-10", 50.0, "SUPPLIES", account="card",
                    txn_id="dup1")
            # drop the guard, plant the duplicate a double-submit creates
