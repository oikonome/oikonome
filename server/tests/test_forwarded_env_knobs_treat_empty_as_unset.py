...
the app quietly does the other thing. Each such read looks correct on its own
page, so the rule is checked mechanically over the whole package rather than
restated at each new call site (`envnum` holds the same rule for numbers and
flags).
