...
class RenameIsOfferedNotAssumed(_Ledger):
    ...
    def test_nightly_offers_the_new_name_and_approval_attaches_it(self):
        ...
        self.assertEqual((p["payee"], p["evidence"]["bill_payee"]),
                         ("Northwind", PAYEE))
        # the offer says what approving it changes: the merchant's charges
        # and what the bill counts today
        self.assertRegex(bills.merchant_offer_summary(self.conn, p),
                         r"^\d+ charges? under Northwind, last \d\d/\d\d/\d\d"
                         r" · the bill counts Northwind Headquarters today$")
...
class AttachBoundary(unittest.TestCase):
    ...
    def test_a_one_word_bill_offered_siblings_keeps_only_the_shortening(self):
        ...
        for n in ("Riverton Power", "Riverton County Service"):
            self._merchant(n)
        self.assertEqual(bills.attachable(self.conn, "Riverton", "riverton",
                                          {"Riverton Power", "Riverton County Service"}),
                         [])

    def test_a_single_offer_stands_when_nothing_shorter_exists(self):
        self._merchant("Zenith Insurance")
        self.assertEqual(bills.attachable(self.conn, "Zenith Ins",
    ...
    def test_a_lone_sibling_is_not_a_rename_when_the_shortening_exists(self):
        ...
        for n in ("Riverton", "Riverton Power"):
            self._merchant(n)
        self.assertEqual(bills.attachable(self.conn, "Riverton", "riverton",
                                          {"Riverton Power"}), [])
