...
class InviteFanOutTests(unittest.TestCase):
    ...
    A thread per invited address would each open a tenant connection out of
    the pool and then sit in an SMTP conversation, so a single settings save
    could hold MAX_RECIPIENTS of them at once and starve every other request.
    They are all one tenant's invites, so one thread walks them and the SMTP
    settings are resolved once."""
...
class InviteLinkRefusalTests(unittest.TestCase):
    ...
    safe link, so the invite is logged instead of mailed. It must be
    recorded as undelivered; otherwise Settings shows "invited", the owner
    is told the invitation went out, and the only copy of the link sits in a
    server log nobody reads on a mail-configured box — the person can never
    accept and never learn why."""
