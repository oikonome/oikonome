...
class CostcoMatchTests(unittest.TestCase):
    ...
    def test_same_day_receipt_matches_and_carries_its_category(self):
        add_receipt(self.conn, "r1", "2025-07-10", -300.00, "Food & Drink",
                    summary="groceries $200 · household $100")
        t = add_txn(self.conn, "2025-07-10", 300.00, "COSTCO WHSE #0555")
    ...
    def test_unmatched_costco_charge_keeps_its_bank_category(self):
        ...
        t = add_txn(self.conn, "2025-07-10", 60.00, "COSTCO GAS #0555",
    ...
    def test_an_online_order_charged_per_shipment_matches_every_charge(self):
        # costco.com bills each shipment as it leaves: one order, two
        # charges, neither equal to the order total
        add_receipt(self.conn, "o1", "2025-07-01", -140.00, "Home",
                    rtype="online")
        a = add_txn(self.conn, "2025-07-03", 60.00, "COSTCO.COM")
        b = add_txn(self.conn, "2025-07-09", 80.00, "COSTCO.COM")
        res = costco_match.run_match(self.conn)
        self.assertEqual(res["matched"], 2)
        self.assertEqual(override(self.conn, a), "Costco - Home")
        self.assertEqual(override(self.conn, b), "Costco - Home")
        self.assertEqual({r["dedup_key"] for r in self.conn.execute(
            "SELECT dedup_key FROM costco_matches").fetchall()}, {"o1"})

    def test_a_return_refunded_in_parts_matches_the_refund_record(self):
        add_receipt(self.conn, "o1", "2025-07-01", 180.00, "Refunds",
                    is_refund=1, rtype="online")
        a = add_txn(self.conn, "2025-07-05", -125.00, "COSTCO.COM")
        b = add_txn(self.conn, "2025-07-05", -55.00, "COSTCO.COM")
        costco_match.run_match(self.conn)
        self.assertEqual(override(self.conn, a), "Costco - Refunds")
        self.assertEqual(override(self.conn, b), "Costco - Refunds")

    def test_a_warehouse_receipt_is_never_matched_to_a_sum(self):
        # a warehouse receipt is one swipe; two unrelated charges that
        # happen to add up must not be taken for it
        add_receipt(self.conn, "w1", "2025-07-01", -100.00, "Food & Drink")
        a = add_txn(self.conn, "2025-07-01", 60.00, "COSTCO WHSE")
        b = add_txn(self.conn, "2025-07-02", 40.00, "COSTCO WHSE")
        costco_match.run_match(self.conn)
        self.assertIsNone(override(self.conn, a))
        self.assertIsNone(override(self.conn, b))

    def test_a_shipment_past_the_split_window_leaves_the_order_unmatched(self):
        add_receipt(self.conn, "o1", "2025-07-01", -100.00, "Home",
                    rtype="online")
        a = add_txn(self.conn, "2025-07-03", 60.00, "COSTCO.COM")
        b = add_txn(self.conn, "2025-07-20", 40.00, "COSTCO.COM")  # +19
        costco_match.run_match(self.conn)
        self.assertIsNone(override(self.conn, a))
        self.assertIsNone(override(self.conn, b))

    def test_an_exact_charge_is_preferred_over_a_split(self):
        add_receipt(self.conn, "o1", "2025-07-01", -90.00, "Home",
                    rtype="online")
        whole = add_txn(self.conn, "2025-07-03", 90.00, "COSTCO.COM")
        part = add_txn(self.conn, "2025-07-04", 50.00, "COSTCO.COM")
        part2 = add_txn(self.conn, "2025-07-04", 40.00, "COSTCO.COM")
        costco_match.run_match(self.conn)
        self.assertEqual(override(self.conn, whole), "Costco - Home")
        self.assertIsNone(override(self.conn, part))
        self.assertIsNone(override(self.conn, part2))
...
class CostcoImportDoorTests(unittest.TestCase):
    ...
    def test_push_matches_and_a_repush_refreshes_categories(self):
        t = add_txn(self.conn, "2025-07-10", 99.00, "COSTCO WHSE")
        row = {"dedup_key": "main|w|2025-07-10|C|99.00", "date": "2025-07-10",
               "amount": -99.00, "category": "Shopping",
               "summary": "shopping $99",
               "items": [{"description": "ORGANIC EGGS", "amount": 9.00,
