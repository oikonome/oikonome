...
Every section's row carries a subtitle stating its current state, so the
pane answers "what is my setup?" for all of itself, and a real problem (a
dead connection, 2FA off) is visible without opening the section that owns
it.

A row states its current value first: a cadence reads label, value, change —
not a control cluster you have to read to learn what is set.
"""
