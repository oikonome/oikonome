...
class ClassificationTests(unittest.TestCase):
    ...
    def test_org_name_classifies_card_and_labels_connection(self):
        ...
        payload = copy.deepcopy(PAYLOAD)
        payload["accounts"] = [
            {"id": "acc-card-a", "name": "Alex Rivera (1234)",     # no credit hint
             "currency": "USD", "balance": "-43.00",
             "org": {"name": "Northwind Credit Card"}, "transactions": []},
            {"id": "acc-card-b", "name": "Blue (1111)",
        ...
        self.assertEqual(types["acc-card-a"], "credit")   # org → credit
        self.assertEqual(types["acc-card-b"], "credit")   # "american express"
    ...
    def test_user_classification_survives_resync(self):
        ...
        payload = copy.deepcopy(PAYLOAD)
        payload["accounts"] = [
            {"id": "acc-misread", "name": "Everyday Spending",  # no credit hint
             "currency": "USD", "balance": "-250.00",
             "org": {"name": "Demo Bank"}, "transactions": []}]
        self._sync(payload)
        row = self.conn.execute(
            "SELECT type, balance_current FROM accounts WHERE id='acc-misread'"
        ).fetchone()
        ...
        sync_base.mark_type_user_set(self.conn, "acc-misread", "credit",
                                     "credit card")
        self._sync(payload)                              # the hourly worker
        row = self.conn.execute(
            "SELECT type, subtype, type_user_set, balance_current "
            "FROM accounts WHERE id='acc-misread'").fetchone()
    ...
    def test_classifying_a_card_flips_the_stored_balance_at_once(self):
        ...
        self._sync(payload)
        self.assertEqual(self.conn.execute(
            "SELECT balance_current FROM accounts WHERE id='acc-misread'"
