...
def clear_totp_burn(email: str | None = None) -> None:
    ...
    One-time codes are one-time ACCOUNT-WIDE: login
    always burned the counter, but every step-up / posture-change door
    (password change, TOTP re-enroll/disable, recovery regenerate, account
    erasure, email change) used a bare `verify()` and consumed nothing, so
    one observed code opened them repeatedly inside its ~90s window.

    A test that presents the same code at two doors is therefore now doing a
    replay, and gets refused — correctly. Most of these tests are exercising
    the step-up GATE, not replay protection, and minting a second code inside
    the same 30-second step is not something they can do. Calling this first
    says plainly "this test is not about replay"; the replay contract itself
    is pinned by test_auth_hardening's
