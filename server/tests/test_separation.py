...
class LedgerListsBusinessRowsLabelledTests(unittest.TestCase):
    ...
    spend. Hiding them outright would make a business account's own
    history unreachable from the Accounts page (0 results), and the
    aggregates already carry PERSONAL_ONLY_SQL, so hiding buys nothing.
...
class BusinessSettingsTabTests(unittest.TestCase):
    ...
    def test_entity_edit_is_config_too(self):
        ...
        line — the lifecycle controls sit in a flex row under the same
        guard, and a layout change there alters nothing about where they
        render or who may press them."""
        b = self._src("pages/Business.tsx")
