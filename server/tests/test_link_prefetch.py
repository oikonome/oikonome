...
and chat unfurlers and browser prefetchers do the same. /unlock consumed its
token inside the GET handler, so the scan burned it and the Turnstile escape
hatch — whose whole audience is people behind corporate proxies, i.e. exactly
the mail that gets prescanned — expired before they clicked. It now peeks on
GET and acts on POST, like /admin/approve-request and /reset.
...
class LinkPrefetchTests(unittest.TestCase):
    ...
    def test_verification_click_is_the_whole_task(self):
        ...
        self.assertIn("Email confirmed", r.text)
        self.assertIn("/login?verified=1", r.headers.get("refresh", ""))
        # the old POST form still works the same way
    ...
    def test_spent_links_still_refuse(self):
        """The one-time property must survive the split."""
        email, client = self._account()
        with self.appmod._control_conn() as conn:
            token = login_unlock.create(conn, _user_row(email)["id"])
