...
class _Base(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        try:
            ...
            # suite pass exactly once per database (test_passkeys learned
            # this the hard way).
...
class DashboardStepupFlagTests(_Base):
    def test_authed_dashboard_arms_the_enrol_stepup_once_a_key_exists(self):
        ...
        passkeys_enrolled, which the AUTHED dashboard never passed —
        Jinja's Undefined is falsy, so on any console with one key the
        second-key enrolment ran a full WebAuthn ceremony and then 403'd
        on the missing step-up ticket, every time."""
...
class TenantDeleteStepupTests(_Base):
    def test_immediate_purge_needs_a_fresh_assertion_once_keys_exist(self):
        """An immediate tenant purge is as destructive (to
        that customer) as host restore/reset/uninstall, which already
        demand a fresh single-use passkey assertion — a stolen console
        cookie could not reset the box but could permanently destroy any
        household. Same gate now."""
        self._token_login()
        self._enrol()
...
class PasskeyOnlyTests(_Base):
    ...
    def test_ticket_redeemed_twice_in_a_race_stores_exactly_one_key(self):
        ...
        the second used to be logged and forgiven. The burn is the only
...
class StepUpTests(_Base):
    ...
    def test_host_reboot_needs_typed_confirm_and_a_fresh_assertion(self):
        """A reboot takes every instance on the box down. The armed banner
        used to be nonce-only, so a stolen cookie could arm and go in two
        POSTs; now the go leg needs the typed word and, once a key is
        enrolled, a fresh passkey assertion — the same gates as the
...
class EnrolTicketWithLiveSessionTests(_Base):
    def test_a_host_ticket_enrols_even_when_the_browser_holds_a_session(self):
        ...
        in. `by_ticket` used to require `sess is None`, so a console cookie
        skipped the ticket path and fell through to the step-up branch, which
        demands a passkey you already hold — exactly the one that was lost.
