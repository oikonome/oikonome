...
processor posts beside the water bill used to be a stranger to it —
proposed as its own $1 bill, missing from the bill's cost. A companion
...
def _bill_with_fee(conn, **kw):
    add_bill(conn, "City Of Springfield", 90.60, frequency="MONTHLY",
             next_due=dt.date(TODAY.year, TODAY.month, 5),
             companions=[{"tokens": "dp convenience", "amount": 1.0,
                          "window_days": 3}], **kw)
...
class CompanionMatchingTests(unittest.TestCase):
    ...
    def test_companion_attaches_within_window_and_plans_on_top(self):
        ...
        add_txn(self.conn, d, 90.60, "CITY OF SPRINGFIELD WEB PMTS",
                primary="RENT_AND_UTILITIES")
        add_txn(self.conn, d + dt.timedelta(days=1), 1.00, "DP CONVENIENCE F",
                primary="GENERAL_SERVICES")
        matched, occs, rows = self._occs()
        occ = next(o for o in occs if o["bill"]["payee"] == "City Of Springfield")
        self.assertEqual(occ["planned"], 91.60)          # charge + fee
    ...
    def test_companion_outside_the_window_is_not_attached(self):
        ...
        d = dt.date(TODAY.year, TODAY.month, 5)
        add_txn(self.conn, d, 90.60, "CITY OF SPRINGFIELD WEB PMTS")
        add_txn(self.conn, d + dt.timedelta(days=9), 1.00, "DP CONVENIENCE F")
    ...
    def test_paid_amount_counts_both_legs(self):
        ...
        d = dt.date(TODAY.year, TODAY.month, 5)
        add_txn(self.conn, d, 90.60, "CITY OF SPRINGFIELD WEB PMTS")
        add_txn(self.conn, d, 1.00, "DP CONVENIENCE F")
        stats = web_api._month_bill_stats(self.conn, budget.load_config(self.conn),
                                          TODAY)
        self.assertEqual(stats["City Of Springfield"]["paid_amount"], 91.60)
    ...
    def test_fee_row_inherits_the_bills_transaction_category(self):
        ...
        add_txn(self.conn, d, 90.60, "CITY OF SPRINGFIELD WEB PMTS",
                txn_id="main")
        add_txn(self.conn, d, 1.00, "DP CONVENIENCE F", txn_id="fee")
    ...
    def test_save_keeps_companions_unless_told(self):
        _bill_with_fee(self.conn)
        bills.save_bill(self.conn, payee="City Of Springfield", amount=92,
                        frequency="MONTHLY", next_due=TODAY)
        raw = as_dict(bills.get_bill(self.conn, "City Of Springfield")["raw"])
...
class CompanionDetectionTests(unittest.TestCase):
        ...
        for i in range(n, 0, -1):
            ...
            if fee:
                add_txn(self.conn, d + dt.timedelta(days=1), 1.00,
                        "DP CONVENIENCE F", primary="GENERAL_SERVICES")
    ...
    def test_a_riding_fee_is_offered_as_an_attachment_not_a_bill(self):
        add_bill(self.conn, "City Of Springfield", 90.60, frequency="MONTHLY",
                 next_due=dt.date(TODAY.year, TODAY.month, 5))
        self._seed_months()
        bills.run(self.conn, TODAY)
        kinds = {(k, p) for k, p, _ in self._pending()}
        self.assertNotIn(("add", "Dp Convenience F"), kinds, kinds)
        att = [(k, p, ev) for k, p, ev in self._pending() if k == "attach"]
        self.assertEqual(len(att), 1, self._pending())
        _, payee, ev = att[0]
        self.assertEqual(ev["bill_payee"], "City Of Springfield")
        ...
        self.assertFalse([p for p in self._pending()
                          if p[1].lower().startswith("dp convenience")])
...
class CompanionFeeInMonthStatusTests(unittest.TestCase):
    ...
    Planned carries the companion's fee; the actual used to add only the
    main charge, so every companion-carrying bill reported a phantom
    negative drift equal to its own fee, forever, and the Today page's
    pinned 'paid' understated what actually left the account."""
    ...
    def test_actual_and_drift_count_the_fee(self):
        ...
        fx = ms["buckets"]["fixed"]
        self.assertAlmostEqual(fx["actual"], 91.60, places=2)
        self.assertNotIn("City Of Springfield", ms["drifts"],
                         "a paid fee is not drift — planned already carries it")
        pin = ms["pinned_bills"]
        row = next((p for p in (pin.values() if isinstance(pin, dict) else pin)
                    if p["payee"] == "City Of Springfield"), None)
        if row is not None:
            self.assertAlmostEqual(row["paid"], 91.60, places=2)
