...
vet, a groomer, a supply shop) that no merchant matcher can unify.
"""
...
class CategoryEnvelopeTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        add_bill(self.conn, "Pet Care", 1200.0, bill_type="envelope",
                 frequency=None, interval=1, category="PETS",
    ...
    def test_unrelated_merchants_pool_by_category(self):
        ...
        add_txn(self.conn, TODAY, 300.0, "NORTHWIND VET",
                override="PETS")
        add_txn(self.conn, TODAY - dt.timedelta(days=2), 50.0,
                "PAWS GROOMING", override="PETS")
        add_txn(self.conn, TODAY - dt.timedelta(days=4), 40.0,
                "CORNER PET SUPPLY", override="PETS")
        st = self.status()
        env = st["buckets"]["fixed"]["envelopes"][0]
        self.assertAlmostEqual(env["used"], 390.0, places=2)
        self.assertAlmostEqual(env["period_left"], 1200.0 - 390.0, places=2)
    ...
    def test_occurrence_bill_still_wins_the_row(self):
        ...
        add_bill(self.conn, "Kennel Co", 500.0,
                 next_due=TODAY, last_seen=TODAY - dt.timedelta(days=31),
                 category="PETS")
        add_txn(self.conn, TODAY, 500.0, "KENNEL CO", override="PETS")
        st = self.status()
        env = st["buckets"]["fixed"]["envelopes"][0]
    ...
    def test_without_flag_category_spend_stays_variable(self):
        ...
        st = self.status()
        self.assertEqual(st["buckets"]["fixed"]["envelopes"][0]["used"], 0.0)
        self.assertAlmostEqual(st["buckets"]["other"]["actual"], 300.0,
                               places=2)
...
class CardCompositionTests(unittest.TestCase):
    ...
    def test_envelope_card_says_whats_left_this_month(self):
        add_bill(self.conn, "Pet Care", 1200.0, bill_type="envelope",
                 frequency=None, interval=1, category="PETS",
                 match_category=True, show_today=True)
        add_txn(self.conn, TODAY, 300.0, "NORTHWIND VET", override="PETS")
        (c,) = self.cards()
        self.assertEqual(c["kind"], "envelope")
        self.assertEqual(c["label"], "Pet Care")
        self.assertAlmostEqual(c["left"], 900.0, places=2)
        self.assertFalse(c["over"])
        self.assertEqual(c["sub"], "used $300 of $1,200 this month")
    ...
    def test_spent_envelope_reports_the_overflow(self):
        add_bill(self.conn, "Pet Care", 100.0, bill_type="envelope",
                 frequency=None, interval=1, category="PETS",
                 match_category=True, show_today=True)
        add_txn(self.conn, TODAY, 150.0, "NORTHWIND VET", override="PETS")
        (c,) = self.cards()
...
class EmailMirrorTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        add_bill(self.conn, "Pet Care", 1200.0, bill_type="envelope",
                 frequency=None, interval=1, category="PETS",
                 match_category=True, show_today=True)
        add_txn(self.conn, self.NOW, 300.0, "NORTHWIND VET",
                override="PETS")
...
class SaveBillFlagTests(unittest.TestCase):
    ...
    def test_flags_survive_an_edit_that_omits_them(self):
        add_bill(self.conn, "Pet Care", 1200.0, bill_type="envelope",
                 ...
                 match_category=True, category="PETS")
        # a later edit (amount tweak) that doesn't mention the flags
        add_bill(self.conn, "Pet Care", 1300.0, bill_type="envelope",
                 frequency=None, interval=1)
        r = self.raw()
...
class BillsApiTests(unittest.TestCase):
    ...
    def test_save_and_read_back(self):
        r = self.client.post("/api/bills/save", json={
            "payee": "Pet Care", "amount": "1200", "cadence": "ENVELOPE:1",
            "show_today": True, "match_category": True,
            "category": "PETS"})
        self.assertEqual(r.status_code, 200, r.text)
        d = self.client.get("/api/bills/bill?payee=Pet Care").json()
        self.assertTrue(d["show_today"])
        self.assertTrue(d["match_category"])
        self.assertEqual(d["category"], "PETS")
        # an edit that omits the keys keeps them (the `cap` contract)
        r = self.client.post("/api/bills/save", json={
            "payee": "Pet Care", "amount": "1300", "cadence": "ENVELOPE:1",
            "orig_payee": "Pet Care"})
        self.assertEqual(r.status_code, 200, r.text)
        d = self.client.get("/api/bills/bill?payee=Pet Care").json()
        self.assertTrue(d["show_today"])
    ...
    def test_today_full_ships_cards(self):
        r = self.client.post("/api/bills/save", json={
            "payee": "Pet Pool", "amount": "200", "cadence": "ENVELOPE:1",
            "show_today": True, "match_category": True,
            "category": "PETS"})
        self.assertEqual(r.status_code, 200, r.text)
        cards = self.client.get("/api/today/full").json()["bill_cards"]
        self.assertTrue(any(c["label"] == "Pet Pool"
