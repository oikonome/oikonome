...
twin: the hosted guard must not also disable the self-hoster.
"""
...
class HostedMailResolveTests(unittest.TestCase):
    ...
    but a tenant row already stored would otherwise keep winning."""
    ...
    def test_hosted_ignores_a_tenant_relay_saved_earlier(self):
        with mock.patch.dict(os.environ,
                             {"OIKONOME_HOSTED": "1",
                              "OIKONOME_SMTP_HOST": "relay.operator.example",
                              "OIKONOME_SMTP_FROM": "no-reply@example.com"}):
            s = report.resolve_smtp(self.conn)
        self.assertEqual(s["host"], "relay.operator.example")
        self.assertEqual(s["sender"], "no-reply@example.com")
        self.assertTrue(s["configured"])

    def test_self_host_still_prefers_the_tenant_relay(self):
        env = dict(os.environ)
        env.pop("OIKONOME_HOSTED", None)
        env["OIKONOME_SMTP_HOST"] = "relay.operator.example"
