...
// the web and the daily email all round this way; half-away-from-zero
// showed -$187 where every other surface says -$186
...
// an override on the EMPTY string — sending the literal once wrote an
...
// Feeding it to Date once rendered "Invalid Date" on every phone, so
// the passthrough lives here where node can pin the contract.
...
/** "Trader Joe's" → "TJ", "Costco" → "Co", "7-Eleven" → "7E" (web monogram) */
 ...
 *  with password_required, which is what the sheet used to do and why a
 *  lost authenticator was a dead end: the recovery form was reachable
 *  only from the passkey path.
 *
...
/** Which provider doors the Connect hub offers, given where we run.
...
// One predicate for every door that adds a bank, because the screens
// disagreed: the Accounts card showed "used of cap" while both connect
// hubs promised as many institutions as you like and left the button
// live, so onboarding sold a limit the add door then refused — and on
// the hosted link path that refusal can land AFTER the bank has been
// authorized, so the household watches a connection it just made
...
// then dropped June from a "3m" window whose server figures covered
// Jun+Jul+Aug — the caption's best and worst month were computed over a
// window one month shorter than the one being read.
//
...
// that offers the removal is usually the screen showing the allowance, and
// with ["me"] left out of the refresh the household disconnected a bank to
// make room and the Connect button stayed disabled behind a cached count.
...
/** `iso` ("YYYY-MM-DD") moved `months` months earlier, with the day
 *  clamped to the target month's length — May 31 − 3 months is Feb 28,
 *  not Mar 3. */
...
// — or a mark written by a build that marked the first step on Continue —
// would otherwise reopen on "when" with the name blank and the field that
