...
export default function TxnDetail() {
  ...
  const hist = useQuery({
    queryKey: ["payee-history", initial?.payee],
    queryFn: () => client!.billsHistory(initial!.payee),
  ...
  const setCat = useMutation({
    ...
    // call and the local echo can never disagree again
      ...
      <Card>
        ...
        <View style={{ flexDirection: "row", gap: 8, marginTop: 8,
          ...
          {/* a check image rarely starts at this camera: the bank's app
              downloads the front, or a scanner makes a PDF. Without
              these two the only way onto the phone was to photograph a
              screen, which parses badly. */}
          <Pressable style={[s.btn, s.btnQuiet]} disabled={upload.isPending}
