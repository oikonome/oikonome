    ...
    <ScrollView style={s.wrap} contentContainerStyle={{ paddingBottom: 24 }}
      ...
      {(d?.coinbase_pl ?? []).length > 0 && (
        <Card>
          ...
          <View style={s.plRow}>
            <Text style={[s.mut, { flex: 1 }]}> </Text>
            <Text style={s.plHead}>Cash in</Text>
            <Text style={s.plHead}>Cash out</Text>
            <Text style={s.plHead}>Value</Text>
            <Text style={s.plHead}>P/L</Text>
            <Text style={[s.plHead, { width: 52 }]}>Ret.</Text>
          </View>
          {d!.coinbase_pl.map((c) => (
            <View key={c.name}
                  style={[s.plRow, c.name === "Combined"
                    && { borderTopColor: C.text, borderTopWidth: 1 }]}>
              <Text style={[s.plName, { flex: 1 }]}>{c.name}</Text>
              <Text style={s.plNum}>{m$(c.cash_in ?? 0)}</Text>
              <Text style={s.plNum}>{m$(c.cash_out ?? 0)}</Text>
              <Text style={s.plNum}>{m$(c.value)}</Text>
              <Text style={[s.plNum,
