...
export default function Accounts() {
  ...
  // supplier the household cannot choose only invites the question. Self-host keeps both: there it is the
  // owner's call. (Mirrors the same gate on the web Accounts card.)
  const me = useMe();
  ...
  const conns = useQuery({
    ...
    // already refreshes on completion. The idle branch used to be an
    // unconditional 30s timer that ran all day on every tab, each tick
    // re-serializing the whole persisted cache (accounts incl. base64
    // logos, every transactions page) into MMKV.
  ...
  const remove = useMutation({
    ...
    onSuccess: (r) => {
      ...
      // ["me"] invalidate used to sit under an unbraced `if` that read as
      // conditional and was not — the allowance on /api/me is exactly the
      // thing a disconnect frees, so state that rather than leave it to
      // the indentation.
  ...
  return (
    <ScrollView ref={topRef} style={s.wrap}
      ...
      {/* There is no global "Sync now". A sync reads the aggregator's
          ...
          anyway — so the button was nearly always a no-op, which is how
          a person learns a control is broken. Each connection keeps its
      ...
      {/* one row, two small doors: the bank sign-in (Plaid) and a manual
          account. The paragraph that used to introduce Plaid is gone —
      ...
      {categories.map(({ cat, insts, total }) => (
        <View key={cat}>
          ...
          {insts.map(([inst, list]) => {
            ...
            // 'broken' now means ACTIONABLE — a bank outage is amber and
...
// in the tab: every keystroke used to re-render the whole Accounts screen
