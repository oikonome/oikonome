...
export default function Bills() {
  ...
  // was a raw promise inside the Alert callback — no in-flight guard, so
  // a double tap stacked Alerts and errors only ever surfaced as another
  ...
  // the draft's own fields live in AddBillFields (keyed on `adding`) —
  // a keystroke there used to re-render this whole tab: every BillRow,
  // the cadence-group passes below, the envelope IIFE, the archived list
  // and CalendarSection
                ...
                <View style={{ flexDirection: "row", alignItems: "center",
                  ...
                  {p.kind ? (
                    <Pill text={p.kind === "attach" ? "fee"
                            : p.kind === "merchant" ? "merchant" : p.kind}
