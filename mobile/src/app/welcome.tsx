...
export default function Welcome() {
  ...
  // Today reads it (index.tsx). Falling back to the server URL when the
  // /api/me query had not resolved yet filed the flag under a key Today
  // never looks at, so a tap on "finish later" during the first second
  // re-offered the wizard on every launch. Wait for the id.
  ...
  // above six hooks, that flip changes the hook count and takes the app
  // down ("Rendered fewer hooks than expected") every time the wizard
  // resumes from Plaid's browser.
  ...
  // step was marked done (its save is what invalidates settings) read
  // the pre-save marks and resumed at the step just finished
  useEffect(() => {
 ...
 *  no schedule saved the daily email is ON by default, so the old lone
 *  "Enable" (which also wrote keys the server never read — email_to /
 *  email_daily — and so did nothing) is gone. */
function FinishStep({ d }: {
  ...
  // the web recap's budgets line: "$170 food / $1,190 everything-else"
