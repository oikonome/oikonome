      ...
      {pending.length > 0 && (
        <Card>
          ...
          {pending.map((p) => {
            ...
            // second tap (which used to toggle partial straight back)
            ...
            <View key={p.id} style={[s.row, unflagBusy && { opacity: 0.5 }]}>
              <View style={{ flex: 1 }}>
                ...
                {/* a PARTIAL flag shows its progress from $0 — hiding
                    the line until money arrived made "partial" invisible
                    exactly when it matters (the web shows got $0 of $X) */}
