...
export default function Login() {
  ...
  // The device-limit refusal, held with the roster it carried. The old
  // behaviour was to print the server's sentence — "revoke an unused
  // device in Settings first" — on a screen the person cannot get past,
  // so the Settings it named were on the far side of the door and the
  // only way through was to find another device. Now the choice is here.
  ...
  const passkey = async (replaceDeviceId = "") => {
    ...
    try {
      ...
      if (tookDeviceLimit(e, "passkey")) {
        ...
        // because "didn't work" once hid a fully broken flow behind a
  ...
  return (
    <View style={s.wrap}>
      ...
      {/* the same door the web login has — the reset flow is mail-driven
          ...
          tab (a locked-out person had no way out of this screen before) */}
      <Pressable disabled={busy}
